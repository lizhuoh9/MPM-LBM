...
STEP147_SATURATION_STATIONARITY_PHASE = "planeflux_saturation_stationarity48"
STEP147_SATURATION_STATIONARITY_ROLE = "saturation_stationarity_diagnostic_48"
...
STEP146_FINAL_REPOSITORY_COMMIT = "54afab0c6b4bdae05fa08f50f274e8d2f557e1d9"
...
STEP146_READINESS_RELATIVE_PATH = (
...
def _step136_cap_slug(value: float) -> str:
    return f"{float(value):.4f}".rstrip("0").rstrip(".").replace(".", "p")


def step121_plane_flux_ramp_tuned_48_specs(output_interval: int = 5) -> List[Step120RunSpec]:
    interior_specs = step121_plane_flux_interior_diag_48_specs(output_interval=output_interval)
    regularized_base = next(
        spec
        for spec in interior_specs
        if spec.open_boundary_semantics == "regularized_plane_flux_controlled_pressure_outlet"
        and int(spec.open_boundary_inlet_ramp_steps) == 100
        and math.isclose(float(spec.niu), 0.10, rel_tol=0.0, abs_tol=1.0e-12)
    )
    tuning_plan = [
        (100, 0.35, 0.005, 1.00),
        (100, 0.50, 0.005, 1.00),
        (100, 0.50, 0.0075, 1.00),
        (100, 0.50, 0.005, 0.95),
        (100, 0.50, 0.005, 0.90),
        (75, 0.50, 0.005, 1.00),
    ]
    specs: List[Step120RunSpec] = []
    for ramp_steps, gain_u, cap_u, target_scale in tuning_plan:
        specs.append(
            _replace_spec(
                regularized_base,
                name=(
                    ...
                    f"_gain{_step132_gain_slug(gain_u)}"
                    f"_cap{_step136_cap_slug(cap_u)}"
...
def step121_plane_flux_ramp_tuned_tiny_smoke_specs() -> List[Step120RunSpec]:
    target_scale_base = next(
        spec
        for spec in step121_plane_flux_ramp_tuned_48_specs(output_interval=5)
        if math.isclose(float(spec.open_boundary_flux_control_target_scale), 0.95, rel_tol=0.0, abs_tol=1.0e-12)
    )
    return [
        _replace_spec(
            target_scale_base,
            name="tiny_step136_ramped_throughput_calibration_smoke",
            nx=8,
            ny=6,
            nz=6,
            n_steps=20,
            ...
            row_role="tiny_smoke",
            artifact_scope_note=(
                "Step136 tiny smoke for ramped-inlet throughput calibration wiring; "
...
def step121_plane_flux_ramp_refined_48_specs(output_interval: int = 5) -> List[Step120RunSpec]:
    tuned_specs = step121_plane_flux_ramp_tuned_48_specs(output_interval=output_interval)
    regularized_base = next(
        spec
        ...
        and math.isclose(float(spec.open_boundary_flux_correction_cap_u), 0.005, rel_tol=0.0, abs_tol=1.0e-12)
        and math.isclose(float(spec.open_boundary_flux_control_target_scale), 0.90, rel_tol=0.0, abs_tol=1.0e-12)
    )
    refined_plan = [
        (90, 0.90),
        (90, 0.85),
    ...
    specs: List[Step120RunSpec] = []
    for ramp_steps, target_scale in refined_plan:
        specs.append(
            _replace_spec(
                regularized_base,
                name=(
                    "duct_only_48_regularized_plane_flux_controlled"
                    f"_gain{_step132_gain_slug(regularized_base.open_boundary_flux_feedback_gain_u)}"
                    f"_cap{_step136_cap_slug(regularized_base.open_boundary_flux_correction_cap_u)}"
                    f"_rho{_step133_param_slug(regularized_base.open_boundary_flux_feedback_gain_rho, 4)}"
                    f"_alpha{_step133_param_slug(regularized_base.open_boundary_flux_filter_alpha, 3)}"
                    f"_du{_step133_param_slug(regularized_base.open_boundary_flux_feedback_delta_cap_u, 5)}"
                    f"_slew{_step133_param_slug(regularized_base.open_boundary_flux_feedback_slew_alpha, 2, strip_trailing=False)}"
                ...
                open_boundary_flux_control_target_scale=float(target_scale),
                artifact_scope_note=(
                    "Step137 bounded 48^3 refined ramp-target throughput window; "
                    "diagnostic only and not selected96 or 500-step evidence"
                ),
            )
        )
...
def step121_plane_flux_ramp_refined_tiny_smoke_specs() -> List[Step120RunSpec]:
    target_scale_base = next(
        spec
        for spec in step121_plane_flux_ramp_refined_48_specs(output_interval=5)
        if int(spec.open_boundary_inlet_ramp_steps) == 90
        and math.isclose(float(spec.open_boundary_flux_control_target_scale), 0.85, rel_tol=0.0, abs_tol=1.0e-12)
    )
    return [
        _replace_spec(
            target_scale_base,
            name="tiny_step137_ramp_target_refinement_smoke",
            nx=8,
            ny=6,
            nz=6,
            n_steps=20,
            ...
            row_role="tiny_smoke",
            artifact_scope_note=(
                "Step137 tiny smoke for refined ramp-target throughput wiring; "
                "not validation, selected96, or 500-step evidence"
            ),
        )
    ]
...
def _step143_step142_readiness_path() -> Path:
    return REPO_ROOT / STEP142_READINESS_RELATIVE_PATH


def _step143_step142_readiness_provenance() -> Dict[str, Any]:
    path = _step143_step142_readiness_path()
    payload = _read_json(path)
    design = dict(payload.get("recommended_design") or {})
    return {
        "source_step142_readiness_hash": sha256(path.read_bytes()).hexdigest(),
        "source_step142_readiness_path": STEP142_READINESS_RELATIVE_PATH,
        "source_step142_status": payload.get("status"),
        "source_step142_recommended_design": design.get("primary_mode"),
    }


def _step143_mass_gain_slug(value: float) -> str:
    if float(value) == 0.0:
        return "off"
    return f"{float(value):.2f}".replace(".", "p")


def _step143_mass_cap_slug(value: float) -> str:
    if float(value) == 0.0:
        return "0"
...
def step121_mass_neutral_design_48_specs(output_interval: int = 5) -> List[Step120RunSpec]:
    source = next(
        spec
        for spec in step121_density_feedback_isolation_48_specs(output_interval=5)
        if math.isclose(float(spec.open_boundary_flux_feedback_gain_rho), 0.001, rel_tol=0.0, abs_tol=1.0e-12)
    )
    step142 = _step143_step142_readiness_provenance()
    sweep = [
        ("disabled", False, "disabled", 0.0, 0.0, 0.0),
        ("low", True, "global_mass_error_density_offset", 0.10, 0.00025, 1.0),
        ("mid", True, "global_mass_error_density_offset", 0.25, 0.00050, 1.0),
        ("high", True, "global_mass_error_density_offset", 0.50, 0.00100, 1.0),
    ]
    specs: List[Step120RunSpec] = []
    for label, enabled, mode, gain_mass, cap_mass, blend in sweep:
        specs.append(
            _replace_spec(
                source,
                name=(
                    ...
                    f"_mn{label}"
                    f"_mgain{_step143_mass_gain_slug(gain_mass)}"
                    f"_mcap{_step143_mass_cap_slug(cap_mass)}"
                    f"_blend{_step133_param_slug(blend, 2, strip_trailing=False)}"
                    f"_out{int(output_interval)}"
                    "_250step_mass_neutral_design"
                ),
                n_steps=250,
                requested_n_steps=250,
                output_interval=output_interval,
                row_role=STEP143_MASS_NEUTRAL_DESIGN_ROLE,
                allow_large_real_run_without_flag=True,
                not_used_for_validation=True,
                source_step=142,
                source_row_name=source.name,
                source_solver_state_hash=solver_state_hash_for_spec(source),
                source_run_manifest_hash=run_manifest_hash_for_spec(source),
                source_code_commit=STEP142_SOURCE_CODE_COMMIT,
                open_boundary_mass_neutral_flux_control_enabled=bool(enabled),
                open_boundary_mass_neutral_flux_control_mode=str(mode),
                open_boundary_mass_neutral_mass_error_gain=float(gain_mass),
                open_boundary_mass_neutral_mass_error_cap=float(cap_mass),
                open_boundary_mass_neutral_correction_blend=float(blend),
                open_boundary_mass_neutral_reference_mass_mode="initial",
                artifact_scope_note=(
                    "Step143 bounded 48^3 / 250-step mass-neutral design diagnostic; "
                    "not selected96, not selected-static, not 96^3, not Fluent, not FSI, "
                    "not validation, and not 500-step evidence"
                ),
                **step142,
            )
        )
...
def _step147_step146_readiness_path() -> Path:
    return REPO_ROOT / STEP146_READINESS_RELATIVE_PATH


def _step147_step146_readiness_provenance() -> Dict[str, Any]:
    path = _step147_step146_readiness_path()
    payload = _read_json(path)
    if payload.get("status") != "design_ready":
        raise ValueError("Step147 requires Step146 design_ready status")
    if payload.get("step147_250step_diagnostic_proposal_allowed") is not True:
        raise ValueError("Step147 requires Step146 to allow a bounded 250-step diagnostic")
    if payload.get("step146_500step_probe_allowed") is not False:
        raise ValueError("Step147 requires Step146 500-step probe to remain blocked")
    if payload.get("selected96_execution_allowed") is not False:
        raise ValueError("Step147 requires Step146 selected96 execution to remain blocked")
    if payload.get("recommended_step147_phase") != STEP147_SATURATION_STATIONARITY_PHASE:
        raise ValueError("Step147 readiness artifact recommends a different phase")
    if payload.get("recommended_step147_row_role") != STEP147_SATURATION_STATIONARITY_ROLE:
        raise ValueError("Step147 readiness artifact recommends a different row role")
    return {
        "source_step146_readiness_hash": sha256(path.read_bytes()).hexdigest(),
        "source_step146_readiness_path": STEP146_READINESS_RELATIVE_PATH,
        "source_step146_status": payload.get("status"),
        "source_step146_recommended_design": payload.get("recommended_design"),
        "source_step146_recommended_phase": payload.get("recommended_step147_phase"),
        "source_step146_recommended_row_role": payload.get("recommended_step147_row_role"),
        "source_step145_decision_case": payload.get("source_step145_decision_case"),
        "source_step144_decision_case": payload.get("source_step144_decision_case"),
    }


def step121_saturation_stationarity_48_specs(output_interval: int = 5) -> List[Step120RunSpec]:
    source = step121_mass_neutral_final_48_specs(output_interval=10)[0]
    step146 = _step147_step146_readiness_provenance()
    sweep = [
        (
            ...
            1.00,
            0.50,
            "baseline repeat only; repeats the Step144 high mass-neutral setting at 250 steps",
        ),
        (
            "relief_low_slew025",
            "duct_only_48_regularized_plane_flux_controlled_gain0p75_cap0p0075_rho0p0010"
            ...
            0.50,
            0.25,
            "mass-neutral relief plus stationarity damping",
        ),
        (
            "relief_mid_slew025",
            "duct_only_48_regularized_plane_flux_controlled_gain0p75_cap0p0075_rho0p0010"
            ...
            0.50,
            0.25,
            "same mass-neutral gain and cap as baseline with lower blend and stronger stationarity damping",
        ),
        (
            ...
            0.50,
            0.25,
            "diagnostic cap test only; not a promotion row",
        ),
    ]
    specs: List[Step120RunSpec] = []
    for label, name, gain_mass, cap_mass, blend, slew_alpha, purpose in sweep:
        specs.append(
            _replace_spec(
                source,
                name=name,
                ...
                requested_n_steps=250,
                output_interval=output_interval,
                row_role=STEP147_SATURATION_STATIONARITY_ROLE,
                allow_large_real_run_without_flag=True,
                not_used_for_validation=True,
                source_step=146,
                source_row_name=source.name,
                source_solver_state_hash=solver_state_hash_for_spec(source),
                source_run_manifest_hash=run_manifest_hash_for_spec(source),
                source_code_commit=STEP146_FINAL_REPOSITORY_COMMIT,
                source_step142_readiness_hash=None,
                source_step142_readiness_path=None,
                source_step142_status=None,
                source_step142_recommended_design=None,
                source_step143_decision_hash=None,
                source_step143_decision_path=None,
                source_step143_comparison_hash=None,
                source_step143_comparison_path=None,
                source_step143_best_row_name=None,
                source_step143_best_row_solver_state_hash=None,
                source_step143_best_row_run_manifest_hash=None,
                source_step143_best_row_mass_neutral_activation_hash=None,
                source_step143_decision_case=None,
                ...
                open_boundary_mass_neutral_correction_blend=float(blend),
                open_boundary_mass_neutral_reference_mass_mode="initial",
                open_boundary_flux_feedback_slew_alpha=float(slew_alpha),
                open_boundary_flux_feedback_delta_cap_u=0.0005,
                artifact_scope_note=(
                    "Step147 bounded 48^3 / 250-step saturation-stationarity diagnostic; "
                    f"{purpose}; not selected96, not selected-static, not 96^3, not Fluent, "
                    "not FSI, not validation, not selected-candidate-surface review, "
                    "and not 500-step evidence"
                ),
                **step146,
            )
        )
...
def resolve_step121_phase_specs(
    ...
    *,
    ...
    if phase == STEP136_RAMP_TUNED_PHASE:
        return step121_plane_flux_ramp_tuned_48_specs(output_interval=output_interval)
    if phase == "planeflux_ramp_tuned48_tiny":
        return step121_plane_flux_ramp_tuned_tiny_smoke_specs()
    if phase == STEP137_RAMP_REFINED_PHASE:
    ...
    if phase == STEP147_SATURATION_STATIONARITY_PHASE:
        return step121_saturation_stationarity_48_specs(output_interval=output_interval)
...
def _manifest_rejection_reasons(
    ...
    checks = {
        ...
        "source_step142_readiness_hash": row.get("source_step142_readiness_hash"),
        ...
        "source_step146_readiness_hash": row.get("source_step146_readiness_hash"),
        "source_step146_readiness_path": row.get("source_step146_readiness_path"),
        "source_step146_status": row.get("source_step146_status"),
        "source_step146_recommended_design": row.get("source_step146_recommended_design"),
        "source_step146_recommended_phase": row.get("source_step146_recommended_phase"),
        "source_step146_recommended_row_role": row.get("source_step146_recommended_row_role"),
        "source_step145_decision_case": row.get("source_step145_decision_case"),
        "source_step144_decision_case": row.get("source_step144_decision_case"),
        "mass_neutral_label": row.get("mass_neutral_label"),
        "open_boundary_mass_neutral_flux_control_enabled": row.get(
            "open_boundary_mass_neutral_flux_control_enabled"
        ),
        "open_boundary_mass_neutral_flux_control_mode": row.get("open_boundary_mass_neutral_flux_control_mode"),
        "open_boundary_mass_neutral_reference_mass_mode": row.get(
            "open_boundary_mass_neutral_reference_mass_mode"
        ),
        "mass_neutral_activation_hash": row.get("mass_neutral_activation_hash"),
    }
    reasons: List[str] = []
    ...
    for key in (
        ...
        if actual_value is None or not math.isclose(float(actual_value), float(expected_value), rel_tol=1.0e-9, abs_tol=1.0e-12):
            reasons.append(f"{key}_mismatch")
...
def _manifest_run_commands() -> List[str]:
    return [
        ...
        "D:\\working\\taichi\\env\\python.exe -m experiments.steps.step121_lbm_boundary_real_campaign_and_gate_correction --phase planeflux_saturation_stationarity48 --allow-large-real-rows --output-interval 5",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "planeflux_saturation_stationarity48",
            "all48",
            "selected96",
            "selected-static",
            "summary",
