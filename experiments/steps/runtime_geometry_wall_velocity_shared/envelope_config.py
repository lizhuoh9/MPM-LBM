...
EXPECTED_ENVELOPE_ID = "step47_runtime_geometry_wall_velocity_short_step_envelope"
EXPECTED_PHASE_SEQUENCE = (0.0, 0.05, 0.1, 0.2, 0.35)
EXPECTED_N_GRID = 32
EXPECTED_N_LBM_STEPS = 5
...
class RuntimeGeometryWallVelocityShortStepEnvelopeConfig:
    short_step_envelope_id: str
    base_coupling_smoke_config_path: str
    ...
    scope_note: str = "32^3 five-step engineering-only runtime geometry plus wall velocity envelope"
...
def validate_short_step_envelope_config(config: RuntimeGeometryWallVelocityShortStepEnvelopeConfig, root=None) -> list[dict]:
    root_path = _repo_root() if root is None else Path(root)
    rows = [
        _check("short_step_envelope_id_expected", config.short_step_envelope_id == EXPECTED_ENVELOPE_ID, config.short_step_envelope_id, "Step 47 envelope id must be stable"),
        _check("base_coupling_smoke_config_path_exists", _exists(root_path, config.base_coupling_smoke_config_path), config.base_coupling_smoke_config_path, "Step 46 coupling smoke config must exist"),
        ...
        _check("n_grid_expected", config.n_grid == EXPECTED_N_GRID, config.n_grid, "Step 47 n_grid must be 32"),
        _check("n_lbm_steps_expected", config.n_lbm_steps == EXPECTED_N_LBM_STEPS, config.n_lbm_steps, "Step 47 must use five LBM steps"),
        _check("mpm_substeps_expected", config.mpm_substeps_per_lbm_step == EXPECTED_MPM_SUBSTEPS, config.mpm_substeps_per_lbm_step, "Step 47 must use five MPM substeps per LBM step"),
        _check("phase_sequence_expected", _same_phase_sequence(config.phase_sequence, EXPECTED_PHASE_SEQUENCE), list(config.phase_sequence), "Step 47 phase sequence must match the early-contraction envelope"),
        ...
        _check("coupling_mode_expected", config.coupling_mode == EXPECTED_COUPLING_MODE, config.coupling_mode, "Step 47 coupling mode must be moving_boundary"),
        _check("reaction_transfer_mode_expected", config.reaction_transfer_mode == EXPECTED_REACTION_TRANSFER_MODE, config.reaction_transfer_mode, "Step 47 transfer mode must be engineering"),
        ...
        _check("diagnostic_only_true", config.diagnostic_only is True, config.diagnostic_only, "Step 47 must remain diagnostic-only"),
        _check("scope_note_valid", "five-step" in config.scope_note and "engineering-only" in config.scope_note, config.scope_note, "scope note must state five-step engineering-only scope"),
    ]
    for field in MUTATION_FLAG_FIELDS:
...
def summarize_short_step_envelope_config_validation(rows: list[dict], config: RuntimeGeometryWallVelocityShortStepEnvelopeConfig) -> dict:
    return {
        "row_count": len(rows),
        "pass_count": sum(1 for row in rows if bool(row["pass"])),
        "validation_pass": all(bool(row["pass"]) for row in rows),
        "short_step_envelope_id": config.short_step_envelope_id,
...
def mutation_flags(config: RuntimeGeometryWallVelocityShortStepEnvelopeConfig) -> dict:
    return {field: bool(getattr(config, field)) for field in MUTATION_FLAG_FIELDS}


def mutation_flag_enabled_count(config: RuntimeGeometryWallVelocityShortStepEnvelopeConfig) -> int:
