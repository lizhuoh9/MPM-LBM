...
def summarize_short_step_envelope_quality(rows: list[dict]) -> dict:
    by_name = {row["row_name"]: row for row in rows}
    row_count_pass = len(rows) == 4
    step_count_pass = all(len(row["step_records"]) == 5 for row in rows)
    ...
    combined = by_name.get("runtime_geometry_plus_wall_velocity_32_5step", {})
...
def compare_short_step_components(rows: list[dict]) -> tuple[list[dict], dict]:
    by_name = {row["row_name"]: row for row in rows}
    pairs = [
        ("geometry_only_minus_original_static", "runtime_geometry_only_32_5step", "original_static_32_5step"),
        ("wall_velocity_only_minus_original_static", "wall_velocity_only_32_5step", "original_static_32_5step"),
        ("combined_minus_original_static", "runtime_geometry_plus_wall_velocity_32_5step", "original_static_32_5step"),
        ("combined_minus_geometry_only", "runtime_geometry_plus_wall_velocity_32_5step", "runtime_geometry_only_32_5step"),
        ("combined_minus_wall_velocity_only", "runtime_geometry_plus_wall_velocity_32_5step", "wall_velocity_only_32_5step"),
    ]
    comparison_rows = []
    ...
    summary = {
        ...
        "geometry_effect_active_cell_delta_nonzero": by_name["runtime_geometry_only_32_5step"]["active_cell_count_delta_from_original_max"] > 0,
        "wall_velocity_effect_applied_velocity_nonzero": by_name["wall_velocity_only_32_5step"]["max_applied_velocity_norm"] > 0.0,
        "combined_has_geometry_and_wall_velocity": bool(
            by_name["runtime_geometry_plus_wall_velocity_32_5step"]["runtime_geometry_projection_enabled"]
            and by_name["runtime_geometry_plus_wall_velocity_32_5step"]["wall_velocity_application_enabled"]
            and by_name["runtime_geometry_plus_wall_velocity_32_5step"]["max_applied_velocity_norm"] > 0.0
            and by_name["runtime_geometry_plus_wall_velocity_32_5step"]["active_cell_count_delta_from_original_max"] > 0
        ),
    }
...
def summarize_phase_progression(rows: list[dict]) -> tuple[list[dict], dict]:
    progression_rows = []
    for row in rows:
        ...
        progression["progression_pass"] = bool(math.isfinite(active_delta) and math.isfinite(velocity_delta) and len(row["step_records"]) == 5)
    ...
    summary = {
        ...
        "phase_sequence_pass": phase_sequence == [0.0, 0.05, 0.1, 0.2, 0.35],
        "runtime_geometry_phase_response_pass": abs(float(by_name["runtime_geometry_only_32_5step"]["active_cell_count_delta_phase0_to_phase035"])) > 0.0,
        "wall_velocity_phase_response_pass": abs(float(by_name["wall_velocity_only_32_5step"]["max_applied_velocity_delta_phase0_to_phase035"])) > 0.0,
        "combined_phase_response_pass": bool(
            abs(float(by_name["runtime_geometry_plus_wall_velocity_32_5step"]["active_cell_count_delta_phase0_to_phase035"])) > 0.0
            and abs(float(by_name["runtime_geometry_plus_wall_velocity_32_5step"]["max_applied_velocity_delta_phase0_to_phase035"])) > 0.0
        ),
        "phase0_to_phase035_projection_delta_nonzero": abs(float(by_name["runtime_geometry_only_32_5step"]["active_cell_count_delta_phase0_to_phase035"])) > 0.0,
        "phase0_to_phase035_wall_velocity_delta_finite": math.isfinite(float(by_name["wall_velocity_only_32_5step"]["max_applied_velocity_delta_phase0_to_phase035"])),
    }
...
def mass_force_bounceback_envelope(rows: list[dict]) -> tuple[list[dict], dict]:
    diag_rows = []
    for row in rows:
        diag = {
            "row_name": row["row_name"],
