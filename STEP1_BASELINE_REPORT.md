...
- `requirements.txt` is a minimal reproducible dependency list, not a full local environment freeze.
...
## 4. LBM All-Fluid Smoke Baseline

Command:

```powershell
& 'D:\working\taichi\env\python.exe' baseline_tests\run_lbm_smoke_baseline.py
```
...
- grid: `32 x 32 x 32`
- geometry: generated all-fluid smoke geometry, `outputs/lbm_smoke/geo_smoke_32.dat`
...
- VTK output: yes, `outputs/lbm_smoke/LB_SingelPhase_500.vtr`
- log: `logs/lbm_smoke_baseline.log`

Notes:

- This is an independent Single_phase LBM smoke baseline only.
- This is not a physical cavity case; the geometry is all-fluid by design.
...
## 5. LBM Pressure-Driven Poiseuille Baseline

Command:

```powershell
& 'D:\working\taichi\env\python.exe' baseline_tests\run_lbm_poiseuille_baseline.py
```
...
- backend: GPU, Taichi arch `cuda`
- grid: `32 x 32 x 32`
- geometry: generated channel geometry, `outputs/lbm_poiseuille/geo_poiseuille_32.dat`
- solid walls: `y=0`, `y=31`, `z=0`, `z=31`
- x driving: pressure/density boundary, `rho_in=1.0001`, `rho_out=1.0`
- viscosity input: `0.1`
- finished steps: 1000
- final max_v: `2.578184e-05`
- rho range: `1.000000e+00` to `1.000100e+00`
- mean_ux: `1.500350e-05`
- center_ux: `2.041298e-05`
- near_wall_ux: `3.753960e-06`
- VTK output: yes, `outputs/lbm_poiseuille/LB_SingelPhase_1000.vtr`
- log: `logs/lbm_poiseuille_baseline.log`

Notes:

- This is the physical channel-flow baseline to use as a Step 2 refactor comparison.
- The original upstream global-force path was not used for this baseline because enabling the upstream Guo force branch on CUDA did not enter iteration within a 300-second compile/run timeout. Step 2 will test the new `cell_force` path separately after refactor.
...
- `outputs/lbm_smoke/geo_smoke_32.dat`
- `outputs/lbm_smoke/LB_SingelPhase_500.vtr`
- `outputs/lbm_poiseuille/geo_poiseuille_32.dat`
...
- [x] LBM all-fluid smoke case runs for at least 500 steps
- [x] LBM smoke `max_v` is finite
- [x] LBM smoke `rho` is finite
- [x] LBM smoke writes VTK output
- [x] LBM Poiseuille channel case runs for at least 1000 steps
- [x] LBM Poiseuille `rho` remains in range
- [x] LBM Poiseuille `max_v < 0.05`
- [x] LBM Poiseuille x-velocity forms a channel-like profile
- [x] LBM Poiseuille writes VTK output
