...
def main():
    ...
    out_dir = ROOT / "outputs" / "step36_static_regression_smoke"
    rows = []
    for config_path in STEP36_STATIC_DRIVER_CONFIGS:
    ...
    assert_driver_summary(summary, expected_rows=3, expected_reports=0)
    if int(summary["static_application_row_count"]) != 3 or int(summary["experimental_application_row_count"]) != 0:
        raise RuntimeError(f"Step 36 static smoke has wrong application split: {summary}")

    write_rows_csv_npz(
        rows,
        out_dir / "static_regression_results.csv",
        out_dir / "static_regression_results.npz",
        STEP36_DRIVER_FIELDS,
    )
    write_json(out_dir / "static_regression_results.json", {"summary": summary, "rows": rows})
    marker = "[OK] Step 36 static regression smoke finished"
    write_log(
