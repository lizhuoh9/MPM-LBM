...
def _check_driver_configs_disable_heavy_outputs():
    for relative_path in STEP20_DRIVER_CONFIGS:
...
def _check_fixture_sizes():
    ...
    for name in ["voxel_sphere.npy", "cube.obj", "ellipsoid_proxy.obj", "voxel_sphere_metadata.json"]:
        ...
        if os.path.getsize(path) >= 200_000:
            raise RuntimeError(f"fixture is too large for Step 20: {path}")
...
def main():
    ...
    out_dir = os.path.join(ROOT, "outputs", "step20_artifact_manifest")
    ...
    if summary["large_file_count"] != 0:
        raise RuntimeError("Step 20 artifact manifest found large files")
    ...
    print("Step 20 artifact manifest")
    ...
    print("[OK] Step 20 artifact manifest finished")
