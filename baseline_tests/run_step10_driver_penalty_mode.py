...
def assert_final_stats_ok(final_row):
    ...
    if not np.all(np.isfinite(values)):
        raise RuntimeError("penalty driver diagnostics contain NaN or Inf")
    if final_row["fluid_mean_ux"] <= 0.0:
        raise RuntimeError("penalty driver fluid mean ux did not increase")
    if final_row["projection_zone_fluid_mean_ux"] <= 0.0:
        raise RuntimeError("penalty driver projection zone ux did not increase")
    if final_row["cell_force_max_norm"] <= 0.0:
        raise RuntimeError("penalty driver cell_force stayed zero")
    if final_row["hydro_force_max_norm"] <= 0.0:
        raise RuntimeError("penalty driver hydro_force stayed zero")
    if final_row["rho_min"] <= 0.95 or final_row["rho_max"] >= 1.05:
        raise RuntimeError(f"penalty driver rho outside accepted range: {final_row}")
    if final_row["lbm_max_v"] >= 0.1:
        raise RuntimeError(f"penalty driver lbm max_v exceeded threshold: {final_row}")
    if final_row["mpm_min_J"] <= 0.0:
        raise RuntimeError(f"penalty driver mpm min_J became non-positive: {final_row}")
    if final_row["mpm_max_speed"] >= 10.0:
        raise RuntimeError(f"penalty driver mpm max_speed exceeded threshold: {final_row}")


def main():
    ti.init(arch=ti.gpu, default_fp=ti.f32, kernel_profiler=False, print_ir=False)

    config = FSIDriverConfig.from_json(os.path.join(ROOT, "configs", "step10_penalty_default.json"))
    out_dir = os.path.join(ROOT, "outputs", "step10_driver_penalty")

    print("Step 10 driver penalty mode")
    ...
    if final_row["total_mpm_substeps"] != config.n_lbm_steps * config.mpm_substeps_per_lbm_step:
        raise RuntimeError("penalty driver total_mpm_substeps mismatch")
    if final_row["solid_mean_vx_norm"] >= initial_row["solid_mean_vx_norm"]:
        raise RuntimeError("penalty driver solid mean vx did not decrease")

    print(f"completed_lbm_steps={driver.current_lbm_step}")
    print(f"total_mpm_substeps={driver.total_mpm_substeps}")
    print(f"fluid_mean_ux_final={final_row['fluid_mean_ux']:.9e}")
    ...
    print("[OK] Step 10 driver penalty mode finished")
