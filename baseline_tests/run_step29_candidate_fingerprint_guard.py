...
def main():
    ...
    rows = []
    for descriptor_path in STEP29_DESCRIPTORS:
        ...
        geometry_config_path = STEP29_GEOMETRY_CONFIGS[current["candidate_id"]]
    ...
    if len(rows) != 2 or not all(row["guard_pass"] for row in rows):
        raise RuntimeError(f"Step 29 fingerprint guard failed: {rows}")

    out_dir = ROOT / "outputs" / "step29_candidate_fingerprint_guard"
    ...
    marker = "[OK] Step 29 candidate fingerprint guard finished"
    write_log(
