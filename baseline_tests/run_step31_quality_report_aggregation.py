...
def main():
    ...
    out_dir = ROOT / "outputs" / "step31_quality_report_aggregation"
    ...
    marker = "[OK] Step 31 quality report aggregation finished"
    write_log(
...
def _collect_reports():
    root_path = ROOT / "outputs" / "step31_static_driver_smoke"
    if not root_path.is_dir():
        raise RuntimeError("missing Step 31 static driver output root")
    rows = []
...
def _assert_row(row):
    if row["geometry_type"] != "squid_proxy" or row["quality_kind"] != "procedural_voxelized":
        raise RuntimeError(f"Step 31 quality report must be procedural squid proxy quality: {row}")
    if row["strict"] is not True or row["pass"] is not True or row["severity"] != "ok":
        raise RuntimeError(f"Step 31 quality report must pass strict gate: {row}")
    if int(row["warnings_count"]) != 0 or int(row["reasons_count"]) != 0:
        raise RuntimeError(f"Step 31 quality report must have zero warnings and reasons: {row}")
    if int(row["occupied_count"]) <= 0 or int(row["connected_component_count"]) <= 0:
        raise RuntimeError(f"Step 31 quality report has no procedural occupancy: {row}")
    if int(row["report_size_bytes"]) >= 100_000:
        raise RuntimeError(f"Step 31 quality report is too large: {row}")
...
def _assert_summary(summary):
    if int(summary["quality_report_count"]) != 4:
        raise RuntimeError(f"expected 4 Step 31 quality reports: {summary}")
    if int(summary["pass_count"]) != 4 or int(summary["strict_count"]) != 4:
        raise RuntimeError(f"all Step 31 quality reports must pass strict gates: {summary}")
    if int(summary["error_count"]) != 0 or int(summary["warning_count"]) != 0:
        raise RuntimeError(f"Step 31 quality reports have warnings or errors: {summary}")
    if int(summary["procedural_row_count"]) != 4:
        raise RuntimeError(f"Step 31 quality reports must all be procedural rows: {summary}")
    if int(summary["quality_report_total_size_bytes"]) <= 0 or int(summary["quality_report_max_size_bytes"]) >= 100_000:
        raise RuntimeError(f"Step 31 quality report size summary is wrong: {summary}")
