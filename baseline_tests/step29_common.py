...
def run_step29_stability_driver_case(driver_config_path, out_dir) -> dict:
    ...
    enforce_step29_driver_config(config, driver_config_path)
    driver = FSIDriver3D(config, str(out_dir))
    diagnostics = driver.run()
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step 29 driver case: {driver_config_path}")
    ...
    row["notes"] = "Step 29 controlled real geometry 64^3 short-window stability envelope; not real squid validation"
    assert_step29_stability_driver_row(row)
    return row


def enforce_step29_driver_config(config, config_path):
...
def assert_step29_stability_driver_row(row):
    if not as_bool(row["stable"]):
        raise RuntimeError(f"Step 29 driver row is not stable: {row}")
    if not as_bool(row["quality_check_enabled"]) or not as_bool(row["quality_check_strict"]):
        raise RuntimeError(f"Step 29 row must use strict quality checks: {row}")
    if not as_bool(row["quality_gate_strict"]) or not as_bool(row["quality_pass"]):
        raise RuntimeError(f"Step 29 strict quality gate failed: {row}")
    if row["quality_severity"] != "ok":
        raise RuntimeError(f"Step 29 quality severity must be ok: {row}")
    if int(float(row["quality_warnings_count"])) != 0 or int(float(row["quality_reasons_count"])) != 0:
        raise RuntimeError(f"Step 29 quality report must have zero warnings/reasons: {row}")
    if int(float(row["n_grid"])) != 64 or int(float(row["n_lbm_steps"])) != 20:
        raise RuntimeError(f"Step 29 driver row has the wrong grid or window: {row}")
    if int(float(row["completed_lbm_steps"])) < 20 or int(float(row["total_mpm_substeps"])) < 100:
        raise RuntimeError(f"Step 29 driver row did not finish configured short run: {row}")
    if float(row["rho_min_global"]) <= 0.95 or float(row["rho_max_global"]) >= 1.05:
        raise RuntimeError(f"Step 29 density out of range: {row}")
    if float(row["lbm_max_v_global"]) >= 0.1:
        raise RuntimeError(f"Step 29 velocity out of range: {row}")
    if float(row["mpm_min_J_global"]) <= 0.0 or float(row["mpm_max_speed_global"]) >= 10.0:
        raise RuntimeError(f"Step 29 MPM diagnostics out of range: {row}")
    if float(row["projected_mass"]) <= 0.0 or int(float(row["active_cell_count"])) <= 0:
        raise RuntimeError(f"Step 29 projection diagnostics invalid: {row}")
    if as_bool(row["has_nan"]) or as_bool(row["has_inf"]):
        raise RuntimeError(f"Step 29 driver row contains NaN or Inf: {row}")
    if row["mode"] != "moving_boundary":
        raise RuntimeError(f"Step 29 only accepts moving_boundary rows: {row}")
    if float(row["cell_force_max_norm"]) != 0.0:
        raise RuntimeError(f"Step 29 moving_boundary rows must keep cell force at zero: {row}")
    if float(row["hydro_force_max_norm"]) <= 0.0:
        raise RuntimeError(f"Step 29 row lacks positive hydrodynamic force diagnostics: {row}")
    if int(float(row["bb_link_count_min"])) <= 0 or int(float(row["bb_link_count_max"])) <= 0:
        raise RuntimeError(f"Step 29 row lacks moving-boundary links: {row}")
    if int(float(row["active_reaction_particle_count_max"])) <= 0:
        raise RuntimeError(f"Step 29 row lacks active reaction particles: {row}")
    if row["reaction_transfer_mode"] == "link_area_experimental":
        if not math.isfinite(float(row["area_scale_final"])):
            raise RuntimeError(f"Step 29 link-area row area scale is not finite: {row}")
        if not math.isfinite(float(row["raw_area_scale_final"])):
            raise RuntimeError(f"Step 29 link-area row raw area scale is not finite: {row}")
        if not (0.25 <= float(row["area_scale_final"]) <= 2.0):
            raise RuntimeError(f"Step 29 link-area final area scale out of range: {row}")
        if not (0.25 <= float(row["area_scale_min"]) <= float(row["area_scale_max"]) <= 2.0):
            raise RuntimeError(f"Step 29 link-area area scale bounds out of range: {row}")
    if not finite_values(row, excluded=short_driver_string_fields()):
        raise RuntimeError(f"Step 29 driver row has non-finite numeric diagnostics: {row}")


def stability_driver_summary(rows) -> dict:
    ...
    return {
        ...
        "scope_note": "Step 29 64^3 short-window stability envelope only; not real squid validation.",
    }


def assert_stability_driver_summary(summary):
    if int(summary["driver_row_count"]) != 4 or int(summary["candidate_count"]) != 2:
        raise RuntimeError(f"expected 4 Step 29 stability rows across 2 candidates: {summary}")
    if int(summary["mesh_row_count"]) != 2 or int(summary["voxel_row_count"]) != 2:
        raise RuntimeError(f"Step 29 geometry split is wrong: {summary}")
    if int(summary["moving_boundary_row_count"]) != 4:
        raise RuntimeError(f"Step 29 must only contain moving_boundary rows: {summary}")
    if int(summary["engineering_row_count"]) != 2 or int(summary["link_area_row_count"]) != 2:
        raise RuntimeError(f"Step 29 transfer split is wrong: {summary}")
    if int(summary["stable_count"]) != 4 or int(summary["quality_report_count"]) != 4:
        raise RuntimeError(f"all Step 29 rows must be stable and report-backed: {summary}")
    if int(summary["quality_pass_count"]) != 4 or int(summary["strict_count"]) != 4:
        raise RuntimeError(f"all Step 29 rows must pass strict quality gates: {summary}")
    if float(summary["min_rho_min_global"]) <= 0.95 or float(summary["max_rho_max_global"]) >= 1.05:
        raise RuntimeError(f"Step 29 density summary is out of range: {summary}")
    if float(summary["max_lbm_max_v_global"]) >= 0.1:
        raise RuntimeError(f"Step 29 velocity summary is out of range: {summary}")
    if float(summary["min_mpm_min_J_global"]) <= 0.0 or float(summary["max_mpm_max_speed_global"]) >= 10.0:
        raise RuntimeError(f"Step 29 MPM summary is out of range: {summary}")
    if float(summary["min_projected_mass"]) <= 0.0 or int(summary["min_active_cell_count"]) <= 0:
        raise RuntimeError(f"Step 29 projection summary is invalid: {summary}")
    if int(summary["min_bb_link_count_min"]) <= 0:
        raise RuntimeError(f"Step 29 moving-boundary link summary is invalid: {summary}")
    if not math.isfinite(float(summary["max_driver_total_time"])) or float(summary["max_driver_total_time"]) <= 0.0:
        raise RuntimeError(f"Step 29 driver timing summary is invalid: {summary}")
...
def case_name(config_path):
    return Path(config_path).stem.removeprefix("step29_stability_")
