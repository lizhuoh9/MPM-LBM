...
def main():
    ...
    driver_rows = [
        row
        for row in read_csv_rows("outputs/step29_64_stability_driver/stability_driver_results.csv")
    ...
    if int(summary["row_count"]) != 2 or int(summary["pass_count"]) != 2:
        raise RuntimeError(f"Step 29 area-scale envelope failed: {rows}")

    out_dir = ROOT / "outputs" / "step29_area_scale_envelope"
    ...
    marker = "[OK] Step 29 area-scale envelope finished"
    write_log("logs/step29_area_scale_envelope.log", [marker, f"row_count={summary['row_count']}"])
...
def _area_row(driver_row):
    ...
    if not diagnostics_path.is_file():
        raise RuntimeError(f"missing Step 29 link-area diagnostics series: {diagnostics_path}")
    initial = 1.0
