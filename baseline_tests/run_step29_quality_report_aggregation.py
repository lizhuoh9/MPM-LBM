...
def main():
    ...
    out_dir = ROOT / "outputs" / "step29_quality_report_aggregation"
    ...
    marker = "[OK] Step 29 quality report aggregation finished"
    write_log(
...
def _collect_reports():
    root_path = ROOT / "outputs" / "step29_64_stability_driver"
    if not root_path.is_dir():
        raise RuntimeError("missing Step 29 stability driver output root")
    rows = []
...
def _assert_row(row):
    if row["strict"] is not True or row["pass"] is not True or row["severity"] != "ok":
        raise RuntimeError(f"Step 29 quality report must pass strict gate: {row}")
    if int(row["warnings_count"]) != 0 or int(row["reasons_count"]) != 0:
        raise RuntimeError(f"Step 29 quality report must have zero warnings and reasons: {row}")
    if int(row["report_size_bytes"]) >= 100_000:
        raise RuntimeError(f"Step 29 quality report is too large: {row}")
    if row["geometry_type"] == "mesh":
        if int(row["vertices_count"]) <= 0 or int(row["faces_count"]) <= 0:
            raise RuntimeError(f"Step 29 mesh quality report missing mesh size: {row}")
        if int(row["boundary_edge_count"]) != 0 or int(row["degenerate_face_count"]) != 0:
            raise RuntimeError(f"Step 29 mesh quality report has topology issues: {row}")
        if int(row["nonmanifold_edge_count"]) != 0:
            raise RuntimeError(f"Step 29 mesh quality report has nonmanifold edges: {row}")
    if row["geometry_type"] == "voxel":
        if int(row["occupied_count"]) <= 0:
            raise RuntimeError(f"Step 29 voxel quality report has no occupancy: {row}")
        if int(row["connected_component_count"]) != 1:
            raise RuntimeError(f"Step 29 voxel quality report must be connected: {row}")
        if float(row["largest_component_fraction"]) != 1.0:
            raise RuntimeError(f"Step 29 voxel quality report must have full largest component: {row}")
...
def _assert_summary(summary):
    if int(summary["quality_report_count"]) != 4:
        raise RuntimeError(f"expected 4 Step 29 quality reports: {summary}")
    if int(summary["pass_count"]) != 4 or int(summary["strict_count"]) != 4:
        raise RuntimeError(f"all Step 29 quality reports must pass strict gates: {summary}")
    if int(summary["error_count"]) != 0 or int(summary["warning_count"]) != 0:
        raise RuntimeError(f"Step 29 quality reports have warnings or errors: {summary}")
    if int(summary["mesh_row_count"]) != 2 or int(summary["voxel_row_count"]) != 2:
        raise RuntimeError(f"Step 29 quality report type split is wrong: {summary}")
    if int(summary["quality_report_total_size_bytes"]) <= 0 or int(summary["quality_report_max_size_bytes"]) >= 100_000:
        raise RuntimeError(f"Step 29 quality report size summary is wrong: {summary}")
