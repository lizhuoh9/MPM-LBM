...
from src.selected_parameter_64_feasibility import (  # noqa: E402
    MPM_SUBSTEPS_PER_LBM_STEP,
    N_GRID,
    N_LBM_STEPS,
    SELECTED_CAP,
    SELECTED_SCALE,
    scale_from_config_path,
    summarize_selected_parameter_driver_rows,
)


STEP41_GEOMETRY_CONFIG_PATH = "configs/step30_squid_proxy_geometry.json"
STEP41_BOUNDARY_MOTION_CONFIG_PATH = "configs/step34_boundary_motion_interface_prescribed_kinematic.json"
STEP41_APPLICATION_CONFIG_PATH = "configs/step41_wall_velocity_application_scale_0050_64.json"
STEP41_SCHEDULE_CONFIG_PATH = "configs/step32_squid_proxy_kinematics_schedule.json"
STEP41_SCHEDULE_ROWS_PATH = "outputs/step32_kinematics_schedule/kinematics_schedule.json"
...
def run_step41_driver_case(driver_config_path, out_dir) -> dict:
    ...
    enforce_step41_driver_config(config, driver_config_path)
    driver = FSIDriver3D(config, str(out_dir))
    diagnostics = driver.run()
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step 41 driver case: {driver_config_path}")
    ...
    row = {
        "case": case,
        "candidate_id": "squid_proxy_selected_parameter_64_feasibility",
        ...
        "notes": "Step 41 controlled jet-cycle proxy selected-parameter 64^3 feasibility",
    }
    ...
    row["has_nan"] = bool(row["has_nan"] or not finite_values(row, excluded=step41_driver_string_fields()))
    row["has_inf"] = row["has_nan"]
    assert_step41_driver_row(row)
    return row


def enforce_step41_driver_config(config, config_path):
    if config.geometry_type != "squid_proxy":
        raise RuntimeError(f"{config_path} must use geometry_type=squid_proxy")
    if config.geometry_config_path != STEP41_GEOMETRY_CONFIG_PATH:
        raise RuntimeError(f"{config_path} must reuse Step 30 squid proxy geometry")
    if int(config.n_grid) != N_GRID or int(config.n_particles) != 4096:
        raise RuntimeError(f"{config_path} must use n_grid=64 and n_particles=4096")
    ...
    if config.wall_velocity_application_mode == "disabled":
        ...
        if config.boundary_motion_config_path != STEP41_BOUNDARY_MOTION_CONFIG_PATH or not config.boundary_motion_report_enabled:
            raise RuntimeError(f"{config_path} experimental rows must report the Step 34 boundary-motion config")
        if config.wall_velocity_application_config_path != STEP41_APPLICATION_CONFIG_PATH or not config.wall_velocity_application_report_enabled:
            raise RuntimeError(f"{config_path} experimental rows must use/report the Step 41 application config")
    else:
        raise RuntimeError(f"unsupported Step 41 wall_velocity_application_mode: {config.wall_velocity_application_mode}")
...
def wall_velocity_scale(config) -> float:
    if config.wall_velocity_application_mode == "disabled":
        return 0.0
    return scale_from_config_path(config.wall_velocity_application_config_path)
...
def application_row_pass(row) -> bool:
    ...
    return bool(
        ...
        and float(row["wall_velocity_scale"]) == SELECTED_SCALE
        and float(row["wall_velocity_cap_lbm"]) == SELECTED_CAP
...
def assert_step41_driver_row(row):
    if not as_bool(row["quality_check_enabled"]) or not as_bool(row["quality_check_strict"]):
        raise RuntimeError(f"Step 41 row must use strict quality checks: {row}")
    if not as_bool(row["quality_pass"]) or not as_bool(row["quality_gate_strict"]):
        raise RuntimeError(f"Step 41 quality gate failed: {row}")
    if row["quality_severity"] != "ok" or int(row["quality_warnings_count"]) != 0 or int(row["quality_reasons_count"]) != 0:
        raise RuntimeError(f"Step 41 quality report must be clean: {row}")
    if int(row["completed_lbm_steps"]) < N_LBM_STEPS or int(row["total_mpm_substeps"]) < N_LBM_STEPS * MPM_SUBSTEPS_PER_LBM_STEP:
        raise RuntimeError(f"Step 41 row did not complete one cycle: {row}")
    if float(row["rho_min_global"]) <= 0.95 or float(row["rho_max_global"]) >= 1.05:
        raise RuntimeError(f"Step 41 density envelope out of range: {row}")
    if float(row["lbm_max_v_global"]) >= 0.1:
        raise RuntimeError(f"Step 41 velocity envelope out of range: {row}")
    if float(row["mpm_min_J_global"]) <= 0.0 or float(row["mpm_max_speed_global"]) >= 10.0:
        raise RuntimeError(f"Step 41 MPM envelope out of range: {row}")
    if float(row["projected_mass_min"]) <= 0.0 or float(row["projected_mass_max"]) <= 0.0:
        raise RuntimeError(f"Step 41 projected mass envelope is invalid: {row}")
    if int(row["active_cell_count"]) <= 0 or int(row["bb_link_count_max"]) <= 0:
        raise RuntimeError(f"Step 41 moving-boundary diagnostics are missing: {row}")
    if as_bool(row["has_nan"]) or as_bool(row["has_inf"]) or not finite_values(row, excluded=step41_driver_string_fields()):
        raise RuntimeError(f"Step 41 row has non-finite values: {row}")
    if not application_row_pass(row):
        raise RuntimeError(f"Step 41 application fields failed: {row}")
    if not as_bool(row["stable"]):
        raise RuntimeError(f"Step 41 row is not stable: {row}")


def read_driver_rows():
    return read_json("outputs/step41_64_selected_parameter_driver/selected_parameter_64_results.json")["rows"]
...
def read_diagnostics_rows(driver_row: dict):
    return read_csv_rows(f"outputs/step41_64_selected_parameter_driver/{driver_row['case']}/diagnostics_timeseries.csv")
...
def step41_driver_string_fields():
    return {
        "case",
        "candidate_id",
        "mode_class",
...
def driver_summary(rows: list[dict]) -> dict:
    return summarize_selected_parameter_driver_rows(rows)
