...
def main():
    ...
    if not summary["output_guard_pass"]:
        raise RuntimeError(f"Step 61 output guard failed: {summary}")
    out_dir = ROOT / "outputs" / "step61_output_guard"
    ...
    marker = "[OK] Step 61 output guard finished"
    write_log("logs/step61_output_guard.log", [marker, f"row_count={summary['row_count']}"])
    print(marker)
