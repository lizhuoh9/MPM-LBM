...
def main():
    ...
    if not summary["legacy_shim_audit_pass"]:
        raise RuntimeError(f"Step 56 legacy shim audit failed: {summary}")
    out_dir = ROOT / "outputs" / "step56_legacy_shim_audit"
    ...
    marker = "[OK] Step 56 legacy shim audit finished"
    write_log("logs/step56_legacy_shim_audit.log", [marker, f"row_count={summary['row_count']}"])
    print(marker)
