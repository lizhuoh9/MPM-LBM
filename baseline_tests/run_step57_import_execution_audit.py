...
def main():
    ...
    if not summary["driver_support_import_execution_audit_pass"]:
        raise RuntimeError(f"Step 57 import execution audit failed: {summary}")
    out_dir = ROOT / "outputs" / "step57_import_execution_audit"
    ...
    marker = "[OK] Step 57 import execution audit finished"
    write_log("logs/step57_import_execution_audit.log", [marker, f"row_count={summary['row_count']}"])
    print(marker)
