...
from src.mpm_lbm.evidence.step106_output_guard import build_step106_output_guard


def main():
    os.chdir(ROOT)
    rows, summary = build_step106_output_guard(ROOT)
    if not summary["output_guard_pass"]:
        raise RuntimeError(f"Step106 output guard failed: {summary}")
    out_dir = ROOT / "outputs" / "step106_output_guard"
    ...
    marker = "[OK] Step106 output guard finished"
    write_log("logs/step106_output_guard.log", [marker, f"row_count={summary['row_count']}"])
    print(marker)
