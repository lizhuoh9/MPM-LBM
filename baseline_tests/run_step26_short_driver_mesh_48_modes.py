...
def main():
    ...
    out_dir = ROOT / "outputs" / "step26_short_driver_mesh_48_modes"
    rows = []
    for config_path in STEP26_DRIVER_CONFIGS_BY_KIND["mesh"]:
    ...
    write_rows_csv_npz(rows, out_dir / "mesh_48_short_driver_results.csv", out_dir / "mesh_48_short_driver_results.npz", SHORT_DRIVER_FIELDS)
    write_json(out_dir / "mesh_48_short_driver_results.json", {"row_count": len(rows), "summary": summary, "rows": rows})
    marker = "[OK] Step 26 mesh 48 short driver modes finished"
    write_log(
...
def _assert_summary(summary):
    if int(summary["row_count"]) != 4 or int(summary["mode_count"]) != 4:
        raise RuntimeError(f"expected 4 mesh short driver rows: {summary}")
    if int(summary["stable_count"]) != 4 or int(summary["quality_pass_count"]) != 4 or int(summary["strict_count"]) != 4:
        raise RuntimeError(f"mesh short driver rows must all be stable and strict quality-passing: {summary}")
