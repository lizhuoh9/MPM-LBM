...
def main():
    ...
    rows = []
    for descriptor_path in STEP28_DESCRIPTORS:
        ...
        geometry_config_path = STEP28_GEOMETRY_CONFIGS[current["candidate_id"]]
    ...
    if len(rows) != 2 or not all(row["guard_pass"] for row in rows):
        raise RuntimeError(f"Step 28 fingerprint guard failed: {rows}")

    out_dir = ROOT / "outputs" / "step28_candidate_fingerprint_guard"
    ...
    marker = "[OK] Step 28 candidate fingerprint guard finished"
    write_log(
