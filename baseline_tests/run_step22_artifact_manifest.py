...
def _check_driver_configs_disable_heavy_outputs():
    for relative_path in STEP22_DRIVER_CONFIGS:
...
def _check_fixture_sizes():
    ...
    for name in [
        ...
        if os.path.getsize(path) >= 200_000:
            raise RuntimeError(f"fixture is too large for Step 22: {path}")
...
def main():
    ...
    out_dir = os.path.join(ROOT, "outputs", "step22_artifact_manifest")
    ...
    if summary["large_file_count"] != 0:
        raise RuntimeError("Step 22 artifact manifest found large files")
    ...
    print("Step 22 artifact manifest")
    ...
    print("[OK] Step 22 artifact manifest finished")
