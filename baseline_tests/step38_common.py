...
STEP38_APPLICATION_CONFIG_PATH = "configs/step36_wall_velocity_application_solid_vel_experimental.json"
STEP38_GEOMETRY_CONFIG_PATH = "configs/step30_squid_proxy_geometry.json"
STEP38_REGION_CONFIG_PATH = "configs/step30_squid_proxy_region_config.json"
STEP38_BOUNDARY_MOTION_CONFIG_PATH = "configs/step34_boundary_motion_interface_prescribed_kinematic.json"
STEP38_SCHEDULE_CONFIG_PATH = "configs/step32_squid_proxy_kinematics_schedule.json"
STEP38_SCHEDULE_OUTPUT_PATH = "outputs/step32_kinematics_schedule/kinematics_schedule.json"
...
def run_step38_driver_case(driver_config_path, out_dir) -> dict:
    ...
    enforce_step38_driver_config(config, driver_config_path)
    driver = FSIDriver3D(config, str(out_dir))
    diagnostics = driver.run()
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step 38 driver case: {driver_config_path}")
    ...
    row = {
        "case": case,
        "candidate_id": "squid_proxy_tethered_jet_cycle_diagnostics",
        ...
        "notes": "Step 38 controlled tethered jet-cycle proxy diagnostics",
    }
    ...
    row["has_nan"] = bool(row["has_nan"] or not finite_values(row, excluded=step38_driver_string_fields()))
    row["has_inf"] = row["has_nan"]
    assert_step38_driver_row(row)
    return row


def enforce_step38_driver_config(config, config_path):
    if config.geometry_type != "squid_proxy":
        raise RuntimeError(f"{config_path} must use geometry_type=squid_proxy")
    if config.geometry_config_path != STEP38_GEOMETRY_CONFIG_PATH:
    ...
    if int(config.n_lbm_steps) != CYCLE_PERIOD_STEPS or int(config.mpm_substeps_per_lbm_step) != 5:
    ...
    if tuple(float(value) for value in config.target_u_lbm) != (0.0, 0.0, 0.0):
        raise RuntimeError(f"{config_path} must keep target_u_lbm zero for tethered cycle diagnostics")
    ...
    if config.wall_velocity_application_mode == "disabled":
        ...
        if config.boundary_motion_config_path != STEP38_BOUNDARY_MOTION_CONFIG_PATH or not config.boundary_motion_report_enabled:
            raise RuntimeError(f"{config_path} experimental rows must report the Step 34 boundary-motion config")
        if config.wall_velocity_application_config_path != STEP38_APPLICATION_CONFIG_PATH or not config.wall_velocity_application_report_enabled:
            raise RuntimeError(f"{config_path} experimental rows must use/report the Step 36 application config")
    else:
        raise RuntimeError(f"unsupported Step 38 wall_velocity_application_mode: {config.wall_velocity_application_mode}")
...
def application_row_pass(row) -> bool:
    ...
    return bool(
        int(row["application_report_count"]) >= CYCLE_PERIOD_STEPS
...
def assert_step38_driver_row(row):
    if not as_bool(row["quality_check_enabled"]) or not as_bool(row["quality_check_strict"]):
        raise RuntimeError(f"Step 38 row must use strict quality checks: {row}")
    if not as_bool(row["quality_pass"]) or not as_bool(row["quality_gate_strict"]):
        raise RuntimeError(f"Step 38 quality gate failed: {row}")
    if row["quality_severity"] != "ok" or int(row["quality_warnings_count"]) != 0 or int(row["quality_reasons_count"]) != 0:
        raise RuntimeError(f"Step 38 quality report must be clean: {row}")
    if int(row["completed_lbm_steps"]) < CYCLE_PERIOD_STEPS or int(row["total_mpm_substeps"]) < CYCLE_PERIOD_STEPS * 5:
        raise RuntimeError(f"Step 38 row did not complete one 40-step cycle: {row}")
    if float(row["rho_min_global"]) <= 0.95 or float(row["rho_max_global"]) >= 1.05:
        raise RuntimeError(f"Step 38 density envelope out of range: {row}")
    if float(row["lbm_max_v_global"]) >= 0.1:
        raise RuntimeError(f"Step 38 velocity envelope out of range: {row}")
    if float(row["mpm_min_J_global"]) <= 0.0 or float(row["mpm_max_speed_global"]) >= 10.0:
        raise RuntimeError(f"Step 38 MPM envelope out of range: {row}")
    if float(row["projected_mass_min"]) <= 0.0 or float(row["projected_mass_max"]) <= 0.0:
        raise RuntimeError(f"Step 38 projected mass envelope is invalid: {row}")
    if int(row["active_cell_count"]) <= 0 or int(row["bb_link_count_max"]) <= 0:
        raise RuntimeError(f"Step 38 moving-boundary diagnostics are missing: {row}")
    if as_bool(row["has_nan"]) or as_bool(row["has_inf"]) or not finite_values(row, excluded=step38_driver_string_fields()):
        raise RuntimeError(f"Step 38 row has non-finite values: {row}")
    if not application_row_pass(row):
        raise RuntimeError(f"Step 38 application fields failed: {row}")
    if not as_bool(row["stable"]):
        raise RuntimeError(f"Step 38 row is not stable: {row}")
...
def driver_summary(rows) -> dict:
    return {
        ...
        "scope_note": "Step 38 controlled tethered jet-cycle proxy diagnostics; not propulsion or swimming validation.",
    }


def assert_driver_summary(summary):
    if int(summary["row_count"]) != 4:
        raise RuntimeError(f"Step 38 driver row count is wrong: {summary}")
    if int(summary["static_row_count"]) != 2 or int(summary["experimental_row_count"]) != 2:
        raise RuntimeError(f"Step 38 mode class split is wrong: {summary}")
    if int(summary["engineering_row_count"]) != 2 or int(summary["link_area_row_count"]) != 2:
        raise RuntimeError(f"Step 38 transfer split is wrong: {summary}")
    if int(summary["stable_count"]) != 4 or int(summary["quality_pass_count"]) != 4:
        raise RuntimeError(f"all Step 38 driver rows must be stable and quality-passing: {summary}")
    if int(summary["min_completed_lbm_steps"]) < CYCLE_PERIOD_STEPS or int(summary["min_total_mpm_substeps"]) < CYCLE_PERIOD_STEPS * 5:
        raise RuntimeError(f"Step 38 completion summary is wrong: {summary}")
    if float(summary["min_rho_min_global"]) <= 0.95 or float(summary["max_rho_max_global"]) >= 1.05:
        raise RuntimeError(f"Step 38 density summary is out of range: {summary}")
    if float(summary["max_lbm_max_v_global"]) >= 0.1:
        raise RuntimeError(f"Step 38 velocity summary is out of range: {summary}")
    if float(summary["min_projected_mass_min"]) <= 0.0 or int(summary["min_active_cell_count"]) <= 0:
        raise RuntimeError(f"Step 38 projected/active summary is invalid: {summary}")
    if int(summary["min_bb_link_count_max"]) <= 0:
        raise RuntimeError(f"Step 38 bounce-back link summary is invalid: {summary}")
    if int(summary["max_lbm_population_update_count"]) != 0:
        raise RuntimeError(f"Step 38 must not directly update LBM populations: {summary}")


def compare_static_experimental_cycle(static_row: dict, experimental_row: dict) -> dict:
    row = {
        ...
        "notes": "bounded Step 38 static-vs-experimental one-cycle proxy comparison",
    }
...
def compare_engineering_link_area_cycle(engineering_row: dict, link_area_row: dict) -> dict:
    row = {
        "comparison": "experimental_engineering_vs_link_area_cycle",
        ...
        "notes": "bounded Step 38 experimental engineering-vs-link-area one-cycle proxy comparison",
    }
    row["comparison_pass"] = bool(
        row["both_stable"]
...
def read_driver_rows():
    return read_json("outputs/step38_cycle_driver/cycle_driver_results.json")["rows"]
...
def read_diagnostics_rows(driver_row: dict):
    return read_csv_rows(f"outputs/step38_cycle_driver/{driver_row['case']}/diagnostics_timeseries.csv")


def case_name(config_path):
    return Path(config_path).stem.removeprefix("step38_cycle_")


def step38_driver_string_fields():
    return {
        "case",
        "candidate_id",
        "mode_class",
