...
from src.mpm_lbm.evidence.step108_output_guard import build_step108_output_guard


def main():
    os.chdir(ROOT)
    rows, summary = build_step108_output_guard(ROOT)
    if not summary["output_guard_pass"]:
        raise RuntimeError(f"Step108 output guard failed: {summary}")
    out_dir = ROOT / "outputs" / "step108_output_guard"
    ...
    marker = "[OK] Step108 output guard finished"
    write_log("logs/step108_output_guard.log", [marker, f"row_count={summary['row_count']}"])
    print(marker)
