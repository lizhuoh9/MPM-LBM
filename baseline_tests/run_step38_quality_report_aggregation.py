...
def main():
    ...
    out_dir = ROOT / "outputs" / "step38_quality_report_aggregation"
    ...
    marker = "[OK] Step 38 quality report aggregation finished"
    write_log("logs/step38_quality_report_aggregation.log", [marker, f"quality_report_count={summary['quality_report_count']}"])
    print(f"quality_report_count={summary['quality_report_count']}")
    print(marker)


def collect_reports():
    root_path = ROOT / "outputs" / "step38_cycle_driver"
    if not root_path.is_dir():
        raise RuntimeError(f"missing Step 38 driver output root: {root_path}")
    rows = []
...
def row_from_report(report_path):
    ...
    if row["geometry_type"] != "squid_proxy" or row["quality_kind"] != "procedural_voxelized":
        raise RuntimeError(f"Step 38 quality report must be procedural squid proxy quality: {row}")
    return row
...
def assert_summary(summary):
    if int(summary["quality_report_count"]) != 4:
        raise RuntimeError(f"expected 4 Step 38 quality reports: {summary}")
    if int(summary["pass_count"]) != 4 or int(summary["strict_count"]) != 4:
        raise RuntimeError(f"all Step 38 quality reports must pass strict gates: {summary}")
    if int(summary["warning_count"]) != 0 or int(summary["error_count"]) != 0:
        raise RuntimeError(f"Step 38 quality reports contain warnings/errors: {summary}")
    if int(summary["static_report_count"]) != 2 or int(summary["experimental_report_count"]) != 2:
        raise RuntimeError(f"Step 38 quality report mode split is wrong: {summary}")
