...
from step32_common import ROOT, summary_rows, write_csv_rows, write_json, write_log
...
def main():
    ...
    rows = [
        row
        for row in rows
        if not row["path"].startswith("outputs/step32_artifact_manifest/")
        and "__pycache__/" not in row["path"]
        and row["extension"] != ".pyc"
    ]
    summary = summarize_artifacts(rows)
    summary.update(step32_summary(rows))
    assert_artifact_policy(summary)

    out_dir = ROOT / "outputs" / "step32_artifact_manifest"
    ...
    marker = "[OK] Step 32 artifact manifest finished"
    write_log(
        ...
        [
            ...
            f"step32_total_size_mb={summary['step32_total_size_mb']:.6f}",
        ],
    )
...
def step32_summary(rows):
    step32_rows = [
        row
        for row in rows
        if row["path"].startswith("outputs/step32")
        or row["path"].startswith("logs/step32")
        or row["path"].startswith("configs/step32")
        or row["path"].startswith("STEP32_")
        or row["path"].startswith("docs/32_")
        or row["path"].startswith("baseline_tests/run_step32")
        or row["path"].startswith("baseline_tests/step32")
        or row["path"].startswith("tests/test_step32")
        or row["path"].startswith("src/squid_kinematics_")
    ]
    step32_size = sum(int(row["size_bytes"]) for row in step32_rows)
    candidate_raw_rows = [
        row
        for row in rows
    ...
    return {
        "step32_file_count": len(step32_rows),
        "step32_total_size_bytes": int(step32_size),
        "step32_total_size_mb": step32_size / 1024.0 / 1024.0,
        "step32_vtr_count": sum(1 for row in step32_rows if row["path"].endswith(".vtr")),
        "step32_particle_npy_count": sum(1 for row in step32_rows if row["path"].endswith(".npy") and "particle" in row["path"].lower()),
...
def assert_artifact_policy(summary):
    ...
    if float(summary["step32_total_size_mb"]) >= 3.0:
        raise RuntimeError(f"Step 32 output size exceeds 3 MB: {summary['step32_total_size_mb']:.6f}")
    if float(summary["total_size_mb"]) >= 190.0:
        raise RuntimeError(f"repo artifact summary exceeds 190 MB: {summary['total_size_mb']:.6f}")
    ...
    if int(summary["step32_vtr_count"]) != 0:
        raise RuntimeError("Step 32 wrote forbidden .vtr outputs")
    if int(summary["step32_particle_npy_count"]) != 0:
        raise RuntimeError("Step 32 wrote forbidden particle .npy outputs")
