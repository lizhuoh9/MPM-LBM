...
from step40_common import ROOT, STEP40_DRIVER_CONFIGS, fieldnames_from_rows, read_json, write_csv_rows, write_json, write_log
from src.tethered_cycle_diagnostics import guard_rows, summarize_no_free_body_state


def main():
    os.chdir(ROOT)
    configs = [read_json(path) for path in STEP40_DRIVER_CONFIGS]
    summary = summarize_no_free_body_state(configs, ROOT / "outputs" / "step40_parameter_sweep_driver")
    summary["guard"] = "Step 40 parameter sensitivity tethered no-free-body guard"
    summary["guard_pass"] = bool(
        int(summary["config_count"]) == 8
    ...
    if not summary["guard_pass"]:
        raise RuntimeError(f"Step 40 tethered no-free-body guard failed: {summary}")

    out_dir = ROOT / "outputs" / "step40_tethered_no_free_body_guard"
    ...
    marker = "[OK] Step 40 tethered no-free-body guard finished"
    write_log("logs/step40_tethered_no_free_body_guard.log", [marker, f"guard_pass={summary['guard_pass']}"])
