...
def test_step82_wall_velocity_solid_vel_smoke_matrix_artifact_passes():
    payload = read_json("outputs/step82_wall_velocity_solid_vel_smoke_matrix/wall_velocity_solid_vel_smoke_matrix.json")
    summary = payload["summary"]
    rows = payload["rows"]
    assert len(rows) == 1
    row = rows[0]
    assert summary["step82_wall_velocity_solid_vel_smoke_matrix_pass"] is True
    ...
    assert summary["physics_feature_expansion"] == "wall_velocity_solid_vel_only"
    assert row["row_name"] == "canonical_driver_wall_velocity_solid_vel_32_3step_smoke"
    ...
    assert row["wall_velocity_application_mode"] == "solid_vel_experimental"
    assert row["target_lbm_field"] == "solid_vel"
    assert row["apply_to_lbm_solid_vel"] is True
    assert row["apply_to_lbm_populations"] is False
    assert row["apply_to_mpm"] is False
    assert row["apply_to_projector"] is False
    assert row["modify_bounceback_formula"] is False
    assert row["jet_model_enabled"] is False
    assert row["actuation_claim_enabled"] is False
    assert row["stable"] is True
    assert row["has_nan"] is False
    assert row["has_inf"] is False


def test_step82_wall_velocity_application_report_contract_passes():
    payload = read_json(
        "outputs/step82_driver_runs/canonical_driver_wall_velocity_solid_vel_32_3step_smoke/wall_velocity_application_report.json"
    )
    summary = payload["summary"]
...
def test_step82_boundary_motion_report_contract_passes():
    payload = read_json(
        "outputs/step82_driver_runs/canonical_driver_wall_velocity_solid_vel_32_3step_smoke/boundary_motion_interface_report.json"
    )
    summary = payload["summary"]
