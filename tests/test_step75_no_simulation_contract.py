...
def test_build_passes():
    rows, summary = build_step75_no_simulation_audit(ROOT)
    assert rows
    ...
    assert summary["protected_step75_file_count"] == 0


def test_artifact_passes():
    payload = read_json("outputs/step75_no_simulation_audit/no_simulation.json")
    assert payload["rows"]
    assert payload["summary"]["no_simulation_audit_pass"] is True
    assert payload["summary"]["forbidden_python_call_count"] == 0
