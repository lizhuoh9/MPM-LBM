...
def test_step95_output_guard_passes():
    payload = read_json("outputs/step95_output_guard/output_guard.json")
    ...
    assert summary["step95_raw_geometry_output_count"] == 0
    assert summary["step95_real_geometry_candidate_output_count"] == 0
    assert summary["step95_dense_wall_velocity_output_count"] == 0
    assert summary["step95_sparse_wall_velocity_output_count"] == 0
    assert summary["step95_dense_displacement_output_count"] == 0
    assert summary["step95_displaced_particle_output_count"] == 0
    ...
    assert summary["step95_large_file_count"] == 0
