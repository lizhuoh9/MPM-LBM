...
def test_step108_output_guard_blocks_validation_claims_and_official_payloads():
    payload = read_json("outputs/step108_output_guard/output_guard_report.json")
    ...
    assert summary["step108_forbidden_file_count"] == 0
    assert summary["artifact_budget_pass"] is True


def test_step108_artifact_manifest_is_small_and_step_scoped():
    payload = read_json("outputs/step108_artifact_manifest/artifact_manifest.json")
    summary = payload["summary"]

    assert summary["artifact_manifest_pass"] is True
    assert summary["step108_file_count"] > 0
    assert summary["step108_total_size_mb"] < 20.0
    assert summary["step108_ansys_proprietary_file_count"] == 0
    assert summary["step108_official_image_count"] == 0
    assert summary["step108_driver_run_dir_count"] == 1
