...
STEP137_ROLE = "interior_reflection_diagnostic_48"


def _step137_row(name, semantics, *, solver_state_hash="step137-hash", **overrides):
    row = {
        "name": name,
        "row_role": STEP137_ROLE,
        ...
        "open_boundary_inlet_ramp_steps": 90,
...
def _step137_spec(**overrides):
    ...
    base = {
        "name": (
            ...
            "_ramp90_target0p85_out5_250step_ramp_refined"
        ),
        "nx": 48,
        "ny": 48,
        "nz": 48,
...
def test_step137_ramp_refined_phase_is_distinct_bounded_diagnostic_surface():
    from experiments.steps.step121_lbm_boundary_real_campaign_and_gate_correction import (
        ...
        step121_plane_flux_ramp_refined_tiny_smoke_specs,
        step121_plane_flux_ramp_tuned_48_specs,
    )

    specs = step121_plane_flux_ramp_refined_48_specs(output_interval=5)
    step136_names = {spec.name for spec in step121_plane_flux_ramp_tuned_48_specs(output_interval=5)}

    assert resolve_step121_phase_specs("planeflux_ramp_refined48", output_interval=5) == specs
    assert len(specs) == 6
    assert all(spec.row_role == STEP137_ROLE for spec in specs)
    ...
    assert all("Step137" in spec.artifact_scope_note for spec in specs)
    assert all("selected96" in spec.artifact_scope_note and "500-step" in spec.artifact_scope_note for spec in specs)
    assert all(spec.name not in step136_names for spec in specs)
    assert all("target" in spec.name and "250step_ramp_refined" in spec.name for spec in specs)
    assert {
        (int(spec.open_boundary_inlet_ramp_steps), round(float(spec.open_boundary_flux_control_target_scale), 2))
        for spec in specs
    } == {(90, 0.90), (90, 0.85), (90, 0.80), (85, 0.90), (85, 0.85), (100, 0.85)}
    assert {round(float(spec.open_boundary_flux_feedback_gain_u), 2) for spec in specs} == {0.50}
    assert {round(float(spec.open_boundary_flux_correction_cap_u), 4) for spec in specs} == {0.005}
    assert {round(float(spec.niu), 2) for spec in specs} == {0.10}

    tiny = step121_plane_flux_ramp_refined_tiny_smoke_specs()
    assert len(tiny) == 1
    assert tiny[0].name == "tiny_step137_ramp_target_refinement_smoke"
    assert (tiny[0].nx, tiny[0].ny, tiny[0].nz, tiny[0].n_steps) == (8, 6, 6, 20)
    assert int(tiny[0].open_boundary_inlet_ramp_steps) in {10, 20}
...
def test_step137_manifest_identity_and_stale_step136_rows_are_rejected(tmp_path):
    ...
    base = _step137_spec(open_boundary_flux_control_target_scale=0.90)
    scaled = _step137_spec(open_boundary_flux_control_target_scale=0.85)
    ramped = _step137_spec(open_boundary_inlet_ramp_steps=85, open_boundary_flux_control_target_scale=0.90)
    assert solver_state_hash_for_spec(base) != solver_state_hash_for_spec(scaled)
    assert solver_state_hash_for_spec(base) != solver_state_hash_for_spec(ramped)
    ...
    specs = step121_plane_flux_ramp_refined_48_specs(output_interval=5)
    manifest = write_step121_campaign_manifest(out, specs, phase="planeflux_ramp_refined48")
    spec = next(
        item
        for item in specs
        if int(item.open_boundary_inlet_ramp_steps) == 90
        and math_isclose(item.open_boundary_flux_control_target_scale, 0.85)
    )
    expected = manifest["expected_rows"][spec.name]

    assert expected["open_boundary_inlet_ramp_steps"] == 90
    assert expected["open_boundary_flux_feedback_gain_u"] == pytest.approx(0.50)
    ...
    step136_hashes = {solver_state_hash_for_spec(item) for item in step121_plane_flux_ramp_tuned_48_specs(output_interval=5)}
    step137_hashes = {solver_state_hash_for_spec(item) for item in specs}
    assert len(step137_hashes) == 6
    assert step137_hashes.isdisjoint(step136_hashes)

    stale_source = next(
        item
        for item in step121_plane_flux_ramp_tuned_48_specs(output_interval=5)
        if int(item.open_boundary_inlet_ramp_steps) == 100
        and math_isclose(item.open_boundary_flux_control_target_scale, 0.90)
    )
    stale = _step137_row(
        spec.name,
        spec.open_boundary_semantics,
        solver_state_hash=solver_state_hash_for_spec(stale_source),
        open_boundary_flux_control_target_scale=0.90,
...
def test_step137_flow_diagnostics_record_step_number_and_bounded_profile(tmp_path):
    from experiments.steps.step120_lbm_boundary_repair_large_real_execution import (
    ...
    spec = _step137_spec(open_boundary_flux_control_target_scale=0.85)
    records = []
    for step, profile in [
    ...
    assert "step137_ramp_target_refinement_candidate" in FLOW_DEVELOPMENT_DIAGNOSTIC_FIELDS
    assert records[-1]["target_outlet_flux"] == pytest.approx(85.0)
    assert records[-1]["controller_target_scale"] == pytest.approx(0.85)
    assert records[-1]["step137_ramp_target_refinement_candidate"] is True
    assert records[-1]["step136_ramped_throughput_calibration_candidate"] is False
    ...
    assert rows[-1]["controller_target_scale"] == "0.85"
    assert rows[-1]["step137_ramp_target_refinement_candidate"] == "True"
    ...
    assert summary["step"] == 137
    assert summary["final"]["step137_ramp_target_refinement_candidate"] is True
