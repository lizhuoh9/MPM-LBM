...
def test_step60_duration_ramp_quality_passes_current_artifacts():
...
def test_step60_duration_ramp_quality_artifact_passes():
    payload = read_json("outputs/step60_duration_ramp_quality/duration_ramp_quality.json")
...
def test_step60_runtime_summary_is_recorded_as_soft_warning_only():
    ...
    assert summary["slowest_row_name"] in {row["row_name"] for row in payload["rows"]}
