...
def test_step57_legacy_shim_audit_passes_current_source():
    rows, summary = build_driver_support_legacy_shim_audit(ROOT)
    assert summary["driver_support_legacy_shim_audit_pass"] is True
...
def test_step57_legacy_shim_artifact_passes():
    payload = read_json("outputs/step57_legacy_shim_audit/legacy_shim_audit.json")
...
def test_step57_all_migrated_legacy_files_are_thin_canonical_shims():
