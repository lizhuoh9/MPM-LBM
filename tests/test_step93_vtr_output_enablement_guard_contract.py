...
def test_step93_vtr_output_enablement_guard_passes():
    payload = read_json("outputs/step93_vtr_output_enablement_guard/vtr_output_enablement_guard.json")
    summary = payload["summary"]

    assert summary["step93_vtr_output_enablement_guard_pass"] is True
    ...
    assert summary["duration_reduction_for_output_isolation"] is True
    assert summary["vtr_output_smoke_isolation"] is True
    assert summary["only_new_feature_from_step92"] == "write_vtk"
    assert summary["vtr_output_planned_for_step94"] is True
    assert summary["write_vtk_allowed_for_step94"] is True
