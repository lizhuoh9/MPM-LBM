...
def test_step83_step82_regression_guard_passes():
    rows, summary = build_step83_step82_regression_guard(ROOT)
    assert rows
    ...
    assert summary["step82_real_geometry_enabled_count"] == 0
    assert summary["step82_squid_proxy_enabled_count"] == 0
    assert summary["step82_link_area_enabled_count"] == 0
    assert summary["step82_vtr_count"] == 0
    assert summary["step82_particle_npy_count"] == 0


def test_step83_step82_regression_guard_artifact_passes():
    payload = read_json("outputs/step83_step82_regression_guard/step82_regression_guard.json")
