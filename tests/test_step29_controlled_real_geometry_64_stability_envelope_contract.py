...
STEP29_LOG_MARKERS = {
    "logs/step29_candidate_fingerprint_guard.log": "[OK] Step 29 candidate fingerprint guard finished",
    "logs/step29_64_stability_driver.log": "[OK] Step 29 64 stability driver finished",
    "logs/step29_stability_envelope_summary.log": "[OK] Step 29 stability envelope summary finished",
    "logs/step29_engineering_vs_link_area_envelope.log": "[OK] Step 29 engineering vs link-area envelope finished",
    "logs/step29_force_reaction_envelope.log": "[OK] Step 29 force reaction envelope finished",
    "logs/step29_area_scale_envelope.log": "[OK] Step 29 area-scale envelope finished",
    "logs/step29_step28_prefix_regression.log": "[OK] Step 29 Step 28 prefix regression finished",
    "logs/step29_quality_report_aggregation.log": "[OK] Step 29 quality report aggregation finished",
    "logs/step29_step28_regression_guard.log": "[OK] Step 29 Step 28 regression guard finished",
    "logs/step29_artifact_manifest.log": "[OK] Step 29 artifact manifest finished",
}
...
def test_step29_required_artifacts_exist():
    missing = [path for path in STEP29_REQUIRED_FILES + STEP29_OUTPUT_FILES if not (ROOT / path).is_file()]
    assert missing == []


def test_step29_driver_configs_are_valid():
    geometry_configs = [
    ...
    for path in STEP29_DRIVER_CONFIGS:
        ...
        assert int(payload["n_lbm_steps"]) == 20
...
def test_step29_candidate_fingerprint_guard_is_valid():
    summary = read_json("outputs/step29_candidate_fingerprint_guard/fingerprint_guard.json")
    rows = read_csv_rows("outputs/step29_candidate_fingerprint_guard/fingerprint_guard.csv")
...
def test_step29_stability_driver_outputs_are_valid():
    payload = read_json("outputs/step29_64_stability_driver/stability_driver_results.json")
    summary = payload["summary"]
    rows = read_csv_rows("outputs/step29_64_stability_driver/stability_driver_results.csv")
    ...
    assert len(rows) == 4
    for row in rows:
        assert_stability_driver_row(row)
        diagnostics_path = ROOT / Path(row["driver_timing_path"]).parent / "diagnostics_timeseries.csv"
        assert diagnostics_path.is_file()


def test_step29_stability_envelope_summary_is_valid():
    payload = read_json("outputs/step29_stability_envelope_summary/stability_envelope.json")
    rows = read_csv_rows("outputs/step29_stability_envelope_summary/stability_envelope.csv")
    assert int(payload["summary"]["row_count"]) == 4
    assert int(payload["summary"]["pass_count"]) == 4
    assert len(rows) == 4
    for row in rows:
        assert int(float(row["diagnostic_row_count"])) >= 21
        assert int(float(row["step_min"])) == 0
        assert int(float(row["step_max"])) == 20
        assert float(row["rho_min_observed"]) > 0.95
        assert float(row["rho_max_observed"]) < 1.05
        assert float(row["lbm_max_v_observed"]) < 0.1
        assert float(row["mpm_min_J_observed"]) > 0.0
        assert float(row["mpm_max_speed_observed"]) < 10.0
        assert float(row["projected_mass_min"]) > 0.0
        assert int(float(row["active_cell_count_min"])) > 0
        assert as_bool(row["stable_envelope_pass"])


def test_step29_engineering_vs_link_area_envelope_is_valid():
    payload = read_json("outputs/step29_engineering_vs_link_area_envelope/engineering_vs_link_area_envelope.json")
    rows = read_csv_rows("outputs/step29_engineering_vs_link_area_envelope/engineering_vs_link_area_envelope.csv")
    ...
    for row in rows:
        ...
        assert abs(int(float(row["bb_link_count_delta"]))) <= 512
        assert 0.25 <= float(row["link_area_area_scale_final"]) <= 2.0


def test_step29_force_reaction_envelope_is_valid():
    payload = read_json("outputs/step29_force_reaction_envelope/force_reaction_envelope.json")
    rows = read_csv_rows("outputs/step29_force_reaction_envelope/force_reaction_envelope.csv")
    assert int(payload["summary"]["row_count"]) == 4
    assert int(payload["summary"]["pass_count"]) == 4
    assert len(rows) == 4
    for row in rows:
        assert int(float(row["post_step_row_count"])) >= 20
        assert as_bool(row["finite_pass"])
        assert as_bool(row["positive_post_step_pass"])
        assert float(row["hydro_force_max"]) > 0.0
        assert float(row["max_grid_reaction_norm_max"]) > 0.0
        assert int(float(row["active_reaction_particle_count_max"])) > 0
        assert int(float(row["bb_link_count_min"])) > 0


def test_step29_area_scale_envelope_is_valid():
    payload = read_json("outputs/step29_area_scale_envelope/area_scale_envelope.json")
    rows = read_csv_rows("outputs/step29_area_scale_envelope/area_scale_envelope.csv")
    assert int(payload["summary"]["row_count"]) == 2
    assert int(payload["summary"]["pass_count"]) == 2
    assert len(rows) == 2
    for row in rows:
...
def test_step29_step28_prefix_regression_is_valid():
    payload = read_json("outputs/step29_step28_prefix_regression/step28_prefix_regression.json")
    rows = read_csv_rows("outputs/step29_step28_prefix_regression/step28_prefix_regression.csv")
    assert int(payload["summary"]["row_count"]) == 4
    assert int(payload["summary"]["pass_count"]) == 4
    assert len(rows) == 4
    for row in rows:
...
def test_step29_quality_report_aggregation_is_valid():
    payload = read_json("outputs/step29_quality_report_aggregation/quality_report_summary.json")
    summary = payload["summary"]
    assert int(summary["quality_report_count"]) == 4
...
def test_step29_step28_regression_guard_is_valid():
    summary = read_json("outputs/step29_step28_regression_guard/step28_regression_guard.json")
    rows = read_csv_rows("outputs/step29_step28_regression_guard/step28_regression_guard.csv")
    assert int(summary["row_count"]) == 8
    assert int(summary["pass_count"]) == 8
    assert int(summary["step28_driver_row_count"]) == 4
    assert int(summary["step28_stable_count"]) == 4
    assert int(summary["step28_quality_report_count"]) == 4
    assert int(summary["step28_quality_pass_count"]) == 4
    assert int(summary["step28_comparison_pass_count"]) == 2
    assert int(summary["step28_large_file_count"]) == 0
    assert all(as_bool(row["pass"]) for row in rows)


def test_step29_default_modes_remain_unchanged():
...
def test_step29_docs_scope_and_forbidden_claims_are_valid():
    combined = "\n".join(
        read_text(path)
        for path in [
            "README.md",
...
def test_step29_artifact_budget_is_valid():
    summary = read_json("outputs/step29_artifact_manifest/artifact_summary.json")
    assert int(summary["large_file_count"]) == 0
    assert float(summary["step29_total_size_mb"]) < 20.0
    assert float(summary["total_size_mb"]) < 175.0
    assert int(summary["raw_candidate_large_file_count"]) == 0
    assert int(summary["scan_data_file_count"]) == 0
    assert int(summary["step29_vtr_count"]) == 0
    assert int(summary["step29_particle_npy_count"]) == 0
    assert int(summary["step29_quality_report_count"]) == 4
    assert int(summary["private_absolute_path_count"]) == 0
    manifest_rows = read_csv_rows("outputs/step29_artifact_manifest/artifact_manifest.csv")
    step29_paths = [row["path"] for row in manifest_rows if row["path"].startswith("outputs/step29")]
    assert not [path for path in step29_paths if path.endswith(".vtr")]
    assert not [path for path in step29_paths if path.endswith(".npy") and "particle" in path.lower()]


def test_step29_report_acceptance_complete():
    report = read_text("STEP29_CONTROLLED_REAL_GEOMETRY_64_STABILITY_ENVELOPE_REPORT.md")
    required_sections = [
        "## 1. Goal",
    ...
    assert (ROOT / "logs/step29_pytest.log").is_file()
    for path, marker in STEP29_LOG_MARKERS.items():
...
def assert_stability_driver_row(row):
    ...
    assert int(float(row["n_lbm_steps"])) == 20
    assert int(float(row["mpm_substeps_per_lbm_step"])) == 5
    assert int(float(row["completed_lbm_steps"])) >= 20
    assert int(float(row["total_mpm_substeps"])) >= 100
