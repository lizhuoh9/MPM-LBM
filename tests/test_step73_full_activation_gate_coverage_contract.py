...
def test_build_passes():
    rows, summary = build_step73_full_activation_gate_coverage_audit(ROOT)
    assert rows
    assert summary["full_activation_gate_coverage_audit_pass"] is True
    assert summary["required_gate_count"] == 10
    assert summary["step70_gate_count"] == 10
    assert summary["closed_gate_count"] == 10
    assert summary["activation_allowed_count"] == 0


def test_artifact_passes():
    payload = read_json("outputs/step73_full_activation_gate_coverage_audit/full_activation_gate_coverage.json")
    assert payload["rows"]
