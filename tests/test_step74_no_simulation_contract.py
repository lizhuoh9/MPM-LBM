...
def test_build_passes():
    rows, summary = build_step74_no_simulation_audit(ROOT)
    assert rows
    ...
    assert summary["step74_vtr_count"] == 0
    assert summary["step74_particle_npy_count"] == 0
    assert summary["protected_step74_file_count"] == 0


def test_artifact_passes():
    payload = read_json("outputs/step74_no_simulation_audit/no_simulation.json")
    assert payload["rows"]
