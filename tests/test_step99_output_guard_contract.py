...
def test_step99_output_guard_passes():
    payload = read_json("outputs/step99_output_guard/output_guard.json")
    ...
    assert summary["step99_raw_geometry_output_count"] == 0
    assert summary["step99_real_geometry_candidate_output_count"] == 0
    assert summary["step99_dense_wall_velocity_output_count"] == 0
    assert summary["step99_sparse_wall_velocity_output_count"] == 0
    assert summary["step99_dense_displacement_output_count"] == 0
    assert summary["step99_displaced_particle_output_count"] == 0
    ...
    assert summary["step99_large_file_count"] == 0
