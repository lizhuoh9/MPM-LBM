...
from .runtime_geometry_wall_velocity_envelope_config import (
...
def load_short_step_envelope_inputs(config_path):
    config = RuntimeGeometryWallVelocityShortStepEnvelopeConfig.from_json(_resolve_path(config_path))
    validation_rows = validate_short_step_envelope_config(config, root=_repo_root())
    if not all(bool(row["pass"]) for row in validation_rows):
        raise ValueError(f"invalid Step 47 envelope config: {validation_rows}")
...
def build_step_phase_sequence(config) -> list[float]:
    return [float(value) for value in config.phase_sequence]


def run_short_step_envelope_matrix(config_path) -> list[dict]:
    inputs = load_short_step_envelope_inputs(config_path)
...
def run_short_step_row(config, descriptor, runtime_projection_by_phase, original_projection_by_phase, wall_summary_by_phase) -> dict:
    ...
    row = {
        ...
        "phase_sequence": build_step_phase_sequence(config),
...
def summarize_short_step_envelope_matrix(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "original_static_row_count": sum(1 for row in rows if row["row_name"] == "original_static_32_5step"),
        "geometry_only_row_count": sum(1 for row in rows if row["row_name"] == "runtime_geometry_only_32_5step"),
        "wall_velocity_only_row_count": sum(1 for row in rows if row["row_name"] == "wall_velocity_only_32_5step"),
        "combined_row_count": sum(1 for row in rows if row["row_name"] == "runtime_geometry_plus_wall_velocity_32_5step"),
        ...
        "matrix_pass": bool(
            ...
            and all(len(row["step_records"]) == 5 for row in rows)
            and by_name["runtime_geometry_only_32_5step"]["active_cell_count_delta_from_original_max"] > 0
            and by_name["wall_velocity_only_32_5step"]["applied_cell_count_max"] > 0
            and by_name["runtime_geometry_plus_wall_velocity_32_5step"]["runtime_geometry_projection_enabled"]
            ...
            and by_name["runtime_geometry_plus_wall_velocity_32_5step"]["applied_cell_count_max"] > 0
        ),
    }


def write_short_step_envelope_rows(rows: list[dict], csv_path, json_path, npz_path, summary: dict | None = None) -> None:
    payload_summary = summarize_short_step_envelope_matrix(rows) if summary is None else summary
...
def _validate_descriptors(descriptors, config):
    ...
    if names != expected:
        raise ValueError(f"unexpected Step 47 row descriptors: {names}")
