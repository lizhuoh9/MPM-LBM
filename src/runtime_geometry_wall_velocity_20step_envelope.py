...
from .runtime_geometry_wall_velocity_20step_config import (
...
def load_twenty_step_envelope_inputs(config_path):
    config = RuntimeGeometryWallVelocityTwentyStepEnvelopeConfig.from_json(_resolve_path(config_path))
    validation_rows = validate_twenty_step_envelope_config(config, root=_repo_root())
    if not all(bool(row["pass"]) for row in validation_rows):
        raise ValueError(f"invalid Step 49 envelope config: {validation_rows}")
...
def build_twenty_step_phase_sequence(config) -> list[float]:
    return [float(value) for value in config.phase_sequence]


def run_twenty_step_envelope_matrix(config_path) -> list[dict]:
    inputs = load_twenty_step_envelope_inputs(config_path)
...
def run_twenty_step_row(config, descriptor, runtime_projection_by_phase, original_projection_by_phase, wall_summary_by_phase) -> dict:
    ...
    row = {
        ...
        "phase_sequence": build_twenty_step_phase_sequence(config),
...
def summarize_twenty_step_envelope_matrix(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "original_static_row_count": sum(1 for row in rows if row["row_name"] == "original_static_32_20step"),
        "geometry_only_row_count": sum(1 for row in rows if row["row_name"] == "runtime_geometry_only_32_20step"),
        "wall_velocity_only_row_count": sum(1 for row in rows if row["row_name"] == "wall_velocity_only_32_20step"),
        "combined_row_count": sum(1 for row in rows if row["row_name"] == "runtime_geometry_plus_wall_velocity_32_20step"),
        ...
        "matrix_pass": bool(
            ...
            and all(len(row["step_records"]) == 20 for row in rows)
            and by_name["runtime_geometry_only_32_20step"]["active_cell_count_delta_from_original_max"] > 0
            and by_name["wall_velocity_only_32_20step"]["applied_cell_count_max"] > 0
            and by_name["runtime_geometry_plus_wall_velocity_32_20step"]["runtime_geometry_projection_enabled"]
            ...
            and by_name["runtime_geometry_plus_wall_velocity_32_20step"]["applied_cell_count_max"] > 0
        ),
    }


def write_twenty_step_envelope_rows(rows: list[dict], csv_path, json_path, npz_path, summary: dict | None = None) -> None:
    payload_summary = summarize_twenty_step_envelope_matrix(rows) if summary is None else summary
...
def _validate_descriptors(descriptors, config):
    ...
    if names != list(ROW_NAMES):
        raise ValueError(f"unexpected Step 49 row descriptors: {names}")
...
def _stable_row(row: dict) -> bool:
    ...
    return bool(
        ...
        and len(row["step_records"]) == 20
        and int(row["completed_lbm_steps"]) >= 20
        and int(row["total_mpm_substeps"]) >= 100
