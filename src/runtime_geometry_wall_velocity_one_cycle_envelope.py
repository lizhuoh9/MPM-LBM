...
from .runtime_geometry_wall_velocity_one_cycle_config import (
...
def load_one_cycle_envelope_inputs(config_path):
    config = RuntimeGeometryWallVelocityOneCycleEnvelopeConfig.from_json(_resolve_path(config_path))
    validation_rows = validate_one_cycle_envelope_config(config, root=_repo_root())
    if not all(bool(row["pass"]) for row in validation_rows):
        raise ValueError(f"invalid Step 50 envelope config: {validation_rows}")
...
def build_one_cycle_phase_sequence(config) -> list[float]:
    return [float(value) for value in config.phase_sequence]


def run_one_cycle_envelope_matrix(config_path) -> list[dict]:
    inputs = load_one_cycle_envelope_inputs(config_path)
...
def build_one_cycle_closure_rows(config_path) -> list[dict]:
    inputs = load_one_cycle_envelope_inputs(config_path)
    config = inputs["config"]
    closure_rows = []
    for descriptor in inputs["descriptors"]:
...
def run_one_cycle_row(config, descriptor, runtime_projection_by_phase, original_projection_by_phase, wall_summary_by_phase) -> dict:
    runtime_enabled = bool(descriptor["runtime_geometry_projection_enabled"])
    wall_enabled = bool(descriptor["wall_velocity_application_enabled"])
    step_records = []
    for step_index, phase in enumerate(config.phase_sequence):
        ...
        step_records.append(_step_record(descriptor["row_name"], step_index, canonical_phase, projection_row, original_projection_row, wall_summary, runtime_enabled, wall_enabled))
    row = {
        "row_name": descriptor["row_name"],
        ...
        "phase_sequence": build_one_cycle_phase_sequence(config),
        "closure_phase": float(config.closure_phase),
...
def summarize_one_cycle_envelope_matrix(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "original_static_row_count": sum(1 for row in rows if row["row_name"] == "original_static_32_40step"),
        "geometry_only_row_count": sum(1 for row in rows if row["row_name"] == "runtime_geometry_only_32_40step"),
        "wall_velocity_only_row_count": sum(1 for row in rows if row["row_name"] == "wall_velocity_only_32_40step"),
        "combined_row_count": sum(1 for row in rows if row["row_name"] == "runtime_geometry_plus_wall_velocity_32_40step"),
        ...
        "matrix_pass": bool(
            ...
            and by_name["runtime_geometry_only_32_40step"]["active_cell_count_delta_from_original_max"] > 0
            and by_name["wall_velocity_only_32_40step"]["applied_cell_count_max"] > 0
            and by_name["runtime_geometry_plus_wall_velocity_32_40step"]["runtime_geometry_projection_enabled"]
            ...
            and by_name["runtime_geometry_plus_wall_velocity_32_40step"]["applied_cell_count_max"] > 0
        ),
    }


def write_one_cycle_envelope_rows(rows: list[dict], csv_path, json_path, npz_path, summary: dict | None = None) -> None:
    payload_summary = summarize_one_cycle_envelope_matrix(rows) if summary is None else summary
...
def _step_record(row_name, step_index, phase, projection_row, original_projection_row, wall_summary, runtime_enabled, wall_enabled) -> dict:
...
def _validate_descriptors(descriptors, config):
    ...
    if names != list(ROW_NAMES):
        raise ValueError(f"unexpected Step 50 row descriptors: {names}")
    for descriptor in descriptors:
        ...
        if "link_area" in json.dumps(descriptor):
            raise ValueError(f"descriptor contains forbidden transfer wording: {descriptor}")
