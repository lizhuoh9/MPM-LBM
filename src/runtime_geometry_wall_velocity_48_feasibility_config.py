...
EXPECTED_FEASIBILITY_ID = "step52_controlled_48_engineering_one_cycle_feasibility"
...
EXPECTED_TRANSFER_MODE = "engineering"
...
class RuntimeGeometryWallVelocity48FeasibilityConfig:
    feasibility_id: str
    base_step51_config_path: str
    step51_matrix_path: str
    ...
    scope_note: str = "48^3 engineering-only one-cycle static plus combined feasibility probe"
...
def validate_48_feasibility_config(config: RuntimeGeometryWallVelocity48FeasibilityConfig, root=None) -> list[dict]:
    root_path = _repo_root() if root is None else Path(root)
    rows = [
        _check("feasibility_id_expected", config.feasibility_id == EXPECTED_FEASIBILITY_ID, config.feasibility_id, "Step 52 feasibility id must be stable"),
        _check("base_step51_config_path_exists", _exists(root_path, config.base_step51_config_path), config.base_step51_config_path, "Step 51 config must exist"),
        _check("step51_matrix_path_exists", _exists(root_path, config.step51_matrix_path), config.step51_matrix_path, "Step 51 matrix output must exist"),
        ...
        _check("n_grid_expected_48", config.n_grid == EXPECTED_N_GRID, config.n_grid, "Step 52 n_grid must be 48"),
        _check("n_lbm_steps_expected", config.n_lbm_steps == EXPECTED_N_LBM_STEPS, config.n_lbm_steps, "Step 52 must use forty LBM steps"),
        _check("mpm_substeps_expected", config.mpm_substeps_per_lbm_step == EXPECTED_MPM_SUBSTEPS, config.mpm_substeps_per_lbm_step, "Step 52 must use five MPM substeps per LBM step"),
        _check("cycle_period_steps_expected", config.cycle_period_steps == EXPECTED_CYCLE_PERIOD_STEPS, config.cycle_period_steps, "Step 52 must use a forty-step prescribed cycle"),
        _check("closure_phase_expected", math.isclose(config.closure_phase, EXPECTED_CLOSURE_PHASE, rel_tol=0.0, abs_tol=1.0e-12), config.closure_phase, "Step 52 diagnostic closure phase must be 1.0"),
        _check("phase_sequence_expected", _same_phase_sequence(config.phase_sequence, EXPECTED_PHASE_SEQUENCE), list(config.phase_sequence), "Step 52 phase sequence must match Step 50/51"),
        _check("phase_count_expected", len(config.phase_sequence) == 40, len(config.phase_sequence), "Step 52 must use forty runner phases"),
        ...
        _check("phase_starts_at_0", math.isclose(config.phase_sequence[0], 0.0, rel_tol=0.0, abs_tol=1.0e-12), config.phase_sequence[0], "Step 52 phase sequence must start at 0.0"),
        _check("phase_ends_at_0975", math.isclose(config.phase_sequence[-1], 0.975, rel_tol=0.0, abs_tol=1.0e-12), config.phase_sequence[-1], "Step 52 phase sequence must end at 0.975"),
        _check("transfer_mode_engineering", config.transfer_mode == EXPECTED_TRANSFER_MODE, config.transfer_mode, "Step 52 must use engineering transfer only"),
        _check("no_link_area_transfer_mode", config.transfer_mode != "link_area_experimental", config.transfer_mode, "Step 52 must not include link_area_experimental"),
        _check("coupling_mode_expected", config.coupling_mode == EXPECTED_COUPLING_MODE, config.coupling_mode, "Step 52 coupling mode must be moving_boundary"),
        ...
        _check("diagnostic_only_true", config.diagnostic_only is True, config.diagnostic_only, "Step 52 must remain diagnostic-only"),
        _check("scope_note_valid", "48^3" in config.scope_note and "engineering-only" in config.scope_note and "one-cycle" in config.scope_note, config.scope_note, "scope note must state 48^3 engineering-only one-cycle scope"),
        _check("no_64_grid", config.n_grid != 64, config.n_grid, "Step 52 must not include 64^3 rows"),
        _check("no_multi_cycle", config.n_lbm_steps == config.cycle_period_steps, {"n_lbm_steps": config.n_lbm_steps, "cycle_period_steps": config.cycle_period_steps}, "Step 52 must remain one-cycle"),
    ]
    for field in MUTATION_FLAG_FIELDS:
...
def summarize_48_feasibility_config_validation(rows: list[dict], config: RuntimeGeometryWallVelocity48FeasibilityConfig) -> dict:
    return {
        "row_count": len(rows),
        "pass_count": sum(1 for row in rows if bool(row["pass"])),
        "validation_pass": all(bool(row["pass"]) for row in rows),
        "feasibility_id": config.feasibility_id,
        ...
        "no_link_area_experimental_row": config.transfer_mode != "link_area_experimental",
...
def mutation_flags(config: RuntimeGeometryWallVelocity48FeasibilityConfig) -> dict:
    return {field: bool(getattr(config, field)) for field in MUTATION_FLAG_FIELDS}


def mutation_flag_enabled_count(config: RuntimeGeometryWallVelocity48FeasibilityConfig) -> int:
