...
from .wall_velocity_application import build_wall_velocity_grid, load_wall_velocity_application_config
...
def load_transfer_comparison_config(path: str):
    return RuntimeGeometryWallVelocityTransferConfig.from_json(path)


def load_transfer_comparison_inputs(config_path):
    config = load_transfer_comparison_config(config_path)
    validation_rows = validate_transfer_comparison_config(config, root=_repo_root())
    if not all(bool(row["pass"]) for row in validation_rows):
        raise ValueError(f"invalid Step 51 transfer comparison config: {validation_rows}")
    descriptors = build_transfer_row_descriptors(config)
...
def build_transfer_row_descriptors(config) -> list[dict]:
    return [read_json(path) for path in ROW_DESCRIPTOR_PATHS]


def run_transfer_comparison_matrix(config_path: str) -> list[dict]:
    inputs = load_transfer_comparison_inputs(config_path)
...
def build_transfer_closure_rows(config_path) -> list[dict]:
    inputs = load_transfer_comparison_inputs(config_path)
    config = inputs["config"]
    closure_rows = []
    for descriptor in inputs["descriptors"]:
        ...
        transfer_mode = descriptor["reaction_transfer_mode"]
        ...
        phase0_original = inputs["original_projection_by_phase"][phase0]
        closure_original = inputs["original_projection_by_phase"][phase1]
        ...
        phase0_area = _area_scale(config, transfer_mode, phase0_projection, phase0_original)
        closure_area = _area_scale(config, transfer_mode, closure_projection, closure_original)
        row = {
            "row_name": descriptor["row_name"],
            "component_name": descriptor["component_name"],
            "transfer_mode": transfer_mode,
            "reaction_transfer_mode": transfer_mode,
            ...
            "phase0_area_scale": float(phase0_area["area_scale"]),
            "closure_area_scale": float(closure_area["area_scale"]),
            "phase0_phase1_area_scale_delta": abs(float(closure_area["area_scale"]) - float(phase0_area["area_scale"])),
        ...
        row["closure_pass"] = bool(
            ...
            and math.isfinite(row["phase0_phase1_area_scale_delta"])
            ...
            and config.link_area_scale_min <= float(row["phase0_area_scale"]) <= config.link_area_scale_max
            and config.link_area_scale_min <= float(row["closure_area_scale"]) <= config.link_area_scale_max
...
def run_transfer_row(row_descriptor, config, runtime_projection_by_phase, original_projection_by_phase, wall_summary_by_phase) -> dict:
    runtime_enabled = bool(row_descriptor["runtime_geometry_projection_enabled"])
    wall_enabled = bool(row_descriptor["wall_velocity_application_enabled"])
    transfer_mode = row_descriptor["reaction_transfer_mode"]
    step_records = []
    for step_index, phase in enumerate(config.phase_sequence):
        ...
        step_records.append(
            _step_record(
                config,
                row_descriptor["row_name"],
                row_descriptor["component_name"],
                transfer_mode,
                step_index,
                canonical_phase,
                projection_row,
                original_projection_row,
                wall_summary,
                runtime_enabled,
                wall_enabled,
            )
    ...
    row = {
        "row_name": row_descriptor["row_name"],
        "component_name": row_descriptor["component_name"],
        "transfer_mode": transfer_mode,
        ...
        "area_scale_min_observed": _min(step_records, "area_scale"),
        "area_scale_max_observed": _max(step_records, "area_scale"),
        "area_scale_final": float(step_records[-1]["area_scale"]),
        "raw_area_scale_final": float(step_records[-1]["raw_area_scale"]),
        "has_nan": any(bool(step["has_nan"]) for step in step_records),
        "has_inf": any(bool(step["has_inf"]) for step in step_records),
        "diagnostic_only": bool(config.diagnostic_only and row_descriptor["diagnostic_only"]),
        "persist_projected_state": bool(config.persist_projected_state or row_descriptor["persist_projected_state"]),
        "persist_displaced_geometry": bool(config.persist_displaced_geometry or row_descriptor["persist_displaced_geometry"]),
        "persist_lbm_solid_vel": bool(config.persist_lbm_solid_vel or row_descriptor["persist_lbm_solid_vel"]),
        "complete_cycle_claim": bool(row_descriptor["complete_cycle_claim"]),
        "production_geometry_claim": bool(row_descriptor["production_geometry_claim"]),
        "one_cycle_diagnostic_envelope": True,
...
def summarize_transfer_comparison_matrix(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "component_count": len({row["component_name"] for row in rows}),
        ...
        "area_scale_min_observed": min(float(row["area_scale_min_observed"]) for row in rows) if rows else 0.0,
        "area_scale_max_observed": max(float(row["area_scale_max_observed"]) for row in rows) if rows else 0.0,
        ...
        "matrix_pass": bool(
            ...
            and set(row["component_name"] for row in engineering) == set(COMPONENT_NAMES)
            and set(row["component_name"] for row in link_area) == set(COMPONENT_NAMES)
            and by_name["engineering_runtime_geometry_only_32_40step"]["active_cell_count_delta_from_original_max"] > 0
            and by_name["link_area_runtime_geometry_only_32_40step"]["active_cell_count_delta_from_original_max"] > 0
            and by_name["engineering_wall_velocity_only_32_40step"]["applied_cell_count_max"] > 0
            and by_name["link_area_wall_velocity_only_32_40step"]["applied_cell_count_max"] > 0
            and by_name["engineering_runtime_geometry_plus_wall_velocity_32_40step"]["runtime_geometry_projection_enabled"]
            and by_name["link_area_runtime_geometry_plus_wall_velocity_32_40step"]["runtime_geometry_projection_enabled"]
            and by_name["engineering_runtime_geometry_plus_wall_velocity_32_40step"]["wall_velocity_application_enabled"]
            and by_name["link_area_runtime_geometry_plus_wall_velocity_32_40step"]["wall_velocity_application_enabled"]
        ),
    }


def write_transfer_envelope_outputs(rows: list[dict], csv_path, json_path, npz_path=None, summary: dict | None = None) -> None:
    payload_summary = summarize_transfer_comparison_matrix(rows) if summary is None else summary
    write_csv_rows(csv_path, rows, TRANSFER_FIELDS)
    ...
    np.savez_compressed(
        ...
        transfer_mode=np.asarray([row["transfer_mode"] for row in rows]),
        ...
        area_scale_final=np.asarray([float(row["area_scale_final"]) for row in rows], dtype=np.float64),
    )
...
def _step_record(config, row_name, component_name, transfer_mode, step_index, phase, projection_row, original_projection_row, wall_summary, runtime_enabled, wall_enabled) -> dict:
    ...
    area = _area_scale(config, transfer_mode, projection_row, original_projection_row)
    transfer_multiplier = float(area["area_scale"]) if transfer_mode == "link_area_experimental" else 1.0
    bb_max_correction = max_applied_velocity_norm * transfer_multiplier
    hydro_force_max_norm = float((projection_delta * 1.0e-5) + (applied_cell_count * max_applied_velocity_norm * 1.0e-6)) * transfer_multiplier
    ...
    return add_proxy_step_metadata({
        ...
        "raw_area_scale": float(area["raw_area_scale"]),
        "area_scale": float(area["area_scale"]),
        "link_area_scale_min": float(config.link_area_scale_min),
...
def _area_scale(config, transfer_mode, projection_row, original_projection_row) -> dict:
    if transfer_mode != "link_area_experimental":
        return {"raw_area_scale": 1.0, "area_scale": 1.0}
    active = max(float(projection_row["active_cell_count"]), 1.0)
    original = max(float(original_projection_row["active_cell_count"]), 1.0)
    raw_area_scale = math.sqrt(original / active)
    if not math.isfinite(raw_area_scale):
        raw_area_scale = float(config.link_area_scale_min)
    area_scale = min(max(raw_area_scale, float(config.link_area_scale_min)), float(config.link_area_scale_max))
    return {"raw_area_scale": raw_area_scale, "area_scale": area_scale}
...
def _validate_descriptors(descriptors, config):
    ...
    if names != list(ROW_NAMES):
        raise ValueError(f"unexpected Step 51 row descriptors: {names}")
    transfer_components = {mode: [] for mode in config.transfer_modes}
    for descriptor in descriptors:
        ...
        if descriptor["reaction_transfer_mode"] not in config.transfer_modes:
            raise ValueError(f"descriptor transfer mode mismatch: {descriptor}")
        if descriptor["component_name"] not in COMPONENT_NAMES:
            raise ValueError(f"descriptor component mismatch: {descriptor}")
        ...
        transfer_components[descriptor["reaction_transfer_mode"]].append(descriptor["component_name"])
    for transfer_mode, components in transfer_components.items():
        if tuple(components) != COMPONENT_NAMES:
            raise ValueError(f"descriptor components for {transfer_mode} mismatch: {components}")
...
def _stable_row(row: dict) -> bool:
    ...
    return bool(
        ...
        and 0.25 <= float(row["area_scale_min_observed"]) <= 2.0
        and 0.25 <= float(row["area_scale_max_observed"]) <= 2.0
        ...
        and bool(row["one_cycle_diagnostic_envelope"])
    )
