...
from src.mpm_lbm.evidence.step104_common import read_json
...
def build_step104_output_guard(
    root: Path,
    policy_path: str = "configs/step104_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    video_extensions = set(policy["video_extensions"])
    rows = [output_row(root, path, policy) for path in step104_output_files(root)]
    ...
    summary = {
        "artifact_budget_pass": bool(total_size_mb < float(policy["max_step104_total_size_mb"]) and large_file_count == 0),
        ...
        "row_count": len(rows),
        "step104_ansys_proprietary_file_count": sum(1 for row in rows if is_ansys_proprietary_output(row, policy)),
        "step104_fluent_run_output_count": sum(1 for row in rows if is_fluent_run_output(row, policy)),
        "step104_forbidden_file_count": len(forbidden_rows),
        "step104_large_file_count": large_file_count,
        "step104_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step104_private_fluent_csv_count": sum(1 for row in rows if row["path"].startswith("benchmarks/private/")),
        "step104_step36_wall_velocity_reference_count": len(step36_references),
        "step104_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step104_total_size_mb": total_size_mb,
        "step104_video_count": sum(1 for row in rows if row["extension"] in video_extensions),
        "step104_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        rows
        and summary["row_count"] == summary["pass_count"]
        and summary["step104_ansys_proprietary_file_count"] == 0
        and summary["step104_fluent_run_output_count"] == 0
        and summary["step104_forbidden_file_count"] == 0
        and summary["step104_particle_npy_count"] == 0
        and summary["step104_private_fluent_csv_count"] == 0
        and summary["step104_step36_wall_velocity_reference_count"] == 0
        and summary["step104_video_count"] == 0
        and summary["step104_vtr_count"] == 0
...
def step104_output_files(root: Path) -> list[Path]:
    files = []
    for base in (root / "configs", root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith(("outputs/step104_output_guard/", "outputs/step104_artifact_manifest/")):
                continue
            if rel.startswith(("configs/step104_", "outputs/step104_", "logs/step104_")):
                files.append(path)
    return sorted(files)
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step104_output_files(root):
