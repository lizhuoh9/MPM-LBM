...
def build_step80_output_guard(
    root: Path,
    policy_path: str = "configs/step80_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    rows = [output_row(root, path, policy) for path in step80_output_files(root)]
    ...
    optional_dirs = step80_optional_driver_run_dirs(root, policy)
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step80_dense_displacement_output_count": sum(1 for row in rows if "dense_displacement" in row["path"].lower()),
        "step80_displaced_particle_output_count": sum(1 for row in rows if "displaced_particle" in row["path"].lower()),
        "step80_forbidden_file_count": len(forbidden_rows),
        "step80_large_file_count": sum(1 for row in rows if int(row["size_bytes"]) > int(policy["large_file_threshold_bytes"])),
        "step80_optional_driver_run_dir_count": len(optional_dirs),
        "step80_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step80_raw_geometry_output_count": sum(1 for row in rows if row["extension"] in RAW_GEOMETRY_EXTENSIONS),
        "step80_required_driver_run_dir_count": len(required_dirs),
        "step80_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step80_total_size_mb": sum(int(row["size_bytes"]) for row in rows) / (1024.0 * 1024.0),
        "step80_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        rows
        and summary["row_count"] == summary["pass_count"]
        and summary["step80_required_driver_run_dir_count"] == len(policy["required_driver_run_dirs"])
        and summary["step80_optional_driver_run_dir_count"] == 0
        and summary["step80_vtr_count"] == 0
        and summary["step80_particle_npy_count"] == 0
        and summary["step80_displaced_particle_output_count"] == 0
        and summary["step80_dense_displacement_output_count"] == 0
        and summary["step80_raw_geometry_output_count"] == 0
        and summary["step80_large_file_count"] == 0
        and summary["step80_forbidden_file_count"] == 0
...
def step80_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step80_") or rel.startswith("logs/step80_"):
                files.append(path)
    return sorted(files)
...
def step80_optional_driver_run_dirs(root: Path, policy: dict) -> list[Path]:
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step80_output_files(root):
