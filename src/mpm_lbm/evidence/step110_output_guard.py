...
from src.mpm_lbm.evidence.step110_common import read_json


PRIVATE_ABSOLUTE_RE = re.compile(r"([A-Za-z]:[\\/]+Users[\\/]+|D:[\\/]+working[\\/]+)")


def build_step110_output_guard(root: Path, policy_path: str = "configs/step110_output_guard_policy.json") -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    rows = [output_row(root, path, policy) for path in step110_files(root)]
    ...
    summary = {
        "artifact_budget_pass": bool(total_size_bytes / (1024.0 * 1024.0) < float(policy["max_step110_total_size_mb"]) and large_file_count == 0),
        ...
        "official_journal_file_count": sum(1 for row in rows if row["extension"] in {".jou"}),
        ...
        "row_count": len(rows),
        "step110_forbidden_file_count": sum(1 for row in rows if row["forbidden"]),
        "step110_large_file_count": large_file_count,
        "step110_total_size_bytes": total_size_bytes,
        "step110_total_size_mb": total_size_bytes / (1024.0 * 1024.0),
    ...
    summary["output_guard_pass"] = bool(
        ...
        and summary["step110_forbidden_file_count"] == 0
...
def step110_files(root: Path) -> list[Path]:
    files = []
    for base in (root / "configs", root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith(("outputs/step110_output_guard/", "outputs/step110_artifact_manifest/")):
                continue
            if rel.startswith(("configs/step110_", "outputs/step110_", "logs/step110_")):
                files.append(path)
    return sorted(files)
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step110_files(root):
