...
STEP84_SMOKE_FIELDS = [
    ...
    "boundary_motion_diagnostic_only",
    "wall_velocity_application_config_path_exists",
    "wall_velocity_application_report_exists",
    "wall_velocity_application_report_pass",
    "target_lbm_field",
    "application_policy",
    "apply_to_lbm_solid_vel_wall_velocity",
...
def build_step84_runtime_geometry_wall_velocity_combined_smoke_matrix(
    root: Path,
    matrix_policy_path: str = "configs/step84_runtime_geometry_wall_velocity_combined_smoke_matrix.json",
    acceptance_policy_path: str = "configs/step84_runtime_geometry_wall_velocity_combined_acceptance_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    matrix_policy = read_json(root / matrix_policy_path)
    acceptance_policy = read_json(root / acceptance_policy_path)
    enforce_step84_policy(matrix_policy)
    rows = [
        run_step84_runtime_geometry_wall_velocity_combined_case(root, config_path, matrix_policy, acceptance_policy)
        for config_path in matrix_policy["required_smoke_configs"]
    ]
    summary = step84_runtime_geometry_wall_velocity_combined_summary(rows, matrix_policy, acceptance_policy)
    return rows, summary


def run_step84_runtime_geometry_wall_velocity_combined_case(
    root: Path,
    config_path: str,
    matrix_policy: dict,
    ...
    enforce_step84_config(root, config, row_name, acceptance_policy)
    ...
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step84 combined smoke row: {row_name}")
    ...
    row = {
        ...
        **{field: bool(geometry_summary.get(field, True)) for field in MUTATION_FLAG_FIELDS},
        ...
        "stable": False,
        "notes": "runtime geometry diagnostic-only plus wall velocity solid_vel canonical driver 3-step smoke completed",
    }
    row["stable"] = step84_runtime_geometry_wall_velocity_combined_row_pass(row, acceptance_policy)
    if not row["stable"]:
        raise RuntimeError(f"Step84 combined smoke row failed acceptance: {row}")
    return row


def enforce_step84_policy(matrix_policy: dict) -> None:
    expected = {
        "post_gate_simulation_allowed": True,
        "allowed_next_step": "Step84",
        "allowed_next_step_scope": (
            "runtime geometry diagnostic-only plus wall velocity solid_vel combined "
            "canonical driver 3-step smoke only"
        ),
        "gate_source_step": "Step83",
        "previous_step": "Step83",
    ...
    if actual != expected:
        raise RuntimeError(f"Step84 policy mismatch: {actual} != {expected}")
    if matrix_policy.get("required_smoke_configs") != [
        ...
        raise RuntimeError("Step84 must run exactly one combined runtime geometry plus wall velocity config")


def enforce_step84_config(root: Path, config, row_name: str, policy: dict) -> None:
    if row_name not in set(policy["required_row_names"]):
        raise RuntimeError(f"unexpected Step84 combined smoke row: {row_name}")
    expected = {
    ...
    if actual != expected:
        raise RuntimeError(f"Step84 config mismatch for {row_name}: {actual} != {expected}")
    if list(config.target_u_lbm) != [0.0, 0.0, 0.0]:
        raise RuntimeError(f"Step84 row must use zero background target_u_lbm: {row_name}")
    if config.coupling_mode != "moving_boundary" or config.reaction_transfer_mode != "engineering":
        raise RuntimeError(f"Step84 row must use moving_boundary engineering transfer: {row_name}")
    if config.geometry_type != "box" or config.geometry_config_path is not None:
        raise RuntimeError(f"Step84 row must use default box geometry: {row_name}")
    if config.output_interval != 1 or config.write_vtk or config.write_particles:
        raise RuntimeError(f"Step84 row must use interval=1 and disable heavy outputs: {row_name}")
    if config.quality_check_enabled or config.quality_check_strict:
        raise RuntimeError(f"Step84 row must keep quality gates disabled: {row_name}")
    if config.boundary_motion_mode != "prescribed_kinematic":
        raise RuntimeError(f"Step84 row must use prescribed_kinematic boundary motion: {row_name}")
    if not config.boundary_motion_report_enabled:
        raise RuntimeError(f"Step84 row must write boundary motion report: {row_name}")
    if config.geometry_motion_mode != "prescribed_kinematic":
        raise RuntimeError(f"Step84 row must use prescribed_kinematic geometry motion: {row_name}")
    if config.geometry_motion_application_mode != "diagnostic_only":
        raise RuntimeError(f"Step84 row must use diagnostic_only geometry motion application: {row_name}")
    if not config.geometry_motion_report_enabled or not config.geometry_motion_application_report_enabled:
        raise RuntimeError(f"Step84 row must write geometry motion reports: {row_name}")
    if config.wall_velocity_application_mode != "solid_vel_experimental":
        raise RuntimeError(f"Step84 row must use solid_vel_experimental wall velocity: {row_name}")
    if not config.wall_velocity_application_report_enabled:
        raise RuntimeError(f"Step84 row must write wall velocity application report: {row_name}")
    for label, relative_path in {
        ...
        if not path_exists_from_repo_root(root, relative_path):
            raise RuntimeError(f"Step84 {label} is not resolvable: {relative_path}")
...
def step84_runtime_geometry_wall_velocity_combined_row_pass(row: dict, policy: dict) -> bool:
    ...
    feature_pass = bool(
        int(row["activation_feature_count"]) == 2
    ...
    return bool(
        ...
        and row["allowed_next_step"] == "Step84"
        and row["allowed_next_step_scope"]
        == "runtime geometry diagnostic-only plus wall velocity solid_vel combined canonical driver 3-step smoke only"
        and row["previous_step"] == "Step83"
...
def step84_runtime_geometry_wall_velocity_combined_summary(
    ...
    summary = {
        ...
        "step84_runtime_geometry_wall_velocity_combined_smoke_matrix_pass": False,
    ...
    summary["step84_runtime_geometry_wall_velocity_combined_smoke_matrix_pass"] = bool(
        summary["post_gate_simulation_allowed"] is True
        and summary["allowed_next_step"] == "Step84"
        and summary["allowed_next_step_scope"]
        == "runtime geometry diagnostic-only plus wall velocity solid_vel combined canonical driver 3-step smoke only"
        and summary["previous_step"] == "Step83"
        ...
        and summary["activation_feature_count"] == 2
        ...
        and summary["physics_feature_expansion"] == "runtime_geometry_diagnostic_only_wall_velocity_solid_vel_only"
    )
    return summary
...
def reset_generated_run_dir(run_root: Path, out_dir: Path) -> None:
    ...
    if resolved_out_dir.parent != resolved_run_root:
        raise RuntimeError(f"refusing to reset unexpected Step84 run directory: {out_dir}")
    if out_dir.exists():
        shutil.rmtree(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)


def geometry_report_pass(report_path: Path, summary: dict) -> bool:
    return bool(
        report_path.is_file()
        and summary.get("no_op_pass") is True
        and summary.get("config_validation_pass") is True
        and summary.get("diagnostic_only") is True
        and int(summary.get("mutation_flag_enabled_count", -1)) == 0
    )


def boundary_report_pass(report_path: Path, summary: dict) -> bool:
    return bool(
        report_path.is_file()
        and summary.get("config_validation_pass") is True
        and summary.get("no_op_pass") is True
        and summary.get("diagnostic_only") is True
        and summary.get("boundary_motion_mode") == "prescribed_kinematic"
    )


def report_summary(report_path: Path) -> dict:
...
@contextmanager
def combined_report_repo_roots(root: Path):
    from src.mpm_lbm.sim.motion import boundary_motion_config, boundary_motion_interface, geometry_motion_interface
    from src.mpm_lbm.sim.squid_proxy import kinematics_schedule, motion_mapping
    from src.mpm_lbm.sim.wall_velocity import application, application_config, config, field

    root_modules = [
        boundary_motion_config,
        boundary_motion_interface,
        geometry_motion_interface,
        application,
        application_config,
        config,
        field,
    ]
    resolve_modules = [kinematics_schedule, motion_mapping]
    originals = []
    for module in root_modules:
        originals.append((module, "_repo_root", module._repo_root))
        module._repo_root = lambda root=root: Path(root)
    for module in resolve_modules:
        originals.append((module, "_resolve_path", module._resolve_path))
        module._resolve_path = lambda path, root=root: Path(path) if Path(path).is_absolute() else Path(root) / Path(path)
    try:
        yield
    finally:
...
def row_name_from_config(config_path: str) -> str:
    return Path(config_path).stem.removeprefix("step84_")
