...
DURATION_32_FIELDS = [
    ...
    "elapsed_seconds",
    "runtime_warning",
    "runtime_hard_limit_exceeded",
    "soft_rho_min_warning",
    "soft_rho_max_warning",
    "stable",
    "notes",
]


def build_canonical_driver_32_duration_matrix(
    root: Path,
    matrix_policy_path: str = "configs/step62_controlled_canonical_32_moving_boundary_3step_duration.json",
    ...
    config_paths = list(matrix_policy["required_duration_configs"])
    if matrix_policy.get("run_optional_penalty_32_3step", False):
        config_paths.extend(matrix_policy.get("optional_penalty_32_3step_configs", []))
    if matrix_policy.get("run_optional_32_5step", False):
        config_paths.extend(matrix_policy.get("optional_32_5step_configs", []))
    rows = [
        run_32_duration_case(root, config_path, matrix_policy, acceptance_policy)
        for config_path in config_paths
    ]
    summary = duration_32_summary(rows, matrix_policy, acceptance_policy)
    return rows, summary


def run_32_duration_case(root: Path, config_path: str, matrix_policy: dict, acceptance_policy: dict) -> dict:
    ...
    enforce_32_duration_config(config, row_name, acceptance_policy)
    ...
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step 62 32 duration row: {row_name}")
    ...
    row = {
        ...
        "stable": False,
        "runtime_hard_limit_exceeded": elapsed_seconds > float(acceptance_policy["max_runtime_seconds"]),
        "soft_rho_min_warning": min_float(diagnostics, "rho_min") < float(acceptance_policy["soft_rho_min_warning"]),
        "soft_rho_max_warning": max_float(diagnostics, "rho_max") > float(acceptance_policy["soft_rho_max_warning"]),
        "notes": "controlled canonical 32^3 moving-boundary 3-step duration probe completed",
    }
    row["stable"] = duration_32_row_pass(row, acceptance_policy)
    if not row["stable"]:
        raise RuntimeError(f"Step 62 32 duration row failed acceptance: {row}")
    return row


def enforce_32_duration_config(config, row_name: str, policy: dict) -> None:
    allowed = set(policy["required_row_names"]) | set(policy.get("optional_row_names", []))
    if row_name not in allowed:
        raise RuntimeError(f"unexpected Step 62 32 duration row: {row_name}")
    if row_name in set(policy["required_row_names"]):
        expected_steps = int(policy["required_n_lbm_steps"])
    elif row_name.endswith("_5step_optional"):
        expected_steps = 5
    else:
        expected_steps = 3
    expected = {
    ...
    if actual != expected:
        raise RuntimeError(f"Step 62 config mismatch for {row_name}: {actual} != {expected}")
    if config.output_interval != 1 or config.write_vtk or config.write_particles:
        raise RuntimeError(f"Step 62 rows must use interval=1 and disable heavy outputs: {row_name}")
    if config.geometry_type != "box" or config.geometry_config_path is not None:
        raise RuntimeError(f"Step 62 rows must use default box geometry: {row_name}")
    if config.quality_check_enabled or config.quality_check_strict:
        raise RuntimeError(f"Step 62 rows must keep quality gates disabled: {row_name}")
    if config.boundary_motion_mode != "static" or config.boundary_motion_config_path is not None:
        raise RuntimeError(f"Step 62 rows must keep boundary motion static: {row_name}")
    if config.wall_velocity_application_mode != "disabled" or config.wall_velocity_application_config_path is not None:
        raise RuntimeError(f"Step 62 rows must keep wall velocity disabled: {row_name}")
    if config.geometry_motion_mode != "static" or config.geometry_motion_application_mode != "disabled":
        raise RuntimeError(f"Step 62 rows must keep geometry motion disabled: {row_name}")


def duration_32_row_pass(row: dict, policy: dict) -> bool:
    ...
    return bool(
        ...
        and not row["runtime_hard_limit_exceeded"]
        and mode_pass
        and finite_values(row, excluded={"notes", "row_name", "coupling_mode", "reaction_transfer_mode", "canonical_driver_module", "geo_path_name"})
    )


def duration_32_summary(rows: list[dict], matrix_policy: dict, acceptance_policy: dict) -> dict:
    ...
    summary = {
        ...
        "run_optional_penalty_32_3step": bool(matrix_policy["run_optional_penalty_32_3step"]),
        "run_optional_32_5step": bool(matrix_policy["run_optional_32_5step"]),
        ...
        "duration_32_matrix_pass": False,
    }
    summary["duration_32_matrix_pass"] = bool(
        ...
        and summary["runtime_hard_limit_exceeded_count"] == 0
        and summary["optional_row_count"] == 0
        and not summary["run_optional_penalty_32_3step"]
        and not summary["run_optional_32_5step"]
...
def reset_generated_run_dir(run_root: Path, out_dir: Path) -> None:
    ...
    if resolved_out_dir.parent != resolved_run_root:
        raise RuntimeError(f"refusing to reset unexpected Step 62 run directory: {out_dir}")
    if out_dir.exists():
...
def row_name_from_config(config_path: str) -> str:
    return Path(config_path).stem.removeprefix("step62_")
