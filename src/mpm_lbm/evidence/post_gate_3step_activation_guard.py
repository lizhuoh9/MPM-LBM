...
def build_step77_post_gate_3step_activation_guard(
    root: Path,
    policy_path: str = "configs/step77_activation_guard_policy.json",
    ...
    summary = {
        ...
        "post_gate_3step_activation_guard_pass": False,
        "previous_activation_feature_count": previous_matrix["summary"]["activation_feature_count"],
        "row_count": len(rows),
    }
    summary["post_gate_3step_activation_guard_pass"] = bool(
        rows
...
def matrix_summary_checks(summary: dict, policy: dict) -> list[dict]:
    checks = [
        ("post_gate_3step_rebaseline_matrix_pass", summary["post_gate_3step_rebaseline_matrix_pass"], True),
        ("allowed_next_step", summary["allowed_next_step"], "Step77"),
        (
            "allowed_next_step_scope",
            summary["allowed_next_step_scope"],
            "minimal post-gate canonical driver 3-step rebaseline only",
        ),
        ("previous_rebaseline_step", summary["previous_rebaseline_step"], "Step76"),
...
def previous_matrix_summary_checks(summary: dict) -> list[dict]:
    checks = [
        ("step76_matrix_pass", summary["post_gate_rebaseline_matrix_pass"], True),
        ("step76_activation_feature_count", summary["activation_feature_count"], 0),
        ("step76_optional_row_count", summary["optional_row_count"], 0),
    ]
    return [
        {
            "actual": actual,
