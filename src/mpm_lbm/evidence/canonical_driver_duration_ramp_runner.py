...
def build_canonical_driver_duration_ramp_matrix(
    root: Path,
    matrix_policy_path: str = "configs/step60_controlled_canonical_moving_boundary_duration_ramp.json",
    ...
    if matrix_policy.get("run_optional_32_probe", False):
        config_paths.extend(matrix_policy.get("optional_duration_configs", []))
    rows = [
        run_duration_ramp_case(root, config_path, matrix_policy, acceptance_policy)
        for config_path in config_paths
    ]
    summary = duration_ramp_summary(rows, matrix_policy, acceptance_policy)
    return rows, summary


def run_duration_ramp_case(root: Path, config_path: str, matrix_policy: dict, acceptance_policy: dict) -> dict:
    ...
    enforce_duration_ramp_config(config, row_name, acceptance_policy)
    ...
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step 60 duration ramp row: {row_name}")
    ...
    row = {
        ...
        "stable": False,
        "notes": "controlled canonical real-driver duration ramp completed",
    }
    row["stable"] = duration_ramp_row_pass(row, acceptance_policy)
    if not row["stable"]:
        raise RuntimeError(f"Step 60 duration ramp row failed acceptance: {row}")
    return row


def enforce_duration_ramp_config(config, row_name: str, policy: dict) -> None:
    allowed = set(policy["required_row_names"]) | set(policy.get("optional_row_names", []))
    if row_name not in allowed:
        raise RuntimeError(f"unexpected Step 60 duration ramp row: {row_name}")
    if row_name in set(policy["required_row_names"]):
        expected_grid = int(policy["common_required_n_grid"])
        expected_particles = int(policy["common_required_n_particles"])
    else:
        expected_grid = 32
        expected_particles = 1024
    expected_steps = expected_lbm_steps(row_name)
    expected = {
        "n_grid": expected_grid,
    ...
    if actual != expected:
        raise RuntimeError(f"Step 60 config mismatch for {row_name}: {actual} != {expected}")
    if config.output_interval != 1 or config.write_vtk or config.write_particles:
        raise RuntimeError(f"Step 60 rows must use interval=1 and disable heavy outputs: {row_name}")
    if config.geometry_type != "box" or config.geometry_config_path is not None:
        raise RuntimeError(f"Step 60 rows must use default box geometry: {row_name}")
    if config.quality_check_enabled or config.quality_check_strict:
        raise RuntimeError(f"Step 60 rows must keep quality gates disabled: {row_name}")
    if config.boundary_motion_mode != "static" or config.boundary_motion_config_path is not None:
        raise RuntimeError(f"Step 60 rows must keep boundary motion static: {row_name}")
    if config.wall_velocity_application_mode != "disabled" or config.wall_velocity_application_config_path is not None:
        raise RuntimeError(f"Step 60 rows must keep wall velocity disabled: {row_name}")
    if config.geometry_motion_mode != "static" or config.geometry_motion_application_mode != "disabled":
        raise RuntimeError(f"Step 60 rows must keep geometry motion disabled: {row_name}")


def duration_ramp_row_pass(row: dict, policy: dict) -> bool:
...
def duration_ramp_summary(rows: list[dict], matrix_policy: dict, acceptance_policy: dict) -> dict:
    ...
    summary = {
        ...
        "duration_ramp_matrix_pass": False,
    }
    summary["duration_ramp_matrix_pass"] = bool(
...
def reset_generated_run_dir(run_root: Path, out_dir: Path) -> None:
    ...
    if resolved_out_dir.parent != resolved_run_root:
        raise RuntimeError(f"refusing to reset unexpected Step 60 run directory: {out_dir}")
    if out_dir.exists():
...
def row_name_from_config(config_path: str) -> str:
    return Path(config_path).stem.removeprefix("step60_")


def expected_lbm_steps(row_name: str) -> int:
    if row_name.endswith("_3step"):
        return 3
    if row_name.endswith("_5step"):
        return 5
    if row_name.endswith("_1step_optional"):
        return 1
    raise RuntimeError(f"cannot infer expected Step 60 LBM steps from row name: {row_name}")
