...
def build_step109_output_guard(
    root: Path,
    policy_path: str = "configs/step109_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    rows = [output_row(root, path, policy) for path in step109_output_files(root)]
    ...
    summary = {
        "artifact_budget_pass": bool(total_size_mb < float(policy["max_step109_total_size_mb"]) and large_file_count == 0),
        ...
        "row_count": len(rows),
        "step109_forbidden_file_count": sum(1 for row in rows if row["forbidden"]),
        "step109_large_file_count": large_file_count,
        "step109_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step109_total_size_mb": total_size_mb,
        "step109_video_count": sum(1 for row in rows if row["extension"] in set(policy["video_extensions"])),
    ...
    summary["output_guard_pass"] = bool(
        ...
        and summary["step109_forbidden_file_count"] == 0
        and summary["step109_video_count"] == 0
...
def step109_output_files(root: Path) -> list[Path]:
    files = []
    for base in (root / "configs", root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith(("outputs/step109_output_guard/", "outputs/step109_artifact_manifest/")):
                continue
            if rel.startswith(("configs/step109_", "outputs/step109_", "logs/step109_")):
                files.append(path)
    return sorted(files)
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step109_output_files(root):
...
def pattern_rows(root: Path, patterns: list[str]) -> list[str]:
    ...
    for path in step109_output_files(root):
        if path.suffix.lower() not in {".json", ".csv", ".log", ".md"}:
            continue
        if path.name == "step109_output_guard_policy.json":
            continue
        text = path.read_text(encoding="utf-8-sig", errors="ignore")
