...
def build_step84_runtime_geometry_wall_velocity_combined_smoke_audit(
    root: Path,
    ...
    for row in payload["rows"]:
        rows.extend(
            [
                ...
                literal_row(row["row_name"], "squid_proxy_enabled", row["squid_proxy_enabled"], False),
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step84_runtime_geometry_wall_velocity_combined_smoke_audit_pass": False,
    }
    summary["step84_runtime_geometry_wall_velocity_combined_smoke_audit_pass"] = bool(
        rows
        and summary["pass_count"] == summary["row_count"]
        and payload["summary"]["step84_runtime_geometry_wall_velocity_combined_smoke_matrix_pass"] is True
    )
    return rows, summary
