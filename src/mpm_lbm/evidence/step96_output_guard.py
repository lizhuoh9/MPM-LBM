...
def build_step96_output_guard(
    root: Path,
    policy_path: str = "configs/step96_output_guard_policy.json",
    ...
    rows = [output_row(root, path, policy) for path in step96_output_files(root)]
    ...
    optional_dirs = step96_optional_driver_run_dirs(root, policy)
    ...
    artifact_budget_pass = bool(total_size_mb < float(policy["max_step96_total_size_mb"]) and large_file_count == 0)
    summary = {
        ...
        "row_count": len(rows),
        "step96_dense_displacement_output_count": sum(1 for row in rows if "dense_displacement" in row["path"].lower()),
        "step96_dense_wall_velocity_output_count": sum(1 for row in rows if "dense_wall_velocity" in row["path"].lower()),
        "step96_displaced_particle_output_count": sum(1 for row in rows if "displaced_particle" in row["path"].lower()),
        "step96_forbidden_file_count": len(forbidden_rows),
        "step96_ggui_screenshot_count": sum(1 for row in rows if row["extension"] in screenshot_extensions),
        "step96_ggui_video_count": sum(1 for row in rows if row["extension"] in video_extensions),
        "step96_large_file_count": large_file_count,
        "step96_optional_driver_run_dir_count": len(optional_dirs),
        "step96_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step96_raw_geometry_output_count": sum(1 for row in rows if row["extension"] in raw_geometry_extensions),
        "step96_real_geometry_candidate_output_count": sum(
            1 for row in rows if "real_geometry_candidate" in row["path"].lower()
        ),
        "step96_required_driver_run_dir_count": len(required_dirs),
        "step96_sparse_wall_velocity_output_count": sum(1 for row in rows if "sparse_wall_velocity" in row["path"].lower()),
        "step96_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step96_total_size_mb": total_size_mb,
        "step96_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        rows
        and summary["row_count"] == summary["pass_count"]
        and summary["step96_required_driver_run_dir_count"] == len(policy["required_driver_run_dirs"])
        and summary["step96_optional_driver_run_dir_count"] == 0
        ...
        and summary["step96_dense_wall_velocity_output_count"] == 0
        and summary["step96_sparse_wall_velocity_output_count"] == 0
        and summary["step96_dense_displacement_output_count"] == 0
        and summary["step96_displaced_particle_output_count"] == 0
        and summary["step96_raw_geometry_output_count"] == 0
        and summary["step96_real_geometry_candidate_output_count"] == 0
        and summary["step96_forbidden_file_count"] == 0
...
def step96_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step96_") or rel.startswith("logs/step96_"):
                files.append(path)
    return sorted(files)
...
def step96_optional_driver_run_dirs(root: Path, policy: dict) -> list[Path]:
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step96_output_files(root):
