...
def build_step88_squid_proxy_runtime_geometry_wall_velocity_combined_smoke_matrix(
    root: Path,
    matrix_policy_path: str = "configs/step88_squid_proxy_runtime_geometry_wall_velocity_combined_smoke_matrix.json",
    acceptance_policy_path: str = "configs/step88_squid_proxy_runtime_geometry_wall_velocity_combined_acceptance_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    matrix_policy = read_json(root / matrix_policy_path)
    acceptance_policy = read_json(root / acceptance_policy_path)
    enforce_step88_policy(matrix_policy)
    rows = [
        run_step88_squid_proxy_runtime_geometry_wall_velocity_combined_case(
            root, config_path, matrix_policy, acceptance_policy
        )
        for config_path in matrix_policy["required_smoke_configs"]
    ]
    summary = step88_squid_proxy_runtime_geometry_wall_velocity_combined_summary(
        rows, matrix_policy, acceptance_policy
    )
    return rows, summary


def run_step88_squid_proxy_runtime_geometry_wall_velocity_combined_case(
    root: Path,
    config_path: str,
    matrix_policy: dict,
    ...
    enforce_step88_config(root, config, row_name, acceptance_policy)
    ...
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step88 combined smoke row: {row_name}")
    ...
    row = {
        ...
        **{field: bool(geometry_summary.get(field, True)) for field in MUTATION_FLAG_FIELDS},
        ...
        "stable": False,
        "notes": (
            "squid_proxy plus runtime geometry diagnostic-only plus wall velocity solid_vel "
            "canonical driver 3-step smoke completed"
        ),
    }
    row["stable"] = step88_squid_proxy_runtime_geometry_wall_velocity_combined_row_pass(row, acceptance_policy)
    if not row["stable"]:
        raise RuntimeError(f"Step88 combined smoke row failed acceptance: {row}")
    return row


def enforce_step88_policy(matrix_policy: dict) -> None:
    expected = {
        "post_gate_simulation_allowed": True,
        "allowed_next_step": "Step88",
        "allowed_next_step_scope": (
            "squid_proxy plus runtime geometry diagnostic-only plus wall velocity solid_vel "
            "combined canonical driver 3-step smoke only"
        ),
        "gate_source_step": "Step87",
        "previous_step": "Step87",
    ...
    if actual != expected:
        raise RuntimeError(f"Step88 policy mismatch: {actual} != {expected}")
    if matrix_policy.get("required_smoke_configs") != [
        ...
        raise RuntimeError("Step88 must run exactly one combined squid_proxy/runtime-geometry/wall-velocity config")


def enforce_step88_config(root: Path, config, row_name: str, policy: dict) -> None:
    if row_name not in set(policy["required_row_names"]):
        raise RuntimeError(f"unexpected Step88 combined smoke row: {row_name}")
    expected = {
    ...
    if actual != expected:
        raise RuntimeError(f"Step88 config mismatch for {row_name}: {actual} != {expected}")
    if config.coupling_mode != "moving_boundary" or config.reaction_transfer_mode != "engineering":
        raise RuntimeError(f"Step88 row must use moving_boundary engineering transfer: {row_name}")
    if list(config.target_u_lbm) != list(policy["required_target_u_lbm"]):
        raise RuntimeError(f"Step88 row must use row-local zero background target_u_lbm: {row_name}")
    if config.geometry_type != policy["required_geometry_type"]:
        raise RuntimeError(f"Step88 row must use squid_proxy geometry: {row_name}")
    if config.geometry_config_path != policy["required_geometry_config_path"]:
        raise RuntimeError(f"Step88 row must use the Step85 squid_proxy geometry config: {row_name}")
    if not path_exists_from_repo_root(root, config.geometry_config_path):
        raise RuntimeError(f"Step88 geometry config path is not resolvable: {config.geometry_config_path}")
    if config.output_interval != 1 or config.write_vtk or config.write_particles:
        raise RuntimeError(f"Step88 row must use interval=1 and disable heavy outputs: {row_name}")
    if not config.quality_check_enabled or config.quality_check_strict:
        raise RuntimeError(f"Step88 row must enable non-strict geometry quality reporting: {row_name}")
    if config.boundary_motion_mode != "prescribed_kinematic" or not config.boundary_motion_report_enabled:
        raise RuntimeError(f"Step88 row must write prescribed_kinematic boundary motion report: {row_name}")
    if config.geometry_motion_mode != "prescribed_kinematic":
        raise RuntimeError(f"Step88 row must use prescribed_kinematic geometry motion: {row_name}")
    if config.geometry_motion_application_mode != "diagnostic_only":
        raise RuntimeError(f"Step88 row must use diagnostic_only geometry motion application: {row_name}")
    if not config.geometry_motion_report_enabled or not config.geometry_motion_application_report_enabled:
        raise RuntimeError(f"Step88 row must write geometry motion reports: {row_name}")
    if config.wall_velocity_application_mode != "solid_vel_experimental":
        raise RuntimeError(f"Step88 row must use solid_vel_experimental wall velocity: {row_name}")
    if not config.wall_velocity_application_report_enabled:
        raise RuntimeError(f"Step88 row must write wall velocity application report: {row_name}")
    for label, relative_path in {
        ...
        if not path_exists_from_repo_root(root, relative_path):
            raise RuntimeError(f"Step88 {label} is not resolvable: {relative_path}")
...
def step88_squid_proxy_runtime_geometry_wall_velocity_combined_row_pass(row: dict, policy: dict) -> bool:
    ...
    return bool(
        ...
        and row["allowed_next_step"] == "Step88"
        and row["allowed_next_step_scope"]
        == "squid_proxy plus runtime geometry diagnostic-only plus wall velocity solid_vel combined canonical driver 3-step smoke only"
        and row["previous_step"] == "Step87"
...
def step88_squid_proxy_runtime_geometry_wall_velocity_combined_summary(
    ...
    summary = {
        ...
        "step88_squid_proxy_runtime_geometry_wall_velocity_combined_smoke_matrix_pass": False,
    ...
    summary["step88_squid_proxy_runtime_geometry_wall_velocity_combined_smoke_matrix_pass"] = bool(
        summary["post_gate_simulation_allowed"] is True
        and summary["allowed_next_step"] == "Step88"
        and summary["allowed_next_step_scope"]
        == "squid_proxy plus runtime geometry diagnostic-only plus wall velocity solid_vel combined canonical driver 3-step smoke only"
        and summary["previous_step"] == "Step87"
...
def reset_generated_run_dir(run_root: Path, out_dir: Path) -> None:
    ...
    if resolved_out_dir.parent != resolved_run_root:
        raise RuntimeError(f"refusing to reset unexpected Step88 run directory: {out_dir}")
    if out_dir.exists():
...
def row_name_from_config(config_path: str) -> str:
    return Path(config_path).stem.removeprefix("step88_")
