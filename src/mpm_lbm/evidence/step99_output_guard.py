...
def build_step99_output_guard(
    root: Path,
    policy_path: str = "configs/step99_output_guard_policy.json",
    ...
    rows = [output_row(root, path, policy) for path in step99_output_files(root)]
    ...
    driver_run_dir_count = step99_driver_run_dir_count(root)
    ...
    artifact_budget_pass = bool(total_size_mb < float(policy["max_step99_total_size_mb"]) and large_file_count == 0)
    summary = {
        ...
        "row_count": len(rows),
        "step99_dense_displacement_output_count": sum(1 for row in rows if "dense_displacement" in row["path"].lower()),
        "step99_dense_wall_velocity_output_count": sum(1 for row in rows if "dense_wall_velocity" in row["path"].lower()),
        "step99_displaced_particle_output_count": sum(1 for row in rows if "displaced_particle" in row["path"].lower()),
        "step99_driver_run_dir_count": driver_run_dir_count,
        "step99_forbidden_file_count": len(forbidden_rows),
        "step99_ggui_screenshot_count": sum(1 for row in rows if row["extension"] in screenshot_extensions),
        "step99_ggui_video_count": sum(1 for row in rows if row["extension"] in video_extensions),
        "step99_large_file_count": large_file_count,
        "step99_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step99_raw_geometry_output_count": sum(1 for row in rows if row["extension"] in raw_geometry_extensions),
        "step99_real_geometry_candidate_output_count": sum(
            1 for row in rows if "real_geometry_candidate" in row["path"].lower()
        ),
        "step99_sparse_wall_velocity_output_count": sum(1 for row in rows if "sparse_wall_velocity" in row["path"].lower()),
        "step99_total_size_bytes": total_size_bytes,
        "step99_total_size_mb": total_size_mb,
        "step99_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        ...
        and summary["step99_dense_wall_velocity_output_count"] == 0
        and summary["step99_sparse_wall_velocity_output_count"] == 0
        and summary["step99_dense_displacement_output_count"] == 0
        and summary["step99_displaced_particle_output_count"] == 0
        and summary["step99_raw_geometry_output_count"] == 0
        and summary["step99_real_geometry_candidate_output_count"] == 0
        and summary["step99_forbidden_file_count"] == 0
...
def step99_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step99_") or rel.startswith("logs/step99_"):
                files.append(path)
    return sorted(files)
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step99_output_files(root):
