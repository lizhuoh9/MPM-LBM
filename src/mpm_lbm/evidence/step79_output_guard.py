...
def build_step79_output_guard(
    root: Path,
    policy_path: str = "configs/step79_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    rows = [output_row(root, path, policy) for path in step79_output_files(root)]
    forbidden_rows = [row for row in rows if row["forbidden"]]
    driver_run_dirs = step79_driver_run_dirs(root, policy)
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step79_driver_run_dir_count": len(driver_run_dirs),
        "step79_forbidden_file_count": len(forbidden_rows),
        "step79_large_file_count": sum(1 for row in rows if int(row["size_bytes"]) > int(policy["large_file_threshold_bytes"])),
        "step79_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step79_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step79_total_size_mb": sum(int(row["size_bytes"]) for row in rows) / (1024.0 * 1024.0),
        "step79_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        ...
        and summary["step79_large_file_count"] == 0
        and summary["step79_forbidden_file_count"] == 0
...
def step79_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step79_") or rel.startswith("logs/step79_"):
                files.append(path)
    return sorted(files)


def step79_driver_run_dirs(root: Path, policy: dict) -> list[Path]:
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step79_output_files(root):
