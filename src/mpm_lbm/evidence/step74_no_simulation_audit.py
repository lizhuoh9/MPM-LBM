...
from src.mpm_lbm.evidence.real_geometry_output_policy_audit import step74_related_files


def build_step74_no_simulation_audit(
    root: Path,
    policy_path: str = "configs/step74_no_simulation_policy.json",
    ...
    rows.extend(absence_row("no_protected_step74_file", path) for path in protected_files)
    summary = {
        ...
        "protected_step74_file_count": len(protected_files),
        "row_count": len(rows),
        "step74_particle_npy_count": len(particle_npy_files),
    ...
    summary["no_simulation_audit_pass"] = bool(
        ...
        and summary["protected_step74_file_count"] == 0
    )
    return rows, summary
...
def python_call_scan_row(root: Path, relative_path: str) -> dict:
    ...
    return {
        ...
        "found_calls": sorted(found_calls),
...
def forbidden_calls(tree: ast.AST) -> list[str]:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.Call):
            continue
        func = node.func
        if isinstance(func, ast.Name) and func.id == "run_candidate_projection_smoke":
            found.append("run_candidate_projection_smoke")
        if isinstance(func, ast.Attribute):
            ...
            if func.attr == "run_candidate_projection_smoke":
                found.append("run_candidate_projection_smoke")
            if func.attr == "run":
