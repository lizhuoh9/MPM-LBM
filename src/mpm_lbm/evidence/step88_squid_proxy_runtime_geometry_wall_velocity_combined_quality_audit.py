...
def build_step88_squid_proxy_runtime_geometry_wall_velocity_combined_quality_audit(
    root: Path,
    ...
    summary = {
        ...
        "step88_squid_proxy_runtime_geometry_wall_velocity_combined_quality_pass": False,
        "wall_velocity_application_report_pass_count": sum(
            1 for row in payload["rows"] if row["wall_velocity_application_report_pass"]
        ),
    }
    summary["step88_squid_proxy_runtime_geometry_wall_velocity_combined_quality_pass"] = bool(
        rows
...
def quality_rows(matrix_row: dict, policy: dict) -> list[dict]:
    ...
    rows.extend(
        [
            ...
            literal_row(matrix_row, "previous_step", "Step87"),
