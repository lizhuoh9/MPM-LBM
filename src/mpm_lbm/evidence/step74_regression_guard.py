...
def build_step74_step73_regression_guard(
    root: Path,
    policy_path: str = "configs/step74_step73_regression_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    artifact_check_names = {check["check"] for check in policy["step73_artifact_checks"]}
    rows = [artifact_row(root, check) for check in policy["step73_artifact_checks"]]
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step73_artifact_check_count": len(policy["step73_artifact_checks"]),
        "step73_artifact_pass_count": sum(1 for row in rows if row["check"] in artifact_check_names and row["pass"]),
        "step74_step73_regression_guard_pass": False,
    }
    summary["step74_step73_regression_guard_pass"] = bool(
        rows
        and summary["pass_count"] == summary["row_count"]
        and summary["step73_artifact_pass_count"] == summary["step73_artifact_check_count"]
