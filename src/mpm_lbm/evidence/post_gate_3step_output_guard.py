...
def build_step77_post_gate_output_guard(
    root: Path,
    policy_path: str = "configs/step77_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    rows = [output_row(root, path, policy) for path in step77_output_files(root)]
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step77_forbidden_file_count": len(forbidden_rows),
        "step77_large_file_count": sum(1 for row in rows if int(row["size_bytes"]) > int(policy["large_file_threshold_bytes"])),
        "step77_optional_driver_run_dir_count": len(optional_dirs),
        "step77_particle_npy_count": sum(1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()),
        "step77_required_driver_run_dir_count": len(required_dirs),
        "step77_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step77_total_size_mb": sum(int(row["size_bytes"]) for row in rows) / (1024.0 * 1024.0),
        "step77_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        rows
        and summary["row_count"] == summary["pass_count"]
        and summary["step77_required_driver_run_dir_count"] == len(policy["required_driver_run_dirs"])
        and summary["step77_optional_driver_run_dir_count"] == 0
        and summary["step77_vtr_count"] == 0
        and summary["step77_particle_npy_count"] == 0
        and summary["step77_large_file_count"] == 0
        and summary["step77_forbidden_file_count"] == 0
        and summary["step77_total_size_mb"] < float(policy["max_step77_total_size_mb"])
...
def step77_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step77_") or rel.startswith("logs/step77_"):
                files.append(path)
    return sorted(files)
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step77_output_files(root):
