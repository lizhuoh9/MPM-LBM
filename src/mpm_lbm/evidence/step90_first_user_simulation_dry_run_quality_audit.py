...
def build_step90_first_user_simulation_dry_run_quality_audit(
    root: Path,
    ...
    summary = {
        ...
        "step90_first_user_simulation_dry_run_quality_pass": False,
        "wall_velocity_application_report_pass_count": sum(
            1 for row in payload["rows"] if row["wall_velocity_application_report_pass"]
        ),
    }
    summary["step90_first_user_simulation_dry_run_quality_pass"] = bool(
        rows
...
def quality_rows(matrix_row: dict, policy: dict) -> list[dict]:
    ...
    rows.extend(
        [
            ...
            literal_row(matrix_row, "previous_step", "Step89"),
