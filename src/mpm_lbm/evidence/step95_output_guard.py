...
def build_step95_output_guard(
    root: Path,
    policy_path: str = "configs/step95_output_guard_policy.json",
    ...
    rows = [output_row(root, path, policy) for path in step95_output_files(root)]
    ...
    driver_run_dir_count = step95_driver_run_dir_count(root)
    ...
    artifact_budget_pass = bool(total_size_mb < float(policy["max_step95_total_size_mb"]) and large_file_count == 0)
    summary = {
        ...
        "row_count": len(rows),
        "step95_dense_displacement_output_count": sum(1 for row in rows if "dense_displacement" in row["path"].lower()),
        "step95_dense_wall_velocity_output_count": sum(1 for row in rows if "dense_wall_velocity" in row["path"].lower()),
        "step95_displaced_particle_output_count": sum(1 for row in rows if "displaced_particle" in row["path"].lower()),
        "step95_driver_run_dir_count": driver_run_dir_count,
        "step95_forbidden_file_count": len(forbidden_rows),
        "step95_ggui_screenshot_count": sum(1 for row in rows if row["extension"] in screenshot_extensions),
        "step95_ggui_video_count": sum(1 for row in rows if row["extension"] in video_extensions),
        "step95_large_file_count": large_file_count,
        "step95_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step95_raw_geometry_output_count": sum(1 for row in rows if row["extension"] in raw_geometry_extensions),
        "step95_real_geometry_candidate_output_count": sum(
            1 for row in rows if "real_geometry_candidate" in row["path"].lower()
        ),
        "step95_sparse_wall_velocity_output_count": sum(1 for row in rows if "sparse_wall_velocity" in row["path"].lower()),
        "step95_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step95_total_size_mb": total_size_mb,
        "step95_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        ...
        and summary["step95_dense_wall_velocity_output_count"] == 0
        and summary["step95_sparse_wall_velocity_output_count"] == 0
        and summary["step95_dense_displacement_output_count"] == 0
        and summary["step95_displaced_particle_output_count"] == 0
        and summary["step95_raw_geometry_output_count"] == 0
        and summary["step95_real_geometry_candidate_output_count"] == 0
        and summary["step95_forbidden_file_count"] == 0
...
def step95_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step95_") or rel.startswith("logs/step95_"):
                files.append(path)
    return sorted(files)
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step95_output_files(root):
