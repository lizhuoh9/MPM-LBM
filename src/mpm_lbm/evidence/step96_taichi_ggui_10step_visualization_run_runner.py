...
def build_step96_taichi_ggui_10step_visualization_run_matrix(
    root: Path,
    matrix_policy_path: str = "configs/step96_taichi_ggui_10step_visualization_run_matrix.json",
    ...
    enforce_step96_policy(matrix_policy)
    rows = [
        run_step96_taichi_ggui_10step_visualization_case(root, config_path, matrix_policy, acceptance_policy)
        for config_path in matrix_policy["required_run_configs"]
    ]
    summary = step96_taichi_ggui_10step_visualization_summary(rows, matrix_policy, acceptance_policy)
    return rows, summary


def run_step96_taichi_ggui_10step_visualization_case(
    root: Path,
    config_path: str,
    matrix_policy: dict,
    ...
    enforce_step96_config(root, config, row_name, acceptance_policy)
    ...
    reset_step96_output_dir(root, ggui_output_dir, root / "outputs")
    ...
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step96 Taichi GGUI 10-step visualization row: {row_name}")

    ggui_report = render_step96_ggui_10step_visualization(
    ...
    row = {
        ...
        "previous_step94_n_lbm_steps": int(matrix_policy["previous_step94_n_lbm_steps"]),
        "step96_n_lbm_steps": int(config.n_lbm_steps),
        "from_step94_duration_expansion": bool(matrix_policy["from_step94_duration_expansion"]),
        "from_step92_adds_ggui_visualization": bool(matrix_policy["from_step92_adds_ggui_visualization"]),
        ...
        **feature_flags,
        ...
        "ggui_render_report_exists": (ggui_output_dir / "step96_ggui_visualization_report.json").is_file(),
        ...
        **{field: bool(geometry_summary.get(field, True)) for field in MUTATION_FLAG_FIELDS},
        ...
        "stable": False,
        "notes": (
            "Step96 renders the accepted 32^3 10-step first-user squid_proxy runtime geometry "
            "diagnostic-only wall velocity solid_vel envelope through Taichi GGUI."
        ),
    }
    row["stable"] = step96_taichi_ggui_10step_visualization_row_pass(row, acceptance_policy)
    if not row["stable"]:
        raise RuntimeError(f"Step96 Taichi GGUI 10-step visualization row failed acceptance: {row}")
    return row


def render_step96_ggui_10step_visualization(
    driver_run_dir,
    ggui_config_path,
    output_dir,
    root: Path | None = None,
    ...
    report["visualization_note"] = (
        "GGUI visualizes Step96 procedural squid_proxy proxy points after the accepted 10-step dry-run envelope; "
        "it is not a physical validation or swimming claim."
    )
    metadata = {
        "driver_run_dir": rel_path(root, resolve_under_root(root, driver_run_dir)),
        "ggui_config_path": rel_path(root, ggui_config_path),
        "render_report_path": "outputs/step96_ggui_visualization/step96_ggui_visualization_report.json",
        "screenshot_path": rel_path(root, screenshot_path),
        "visualization_source": report["visualization_source"],
    }
    write_json(output_dir / "step96_ggui_visualization_metadata.json", metadata)
    write_json(output_dir / "step96_ggui_visualization_report.json", report)
    return report


def enforce_step96_policy(matrix_policy: dict) -> None:
    expected = {
        "post_gate_simulation_allowed": True,
        "allowed_next_step": "Step96",
        "allowed_next_step_scope": (
            "Taichi GGUI visualization can render the accepted 32^3 / 10-step first-user dry-run envelope"
        ),
        "gate_source_step": "Step95",
        "previous_step": "Step95",
        "driver_run_required": True,
        "runtime_code_changed": False,
        "solver_behavior_changed": False,
        "physics_feature_expansion": "taichi_ggui_10step_first_user_visualization_run",
    ...
    if actual != expected:
        raise RuntimeError(f"Step96 policy mismatch: {actual} != {expected}")
    if matrix_policy.get("required_run_configs") != [
        ...
        raise RuntimeError("Step96 must run exactly one Taichi GGUI 10-step visualization config")


def enforce_step96_config(root: Path, config, row_name: str, policy: dict) -> None:
    if row_name not in set(policy["required_row_names"]):
        raise RuntimeError(f"unexpected Step96 Taichi GGUI 10-step visualization row: {row_name}")
    expected = {
    ...
    if actual != expected:
        raise RuntimeError(f"Step96 config mismatch for {row_name}: {actual} != {expected}")
    if config.coupling_mode != "moving_boundary" or config.reaction_transfer_mode != "engineering":
        raise RuntimeError(f"Step96 row must use moving_boundary engineering transfer: {row_name}")
    if list(config.target_u_lbm) != list(policy["required_target_u_lbm"]):
        raise RuntimeError(f"Step96 row must use row-local zero background target_u_lbm: {row_name}")
    if config.geometry_type != policy["required_geometry_type"]:
        raise RuntimeError(f"Step96 row must use squid_proxy geometry: {row_name}")
    if config.geometry_config_path != policy["required_geometry_config_path"]:
        raise RuntimeError(f"Step96 row must use the Step85 squid_proxy geometry config: {row_name}")
    if config.output_interval != 1 or config.write_vtk or config.write_particles:
        raise RuntimeError(f"Step96 row must use interval=1 and disable heavy outputs: {row_name}")
    if not config.quality_check_enabled or config.quality_check_strict:
        raise RuntimeError(f"Step96 row must enable non-strict geometry quality reporting: {row_name}")
    if config.boundary_motion_mode != "prescribed_kinematic" or not config.boundary_motion_report_enabled:
        raise RuntimeError(f"Step96 row must write prescribed_kinematic boundary motion report: {row_name}")
    if config.geometry_motion_mode != "prescribed_kinematic":
        raise RuntimeError(f"Step96 row must use prescribed_kinematic geometry motion: {row_name}")
    if config.geometry_motion_application_mode != "diagnostic_only":
        raise RuntimeError(f"Step96 row must use diagnostic_only geometry motion application: {row_name}")
    if not config.geometry_motion_report_enabled or not config.geometry_motion_application_report_enabled:
        raise RuntimeError(f"Step96 row must write geometry motion reports: {row_name}")
    if config.wall_velocity_application_mode != "solid_vel_experimental":
        raise RuntimeError(f"Step96 row must use solid_vel_experimental wall velocity: {row_name}")
    if not config.wall_velocity_application_report_enabled:
        raise RuntimeError(f"Step96 row must write wall velocity application report: {row_name}")
    for label, relative_path in {
        ...
        if not path_exists_from_repo_root(root, relative_path):
            raise RuntimeError(f"Step96 {label} is not resolvable: {relative_path}")


def step96_taichi_ggui_10step_visualization_row_pass(row: dict, policy: dict) -> bool:
    ...
    return bool(
        ...
        and row["allowed_next_step"] == "Step96"
        and row["previous_step"] == "Step95"
        and row["from_step94_duration_expansion"]
        and row["from_step92_adds_ggui_visualization"]
        and row["physics_feature_expansion"] == "taichi_ggui_10step_first_user_visualization_run"
        and row["only_new_feature_from_step92"] == "taichi_ggui_visualization"
        and int(row["previous_step92_n_lbm_steps"]) == 10
        and int(row["previous_step94_n_lbm_steps"]) == 1
        and int(row["step96_n_lbm_steps"]) == 10
...
def step96_taichi_ggui_10step_visualization_summary(
    ...
    summary = {
        ...
        "previous_step94_n_lbm_steps": min(int(row["previous_step94_n_lbm_steps"]) for row in rows),
        ...
        "step96_n_lbm_steps": min(int(row["step96_n_lbm_steps"]) for row in rows),
        "step96_taichi_ggui_10step_visualization_run_matrix_pass": False,
    ...
    summary["step96_taichi_ggui_10step_visualization_run_matrix_pass"] = bool(
        summary["post_gate_simulation_allowed"] is True
        and summary["allowed_next_step"] == "Step96"
        and summary["previous_step"] == "Step95"
        ...
        and summary["min_completed_lbm_steps"] == 10
        and summary["min_diagnostics_row_count"] >= 11
        and summary["min_total_mpm_substeps"] >= 10
        and summary["previous_step92_n_lbm_steps"] == 10
        and summary["previous_step94_n_lbm_steps"] == 1
        and summary["step96_n_lbm_steps"] == 10
        and summary["runtime_hard_fail_count"] == 0
        and not summary["runtime_code_changed"]
        and not summary["solver_behavior_changed"]
        and summary["physics_feature_expansion"] == "taichi_ggui_10step_first_user_visualization_run"
    )
    return summary


def reset_step96_output_dir(root: Path, out_dir: Path, required_parent: Path) -> None:
    import shutil
    ...
    if resolved_parent not in resolved_out.parents:
        raise RuntimeError(f"refusing to reset unexpected Step96 output directory: {out_dir}")
    if out_dir.exists():
...
def row_name_from_config(config_path: str) -> str:
    return Path(config_path).stem.removeprefix("step96_")
