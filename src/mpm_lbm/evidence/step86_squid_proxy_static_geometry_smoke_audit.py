...
def build_step86_squid_proxy_static_geometry_smoke_audit(
    root: Path,
    ...
    for row in payload["rows"]:
        rows.extend(
            [
                ...
                literal_row(row["row_name"], "canonical_driver_module", row["canonical_driver_module"], "src.mpm_lbm.sim.drivers.fsi_driver"),
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step86_squid_proxy_static_geometry_smoke_audit_pass": False,
    }
    summary["step86_squid_proxy_static_geometry_smoke_audit_pass"] = bool(
        rows
        and summary["pass_count"] == summary["row_count"]
        and payload["summary"]["step86_squid_proxy_static_geometry_smoke_matrix_pass"] is True
    )
    return rows, summary
