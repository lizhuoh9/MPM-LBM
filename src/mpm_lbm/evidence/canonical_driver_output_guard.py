...
def build_canonical_driver_output_guard(
    root: Path,
    acceptance_policy_path: str = "configs/step59_smoke_acceptance_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / acceptance_policy_path)
    run_root = root / "outputs" / "step59_driver_runs"
    rows = [output_row(root, path, run_root, policy) for path in step59_output_files(root)]
    ...
    summary = {
        ...
        "step59_driver_run_dir_count": len(run_dirs),
        "step59_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
        "step59_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step59_large_file_count": sum(1 for row in rows if int(row["size_bytes"]) > LARGE_FILE_THRESHOLD_BYTES),
        "step59_forbidden_file_count": len(forbidden_rows),
        "step59_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step59_total_size_mb": sum(int(row["size_bytes"]) for row in rows) / (1024.0 * 1024.0),
    ...
    summary["output_guard_pass"] = bool(
        summary["step59_driver_run_dir_count"] == int(policy["expected_driver_run_dir_count"])
        and summary["step59_vtr_count"] == 0
        and summary["step59_particle_npy_count"] == 0
        and summary["step59_large_file_count"] == 0
        and summary["step59_forbidden_file_count"] == 0
        and float(summary["step59_total_size_mb"]) < float(policy["max_step59_total_size_mb"])
...
def step59_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step59_") or rel.startswith("logs/step59_"):
                files.append(path)
    return sorted(files)
