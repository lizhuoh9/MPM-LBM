...
def build_canonical_driver_duration_ramp_output_guard(
    root: Path,
    output_policy_path: str = "configs/step60_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / output_policy_path)
    run_root = root / "outputs" / "step60_driver_runs"
    rows = [output_row(root, path, run_root, policy) for path in step60_output_files(root)]
    ...
    summary = {
        ...
        "step60_driver_run_dir_count": len(run_dirs),
        "step60_required_driver_run_dir_count": sum(1 for path in run_dirs if not path.name.endswith("_optional")),
        "step60_optional_driver_run_dir_count": sum(1 for path in run_dirs if path.name.endswith("_optional")),
        "step60_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
        "step60_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step60_large_file_count": sum(1 for row in rows if int(row["size_bytes"]) > LARGE_FILE_THRESHOLD_BYTES),
        "step60_forbidden_file_count": len(forbidden_rows),
        "step60_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step60_total_size_mb": sum(int(row["size_bytes"]) for row in rows) / (1024.0 * 1024.0),
    ...
    summary["output_guard_pass"] = bool(
        summary["step60_required_driver_run_dir_count"] == int(policy["expected_required_driver_run_dir_count"])
        and summary["step60_vtr_count"] == 0
        and summary["step60_particle_npy_count"] == 0
        and summary["step60_large_file_count"] == 0
        and summary["step60_forbidden_file_count"] == 0
        and float(summary["step60_total_size_mb"]) < float(policy["max_step60_total_size_mb"])
...
def step60_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step60_") or rel.startswith("logs/step60_"):
                files.append(path)
    return sorted(files)
