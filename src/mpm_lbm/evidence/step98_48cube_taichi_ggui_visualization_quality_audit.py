...
def build_step98_48cube_taichi_ggui_visualization_quality_audit(
    root: Path,
    ...
    summary = {
        ...
        "from_step96_duration_reduction_count": sum(1 for row in matrix_rows if row["from_step96_duration_reduction"]),
        "from_step96_grid_expansion_count": sum(1 for row in matrix_rows if row["from_step96_grid_expansion"]),
        ...
        "step98_48cube_taichi_ggui_visualization_quality_pass": False,
        "wall_velocity_application_report_pass_count": sum(
            1 for row in matrix_rows if row["wall_velocity_application_report_pass"]
        ),
    }
    summary["step98_48cube_taichi_ggui_visualization_quality_pass"] = bool(
        rows
        ...
        and summary["from_step96_grid_expansion_count"] == 1
        and summary["from_step96_duration_reduction_count"] == 1
...
def quality_rows(matrix_row: dict) -> list[dict]:
    return [
        ...
        literal_row(matrix_row, "n_lbm_steps", 1),
        literal_row(matrix_row, "completed_lbm_steps", 1),
        literal_row(matrix_row, "from_step96_grid_expansion", True),
        literal_row(matrix_row, "previous_step96_n_grid", 32),
        literal_row(matrix_row, "from_step96_duration_reduction", True),
        literal_row(matrix_row, "previous_step96_n_lbm_steps", 10),
        literal_row(matrix_row, "grid_expansion_smoke_isolation", True),
