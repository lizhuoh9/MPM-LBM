...
def build_step94_taichi_ggui_visualization_smoke_audit(
    root: Path,
    ...
    for row in payload["rows"]:
        rows.extend(
            [
                ...
                literal_row(row["row_name"], "completed_lbm_steps", row["completed_lbm_steps"], 1),
                literal_row(row["row_name"], "previous_step92_n_lbm_steps", row["previous_step92_n_lbm_steps"], 10),
                literal_row(row["row_name"], "step94_n_lbm_steps", row["step94_n_lbm_steps"], 1),
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step94_taichi_ggui_visualization_smoke_audit_pass": False,
    }
    summary["step94_taichi_ggui_visualization_smoke_audit_pass"] = bool(
        rows
        and summary["pass_count"] == summary["row_count"]
        and payload["summary"]["step94_taichi_ggui_visualization_smoke_matrix_pass"] is True
    )
    return rows, summary
