...
def build_step93_output_guard(
    root: Path,
    policy_path: str = "configs/step93_output_guard_policy.json",
    ...
    driver_run_dir_count = step93_driver_run_dir_count(root)
    ...
    artifact_budget_pass = bool(total_size_mb < float(policy["max_step93_total_size_mb"]) and large_file_count == 0)
    summary = {
        ...
        "row_count": len(rows),
        "step93_dense_displacement_output_count": sum(1 for row in rows if "dense_displacement" in row["path"].lower()),
        "step93_dense_wall_velocity_output_count": sum(1 for row in rows if "dense_wall_velocity" in row["path"].lower()),
        ...
        "step93_raw_geometry_output_count": sum(1 for row in rows if row["extension"] in raw_geometry_extensions),
        "step93_real_geometry_candidate_output_count": sum(
    ...
    summary["output_guard_pass"] = bool(
        ...
        and summary["step93_displaced_particle_output_count"] == 0
        and summary["step93_raw_geometry_output_count"] == 0
        and summary["step93_real_geometry_candidate_output_count"] == 0
        and summary["step93_forbidden_file_count"] == 0
...
def step93_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step93_") or rel.startswith("logs/step93_"):
                files.append(path)
    return sorted(files)
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step93_output_files(root):
