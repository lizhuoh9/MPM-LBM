...
from src.mpm_lbm.evidence.step108_common import read_json


PRIVATE_ABSOLUTE_RE = re.compile(r"([A-Za-z]:[\\/]+Users[\\/]+|D:[\\/]+working[\\/]+)")


def build_step108_output_guard(
    root: Path,
    policy_path: str = "configs/step108_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    rows = [output_row(root, path, policy) for path in step108_output_files(root)]
    ...
    summary = {
        "artifact_budget_pass": bool(total_size_mb < float(policy["max_step108_total_size_mb"]) and large_file_count == 0),
        ...
        "row_count": len(rows),
        "step108_forbidden_file_count": sum(1 for row in rows if row["forbidden"]),
        "step108_large_file_count": large_file_count,
        "step108_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step108_total_size_mb": total_size_mb,
        "step108_video_count": sum(1 for row in rows if row["extension"] in set(policy["video_extensions"])),
    ...
    summary["output_guard_pass"] = bool(
        ...
        and summary["step108_forbidden_file_count"] == 0
        and summary["step108_video_count"] == 0
...
def step108_output_files(root: Path) -> list[Path]:
    files = []
    for base in (root / "configs", root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith(("outputs/step108_output_guard/", "outputs/step108_artifact_manifest/")):
                continue
            if rel.startswith(("configs/step108_", "outputs/step108_", "logs/step108_")):
                files.append(path)
    return sorted(files)
...
def is_case_data_file(row: dict, policy: dict) -> bool:
    ...
    official_names = set(name.lower() for name in policy["official_file_names"])
    name = Path(lower).name
    return name in official_names or lower.endswith((".cas.h5", ".dat.h5"))
...
def is_mesh_file(row: dict, policy: dict) -> bool:
    lower = row["path"].lower()
    names = set(name.lower() for name in policy["official_file_names"])
    return Path(lower).name in names or lower.endswith((".msh", ".msh.gz", ".cas", ".cas.h5"))


def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step108_output_files(root):
...
def pattern_rows(root: Path, patterns: list[str]) -> list[str]:
    ...
    for path in step108_output_files(root):
        if path.suffix.lower() not in {".json", ".csv", ".log", ".md"}:
            continue
        if path.name == "step108_output_guard_policy.json":
            continue
        text = path.read_text(encoding="utf-8-sig", errors="ignore")
