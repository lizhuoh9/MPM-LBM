...
def build_step96_taichi_ggui_10step_visualization_run_audit(
    root: Path,
    ...
    for row in payload["rows"]:
        rows.extend(
            [
                ...
                literal_row(row["row_name"], "completed_lbm_steps", row["completed_lbm_steps"], 10),
                literal_row(row["row_name"], "previous_step92_n_lbm_steps", row["previous_step92_n_lbm_steps"], 10),
                literal_row(row["row_name"], "previous_step94_n_lbm_steps", row["previous_step94_n_lbm_steps"], 1),
                literal_row(row["row_name"], "step96_n_lbm_steps", row["step96_n_lbm_steps"], 10),
                literal_row(row["row_name"], "from_step94_duration_expansion", row["from_step94_duration_expansion"], True),
                literal_row(row["row_name"], "from_step92_adds_ggui_visualization", row["from_step92_adds_ggui_visualization"], True),
                ...
                literal_row(row["row_name"], "ggui_visualization_enabled", row["ggui_visualization_enabled"], True),
                literal_row(row["row_name"], "ggui_renderer_called", row["ggui_renderer_called"], True),
                literal_row(row["row_name"], "ggui_screenshot_exists", row["ggui_screenshot_exists"], True),
                literal_row(row["row_name"], "ggui_video_enabled", row["ggui_video_enabled"], False),
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step96_taichi_ggui_10step_visualization_run_audit_pass": False,
    }
    summary["step96_taichi_ggui_10step_visualization_run_audit_pass"] = bool(
        rows
        and summary["pass_count"] == summary["row_count"]
        and payload["summary"]["step96_taichi_ggui_10step_visualization_run_matrix_pass"] is True
    )
    return rows, summary
