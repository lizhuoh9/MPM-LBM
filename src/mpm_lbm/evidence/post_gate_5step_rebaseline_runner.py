...
POST_GATE_5STEP_REBASELINE_FIELDS = [
    "row_name",
    "campaign_id",
    "gate_source_step",
...
def build_step78_post_gate_5step_rebaseline_matrix(
    root: Path,
    matrix_policy_path: str = "configs/step78_minimal_post_gate_canonical_driver_5step_rebaseline.json",
    acceptance_policy_path: str = "configs/step78_rebaseline_acceptance_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    matrix_policy = read_json(root / matrix_policy_path)
    acceptance_policy = read_json(root / acceptance_policy_path)
    enforce_step78_policy(matrix_policy)

    rows = [
        run_step78_5step_rebaseline_case(root, config_path, matrix_policy, acceptance_policy)
        for config_path in matrix_policy["required_rebaseline_configs"]
    ]
    summary = step78_5step_rebaseline_summary(rows, matrix_policy, acceptance_policy)
    return rows, summary


def run_step78_5step_rebaseline_case(
    root: Path,
    config_path: str,
    matrix_policy: dict,
    ...
    enforce_step78_config(config, row_name, acceptance_policy)
    ...
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step78 5-step rebaseline row: {row_name}")
    ...
    row = {
        ...
        **feature_flags,
        ...
        "stable": False,
        "notes": "minimal post-gate canonical driver 5-step rebaseline completed",
    }
    row["stable"] = step78_5step_rebaseline_row_pass(row, acceptance_policy)
    if not row["stable"]:
        raise RuntimeError(f"Step78 5-step rebaseline row failed acceptance: {row}")
    return row


def enforce_step78_policy(matrix_policy: dict) -> None:
    expected = {
        "post_gate_simulation_allowed": True,
        "allowed_next_step": "Step78",
        "allowed_next_step_scope": "minimal post-gate canonical driver 5-step rebaseline only",
        "gate_status": "ready_for_step78_5step_rebaseline_only",
    ...
    if actual != expected:
        raise RuntimeError(f"Step78 policy does not authorize the 5-step rebaseline envelope: {actual} != {expected}")
    if matrix_policy.get("required_rebaseline_configs") != [
        ...
        raise RuntimeError("Step78 must run exactly the canonical 32^3 5-step rebaseline config")


def enforce_step78_config(config, row_name: str, policy: dict) -> None:
    allowed = set(policy["required_row_names"])
    if row_name not in allowed:
        raise RuntimeError(f"unexpected Step78 5-step rebaseline row: {row_name}")
    expected = {
    ...
    if actual != expected:
        raise RuntimeError(f"Step78 config mismatch for {row_name}: {actual} != {expected}")
    if config.coupling_mode != "moving_boundary" or config.reaction_transfer_mode != "engineering":
        raise RuntimeError(f"Step78 row must use moving_boundary engineering transfer: {row_name}")
    if config.output_interval != 1 or config.write_vtk or config.write_particles:
        raise RuntimeError(f"Step78 row must use interval=1 and disable heavy outputs: {row_name}")
    if config.geometry_type != "box" or config.geometry_config_path is not None:
        raise RuntimeError(f"Step78 row must use default box geometry: {row_name}")
    if config.quality_check_enabled or config.quality_check_strict:
        raise RuntimeError(f"Step78 row must keep quality gates disabled: {row_name}")
    if config.boundary_motion_mode != "static" or config.boundary_motion_config_path is not None:
        raise RuntimeError(f"Step78 row must keep boundary motion static: {row_name}")
    if config.wall_velocity_application_mode != "disabled" or config.wall_velocity_application_config_path is not None:
        raise RuntimeError(f"Step78 row must keep wall velocity disabled: {row_name}")
    if config.geometry_motion_mode != "static" or config.geometry_motion_application_mode != "disabled":
        raise RuntimeError(f"Step78 row must keep geometry motion disabled: {row_name}")
...
def step78_5step_rebaseline_row_pass(row: dict, policy: dict) -> bool:
    ...
    return bool(
        ...
        and row["allowed_next_step"] == "Step78"
        and row["allowed_next_step_scope"] == "minimal post-gate canonical driver 5-step rebaseline only"
        and row["previous_rebaseline_step"] == "Step77"
...
def step78_5step_rebaseline_summary(rows: list[dict], matrix_policy: dict, acceptance_policy: dict) -> dict:
    ...
    summary = {
        ...
        "post_gate_5step_rebaseline_matrix_pass": False,
        "post_gate_simulation_allowed": bool(matrix_policy["post_gate_simulation_allowed"]),
        "previous_rebaseline_step": matrix_policy["previous_rebaseline_step"],
    ...
    summary["post_gate_5step_rebaseline_matrix_pass"] = bool(
        summary["gate_status"] == "ready_for_step78_5step_rebaseline_only"
        and summary["post_gate_simulation_allowed"] is True
        and summary["allowed_next_step"] == "Step78"
        and summary["allowed_next_step_scope"] == "minimal post-gate canonical driver 5-step rebaseline only"
        and summary["previous_rebaseline_step"] == "Step77"
...
def reset_generated_run_dir(run_root: Path, out_dir: Path) -> None:
    ...
    if resolved_out_dir.parent != resolved_run_root:
        raise RuntimeError(f"refusing to reset unexpected Step78 run directory: {out_dir}")
    if out_dir.exists():
...
def row_name_from_config(config_path: str) -> str:
    return Path(config_path).stem.removeprefix("step78_")
