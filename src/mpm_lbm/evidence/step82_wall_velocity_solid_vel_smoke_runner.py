...
def build_step82_wall_velocity_solid_vel_smoke_matrix(
    root: Path,
    matrix_policy_path: str = "configs/step82_wall_velocity_solid_vel_smoke_matrix.json",
    ...
    enforce_step82_policy(matrix_policy)
    rows = [
        run_step82_wall_velocity_solid_vel_case(root, config_path, matrix_policy, acceptance_policy)
        for config_path in matrix_policy["required_smoke_configs"]
    ]
    summary = step82_wall_velocity_solid_vel_summary(rows, matrix_policy, acceptance_policy)
    return rows, summary


def run_step82_wall_velocity_solid_vel_case(
    root: Path,
    config_path: str,
    matrix_policy: dict,
    ...
    enforce_step82_config(root, config, row_name, acceptance_policy)
    ...
    with report_repo_roots(root):
        diagnostics = driver.run()
    elapsed_seconds = time.perf_counter() - started
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step82 wall velocity smoke row: {row_name}")
    ...
    row = {
        ...
        **feature_flags,
        ...
        "apply_to_lbm_solid_vel": bool(wall_summary.get("apply_to_lbm_solid_vel", False)),
        ...
        "stable": False,
        "notes": "wall velocity solid_vel canonical driver 3-step smoke completed",
    }
    row["stable"] = step82_wall_velocity_solid_vel_row_pass(row, acceptance_policy)
    if not row["stable"]:
        raise RuntimeError(f"Step82 wall velocity solid_vel smoke row failed acceptance: {row}")
    return row


def enforce_step82_policy(matrix_policy: dict) -> None:
    expected = {
        "post_gate_simulation_allowed": True,
        "allowed_next_step": "Step82",
        "allowed_next_step_scope": "wall velocity solid_vel canonical driver 3-step smoke only",
        "gate_source_step": "Step81",
        "previous_step": "Step81",
    ...
    if actual != expected:
        raise RuntimeError(f"Step82 policy mismatch: {actual} != {expected}")
    if matrix_policy.get("required_smoke_configs") != [
        "configs/step82_canonical_driver_wall_velocity_solid_vel_32_3step_smoke.json"
    ]:
        raise RuntimeError("Step82 must run exactly one wall velocity solid_vel config")


def enforce_step82_config(root: Path, config, row_name: str, policy: dict) -> None:
    if row_name not in set(policy["required_row_names"]):
        raise RuntimeError(f"unexpected Step82 wall velocity smoke row: {row_name}")
    expected = {
    ...
    if actual != expected:
        raise RuntimeError(f"Step82 config mismatch for {row_name}: {actual} != {expected}")
    if config.coupling_mode != "moving_boundary" or config.reaction_transfer_mode != "engineering":
        raise RuntimeError(f"Step82 row must use moving_boundary engineering transfer: {row_name}")
    if config.geometry_type != "box" or config.geometry_config_path is not None:
        raise RuntimeError(f"Step82 row must use default box geometry: {row_name}")
    if config.output_interval != 1 or config.write_vtk or config.write_particles:
        raise RuntimeError(f"Step82 row must use interval=1 and disable heavy outputs: {row_name}")
    if config.quality_check_enabled or config.quality_check_strict:
        raise RuntimeError(f"Step82 row must keep quality gates disabled: {row_name}")
    if config.geometry_motion_mode != "static" or config.geometry_motion_application_mode != "disabled":
        raise RuntimeError(f"Step82 row must keep runtime geometry disabled: {row_name}")
    if config.boundary_motion_mode != "prescribed_kinematic":
        raise RuntimeError(f"Step82 row must use prescribed_kinematic boundary motion: {row_name}")
    if not config.boundary_motion_report_enabled:
        raise RuntimeError(f"Step82 row must write boundary motion report: {row_name}")
    if config.wall_velocity_application_mode != "solid_vel_experimental":
        raise RuntimeError(f"Step82 row must use solid_vel_experimental wall velocity: {row_name}")
    if not config.wall_velocity_application_report_enabled:
        raise RuntimeError(f"Step82 row must write wall velocity application report: {row_name}")
    if not path_exists_from_repo_root(root, config.boundary_motion_config_path):
        raise RuntimeError(f"Step82 boundary_motion_config_path is not resolvable: {config.boundary_motion_config_path}")
    if not path_exists_from_repo_root(root, config.wall_velocity_application_config_path):
        raise RuntimeError(
            "Step82 wall_velocity_application_config_path is not resolvable: "
            f"{config.wall_velocity_application_config_path}"
        )
...
def step82_wall_velocity_solid_vel_row_pass(row: dict, policy: dict) -> bool:
    ...
    return bool(
        ...
        and row["allowed_next_step"] == "Step82"
        and row["allowed_next_step_scope"] == "wall velocity solid_vel canonical driver 3-step smoke only"
        and row["previous_step"] == "Step81"
...
def step82_wall_velocity_solid_vel_summary(rows: list[dict], matrix_policy: dict, acceptance_policy: dict) -> dict:
    ...
    summary = {
        ...
        "allowed_next_step": matrix_policy["allowed_next_step"],
        "allowed_next_step_scope": matrix_policy["allowed_next_step_scope"],
        "boundary_motion_interface_report_pass_count": sum(1 for row in rows if row["boundary_motion_interface_report_pass"]),
        ...
        "step82_wall_velocity_solid_vel_smoke_matrix_pass": False,
    ...
    summary["step82_wall_velocity_solid_vel_smoke_matrix_pass"] = bool(
        summary["post_gate_simulation_allowed"] is True
        and summary["allowed_next_step"] == "Step82"
        and summary["allowed_next_step_scope"] == "wall velocity solid_vel canonical driver 3-step smoke only"
        and summary["previous_step"] == "Step81"
        ...
        and summary["physics_feature_expansion"] == "wall_velocity_solid_vel_only"
    )
    return summary
...
def reset_generated_run_dir(run_root: Path, out_dir: Path) -> None:
    ...
    if resolved_out_dir.parent != resolved_run_root:
        raise RuntimeError(f"refusing to reset unexpected Step82 run directory: {out_dir}")
    if out_dir.exists():
...
@contextmanager
def report_repo_roots(root: Path):
    from src.mpm_lbm.sim.motion import boundary_motion_config, boundary_motion_interface
    ...
    root_modules = [boundary_motion_config, boundary_motion_interface, application, application_config, config, field]
...
def row_name_from_config(config_path: str) -> str:
    return Path(config_path).stem.removeprefix("step82_")
