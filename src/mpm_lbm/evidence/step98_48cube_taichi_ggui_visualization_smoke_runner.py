...
def build_step98_48cube_taichi_ggui_visualization_smoke_matrix(
    root: Path,
    matrix_policy_path: str = "configs/step98_48cube_taichi_ggui_visualization_smoke_matrix.json",
    acceptance_policy_path: str = "configs/step98_48cube_taichi_ggui_visualization_acceptance_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    matrix_policy = read_json(root / matrix_policy_path)
    acceptance_policy = read_json(root / acceptance_policy_path)
    enforce_step98_policy(matrix_policy)
    rows = [
        run_step98_48cube_taichi_ggui_visualization_smoke_case(root, config_path, matrix_policy, acceptance_policy)
        for config_path in matrix_policy["required_run_configs"]
    ]
    summary = step98_48cube_taichi_ggui_visualization_smoke_summary(rows, matrix_policy, acceptance_policy)
    return rows, summary


def run_step98_48cube_taichi_ggui_visualization_smoke_case(
    root: Path,
    config_path: str,
    matrix_policy: dict,
    ...
    enforce_step98_config(root, config, row_name, acceptance_policy)
    ...
    reset_step98_output_dir(ggui_output_dir, root / "outputs")
    ...
    if not diagnostics:
        raise RuntimeError(f"empty diagnostics for Step98 48cube GGUI visualization smoke row: {row_name}")

    ggui_report = render_step98_48cube_ggui_visual_smoke(
    ...
    row = {
        ...
        "previous_step96_n_grid": int(matrix_policy["previous_step96_n_grid"]),
        "previous_step96_n_lbm_steps": int(matrix_policy["previous_step96_n_lbm_steps"]),
        "step98_n_grid": int(config.n_grid),
        "step98_n_lbm_steps": int(config.n_lbm_steps),
        "from_step96_grid_expansion": bool(matrix_policy["from_step96_grid_expansion"]),
        "from_step96_duration_reduction": bool(matrix_policy["from_step96_duration_reduction"]),
        "grid_expansion_smoke_isolation": bool(matrix_policy["grid_expansion_smoke_isolation"]),
        ...
        **feature_flags,
        ...
        "ggui_render_report_exists": (ggui_output_dir / "step98_ggui_visualization_report.json").is_file(),
        ...
        **{field: bool(geometry_summary.get(field, True)) for field in MUTATION_FLAG_FIELDS},
        ...
        "stable": False,
        "notes": (
            "Step98 renders the accepted first-user squid_proxy runtime geometry diagnostic-only "
            "wall velocity solid_vel envelope at 48^3 for one LBM step through Taichi GGUI."
        ),
    }
    row["stable"] = step98_48cube_taichi_ggui_visualization_smoke_row_pass(row, acceptance_policy)
    if not row["stable"]:
        raise RuntimeError(f"Step98 48cube Taichi GGUI visualization smoke row failed acceptance: {row}")
    return row


def render_step98_48cube_ggui_visual_smoke(
    driver_run_dir,
    ggui_config_path,
    output_dir,
    root: Path | None = None,
    ...
    report = render_step94_ggui_visual_smoke(driver_run_dir, ggui_config_path, output_dir, root)
    for stale_name in ("step94_ggui_visualization_metadata.json", "step94_ggui_visualization_report.json"):
        stale_path = output_dir / stale_name
        if stale_path.exists():
            stale_path.unlink()
    screenshot_path = resolve_under_root(root, ggui_config["screenshot_path"])
    report["visualization_note"] = (
        "GGUI visualizes Step98 procedural squid_proxy proxy points after the accepted first-user envelope "
        "runs at 48^3 for one LBM step; it is not a physical validation or swimming claim."
    )
    metadata = {
        "driver_run_dir": rel_path(root, resolve_under_root(root, driver_run_dir)),
        "ggui_config_path": rel_path(root, ggui_config_path),
        "render_report_path": "outputs/step98_ggui_visualization/step98_ggui_visualization_report.json",
        "screenshot_path": rel_path(root, screenshot_path),
        "visualization_source": report["visualization_source"],
    }
    write_json(output_dir / "step98_ggui_visualization_metadata.json", metadata)
    write_json(output_dir / "step98_ggui_visualization_report.json", report)
    return report


def enforce_step98_policy(matrix_policy: dict) -> None:
    expected = {
        "post_gate_simulation_allowed": True,
        "allowed_next_step": "Step98",
        "allowed_next_step_scope": "48^3 / 1-step Taichi GGUI visualization smoke passed for the first-user envelope",
        "gate_source_step": "Step97",
        "previous_step": "Step97",
    ...
    if actual != expected:
        raise RuntimeError(f"Step98 policy mismatch: {actual} != {expected}")
    if matrix_policy.get("required_run_configs") != [
        ...
        raise RuntimeError("Step98 must run exactly one 48cube GGUI visualization smoke config")


def enforce_step98_config(root: Path, config, row_name: str, policy: dict) -> None:
    if row_name not in set(policy["required_row_names"]):
        raise RuntimeError(f"unexpected Step98 48cube GGUI visualization smoke row: {row_name}")
    expected = {
    ...
    if actual != expected:
        raise RuntimeError(f"Step98 config mismatch for {row_name}: {actual} != {expected}")
    if config.coupling_mode != "moving_boundary" or config.reaction_transfer_mode != "engineering":
        raise RuntimeError(f"Step98 row must keep moving_boundary/engineering transfer: {row_name}")
    if list(config.target_u_lbm) != list(policy["required_target_u_lbm"]):
        raise RuntimeError(f"Step98 row must keep zero background flow: {row_name}")
    if config.geometry_type != policy["required_geometry_type"]:
        raise RuntimeError(f"Step98 row must use squid_proxy geometry: {row_name}")
    if config.geometry_config_path != policy["required_geometry_config_path"]:
        raise RuntimeError(f"Step98 row must use accepted squid_proxy geometry config: {row_name}")
    if not config.quality_check_enabled or config.quality_check_strict:
        raise RuntimeError(f"Step98 row must enable non-strict geometry quality reporting: {row_name}")
    if config.boundary_motion_mode != "prescribed_kinematic" or not config.boundary_motion_report_enabled:
        raise RuntimeError(f"Step98 row must write prescribed_kinematic boundary motion report: {row_name}")
    if config.geometry_motion_mode != "prescribed_kinematic":
        raise RuntimeError(f"Step98 row must use prescribed_kinematic geometry motion: {row_name}")
    if config.geometry_motion_application_mode != "diagnostic_only":
        raise RuntimeError(f"Step98 row must use diagnostic_only geometry motion application: {row_name}")
    if not config.geometry_motion_report_enabled or not config.geometry_motion_application_report_enabled:
        raise RuntimeError(f"Step98 row must write geometry motion reports: {row_name}")
    if config.wall_velocity_application_mode != "solid_vel_experimental":
        raise RuntimeError(f"Step98 row must use solid_vel_experimental wall velocity: {row_name}")
    if not config.wall_velocity_application_report_enabled:
        raise RuntimeError(f"Step98 row must write wall velocity application report: {row_name}")
    for label, relative_path in {
        ...
        if not path_exists_from_repo_root(root, relative_path):
            raise RuntimeError(f"Step98 {label} is not resolvable: {relative_path}")


def step98_48cube_taichi_ggui_visualization_smoke_row_pass(row: dict, policy: dict) -> bool:
    ...
    feature_pass = bool(
        int(row["activation_feature_count"]) == 5
    ...
    return bool(
        ...
        and row["allowed_next_step"] == "Step98"
        and row["previous_step"] == "Step97"
        and row["from_step96_grid_expansion"]
        and row["from_step96_duration_reduction"]
        and row["grid_expansion_smoke_isolation"]
        and int(row["previous_step96_n_grid"]) == 32
        and int(row["previous_step96_n_lbm_steps"]) == 10
        and int(row["step98_n_grid"]) == 48
        and int(row["step98_n_lbm_steps"]) == 1
        and row["physics_feature_expansion"] == "48cube_taichi_ggui_visualization_smoke_only"
...
def step98_48cube_taichi_ggui_visualization_smoke_summary(
    ...
    summary = {
        ...
        "post_gate_simulation_allowed": bool(matrix_policy["post_gate_simulation_allowed"]),
        "previous_step": matrix_policy["previous_step"],
        "previous_step96_n_grid": min(int(row["previous_step96_n_grid"]) for row in rows),
        "previous_step96_n_lbm_steps": min(int(row["previous_step96_n_lbm_steps"]) for row in rows),
        ...
        "step98_48cube_taichi_ggui_visualization_smoke_matrix_pass": False,
        "step98_n_grid": min(int(row["step98_n_grid"]) for row in rows),
        "step98_n_lbm_steps": min(int(row["step98_n_lbm_steps"]) for row in rows),
    ...
    summary["step98_48cube_taichi_ggui_visualization_smoke_matrix_pass"] = bool(
        summary["post_gate_simulation_allowed"] is True
        and summary["allowed_next_step"] == "Step98"
        and summary["previous_step"] == "Step97"
        and summary["from_step96_grid_expansion"] is True
        and summary["from_step96_duration_reduction"] is True
        and summary["grid_expansion_smoke_isolation"] is True
        ...
        and summary["activation_feature_count"] == 5
        ...
        and summary["min_total_mpm_substeps"] >= 1
        and summary["previous_step96_n_grid"] == 32
        and summary["previous_step96_n_lbm_steps"] == 10
        and summary["step98_n_grid"] == 48
        and summary["step98_n_lbm_steps"] == 1
        and summary["runtime_hard_fail_count"] == 0
        and not summary["runtime_code_changed"]
        and not summary["solver_behavior_changed"]
        and summary["physics_feature_expansion"] == "48cube_taichi_ggui_visualization_smoke_only"
    )
    return summary


def reset_step98_output_dir(out_dir: Path, required_parent: Path) -> None:
    import shutil

    resolved_out = out_dir.resolve()
    resolved_parent = required_parent.resolve()
    if resolved_parent not in resolved_out.parents:
        raise RuntimeError(f"refusing to reset unexpected Step98 output directory: {out_dir}")
    if out_dir.exists():
...
def row_name_from_config(config_path: str) -> str:
    return Path(config_path).stem.removeprefix("step98_")
