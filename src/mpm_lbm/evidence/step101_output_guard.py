...
def build_step101_output_guard(
    root: Path,
    policy_path: str = "configs/step101_output_guard_policy.json",
    ...
    rows = [output_row(root, path, policy) for path in step101_output_files(root)]
    ...
    driver_run_dir_count = step101_driver_run_dir_count(root)
    ...
    artifact_budget_pass = bool(total_size_mb < float(policy["max_step101_total_size_mb"]) and large_file_count == 0)
    summary = {
        ...
        "row_count": len(rows),
        "step101_dense_displacement_output_count": sum(1 for row in rows if "dense_displacement" in row["path"].lower()),
        "step101_dense_wall_velocity_output_count": sum(1 for row in rows if "dense_wall_velocity" in row["path"].lower()),
        "step101_displaced_particle_output_count": sum(1 for row in rows if "displaced_particle" in row["path"].lower()),
        "step101_driver_run_dir_count": driver_run_dir_count,
        "step101_forbidden_file_count": len(forbidden_rows),
        "step101_ggui_screenshot_count": sum(1 for row in rows if row["extension"] in screenshot_extensions),
        "step101_ggui_video_count": sum(1 for row in rows if row["extension"] in video_extensions),
        "step101_large_file_count": large_file_count,
        "step101_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step101_raw_geometry_output_count": sum(1 for row in rows if row["extension"] in raw_geometry_extensions),
        "step101_real_geometry_candidate_output_count": sum(
            1 for row in rows if "real_geometry_candidate" in row["path"].lower()
        ),
        "step101_sparse_wall_velocity_output_count": sum(1 for row in rows if "sparse_wall_velocity" in row["path"].lower()),
        "step101_total_size_bytes": total_size_bytes,
        "step101_total_size_mb": total_size_mb,
        "step101_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        ...
        and summary["step101_dense_wall_velocity_output_count"] == 0
        and summary["step101_sparse_wall_velocity_output_count"] == 0
        and summary["step101_dense_displacement_output_count"] == 0
        and summary["step101_displaced_particle_output_count"] == 0
        and summary["step101_raw_geometry_output_count"] == 0
        and summary["step101_real_geometry_candidate_output_count"] == 0
        and summary["step101_forbidden_file_count"] == 0
...
def step101_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step101_") or rel.startswith("logs/step101_"):
                files.append(path)
    return sorted(files)
...
def is_forbidden(root: Path, path: Path, policy: dict) -> bool:
    ...
    if any(token.lower() in lower for token in policy["forbidden_filename_tokens"]):
        return True
    return any(rel.startswith(protected) for protected in policy["protected_prefixes"])
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step101_output_files(root):
