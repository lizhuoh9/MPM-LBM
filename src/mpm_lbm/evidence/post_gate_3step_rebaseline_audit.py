...
from .post_gate_3step_rebaseline_runner import step77_3step_rebaseline_row_pass


def build_step77_post_gate_3step_rebaseline_audit(
    root: Path,
    matrix_artifact_path: str = "outputs/step77_post_gate_3step_rebaseline_matrix/post_gate_3step_rebaseline_matrix.json",
    ...
    summary = {
        ...
        "post_gate_3step_rebaseline_audit_pass": False,
        "required_row_count": len(required_names),
        "row_count": len(rows),
        "stable_count": sum(1 for row in rows if row["stable"]),
    }
    summary["post_gate_3step_rebaseline_audit_pass"] = bool(
...
def audit_row(row: dict, policy: dict) -> dict:
    passed = bool(row["stable"] and step77_3step_rebaseline_row_pass(row, policy))
    return {
