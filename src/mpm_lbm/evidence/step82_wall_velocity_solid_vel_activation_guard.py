...
def build_step82_wall_velocity_solid_vel_activation_guard(
    root: Path,
    policy_path: str = "configs/step82_activation_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    matrix = read_json(root / policy["matrix_artifact_path"])
    step81_guard = read_json(root / policy["step81_activation_guard_artifact_path"])
    rows = []
    for row in matrix["rows"]:
        rows.extend(row_activation_checks(row, policy))
    rows.extend(matrix_summary_checks(matrix["summary"], policy))
    rows.extend(step81_summary_checks(step81_guard["summary"], policy))
    summary = {
        "activation_feature_count": matrix["summary"]["activation_feature_count"],
        "pass_count": sum(1 for row in rows if row["pass"]),
        "planned_step82_activation_feature_count": step81_guard["summary"]["planned_step82_activation_feature_count"],
        "row_count": len(rows),
        "runtime_geometry_enabled_count": matrix["summary"]["runtime_geometry_enabled_count"],
        "step82_activation_guard_pass": False,
        "wall_velocity_enabled_count": matrix["summary"]["wall_velocity_enabled_count"],
    }
    summary["step82_activation_guard_pass"] = bool(
        rows
        ...
        and summary["planned_step82_activation_feature_count"] == int(
            policy["expected_step81_planned_step82_activation_feature_count"]
        )
    )
    return rows, summary
...
def step81_summary_checks(summary: dict, policy: dict) -> list[dict]:
    return [
        literal_row("step81_guard", "step81_activation_guard_pass", summary["step81_wall_velocity_single_feature_activation_guard_pass"], True),
        literal_row(
            "step81_guard",
            "planned_step82_activation_feature_count",
            summary["planned_step82_activation_feature_count"],
            policy["expected_step81_planned_step82_activation_feature_count"],
        ),
        literal_row("step81_guard", "step81_activation_feature_count", summary["step81_activation_feature_count"], 0),
    ]
