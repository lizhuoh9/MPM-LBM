...
def build_step90_first_user_simulation_dry_run_audit(
    root: Path,
    matrix_artifact_path: str = (
        "outputs/step90_first_user_simulation_dry_run_matrix/first_user_simulation_dry_run_matrix.json"
    ),
    ...
    for row in payload["rows"]:
        rows.extend(
            [
                ...
                literal_row(row["row_name"], "completed_lbm_steps", row["completed_lbm_steps"], 5),
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step90_first_user_simulation_dry_run_audit_pass": False,
    }
    summary["step90_first_user_simulation_dry_run_audit_pass"] = bool(
        rows
        and summary["pass_count"] == summary["row_count"]
        and payload["summary"]["step90_first_user_simulation_dry_run_matrix_pass"] is True
    )
    return rows, summary
