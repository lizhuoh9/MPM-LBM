...
def build_canonical_driver_32_probe_output_guard(
    root: Path,
    output_policy_path: str = "configs/step61_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / output_policy_path)
    run_root = root / "outputs" / "step61_driver_runs"
    rows = [output_row(root, path, run_root, policy) for path in step61_output_files(root)]
    ...
    summary = {
        ...
        "step61_driver_run_dir_count": len(run_dirs),
        "step61_required_driver_run_dir_count": sum(1 for path in run_dirs if not path.name.endswith("_optional")),
        "step61_optional_driver_run_dir_count": sum(1 for path in run_dirs if path.name.endswith("_optional")),
        "step61_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
        "step61_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step61_large_file_count": sum(1 for row in rows if int(row["size_bytes"]) > LARGE_FILE_THRESHOLD_BYTES),
        "step61_forbidden_file_count": len(forbidden_rows),
        "step61_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step61_total_size_mb": sum(int(row["size_bytes"]) for row in rows) / (1024.0 * 1024.0),
    ...
    summary["output_guard_pass"] = bool(
        summary["step61_required_driver_run_dir_count"] == int(policy["expected_required_driver_run_dir_count"])
        and summary["step61_optional_driver_run_dir_count"] == 0
        and summary["step61_vtr_count"] == 0
        and summary["step61_particle_npy_count"] == 0
        and summary["step61_large_file_count"] == 0
        and summary["step61_forbidden_file_count"] == 0
        and float(summary["step61_total_size_mb"]) < float(policy["max_step61_total_size_mb"])
...
def step61_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step61_") or rel.startswith("logs/step61_"):
                files.append(path)
    return sorted(files)
