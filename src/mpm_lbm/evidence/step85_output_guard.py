...
def build_step85_output_guard(
    root: Path,
    policy_path: str = "configs/step85_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    raw_geometry_extensions = set(policy["raw_geometry_extensions"])
    rows = [output_row(root, path, policy) for path in step85_output_files(root)]
    forbidden_rows = [row for row in rows if row["forbidden"]]
    driver_dirs = step85_driver_run_dirs(root)
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step85_dense_displacement_output_count": sum(1 for row in rows if "dense_displacement" in row["path"].lower()),
        "step85_dense_wall_velocity_output_count": sum(1 for row in rows if "dense_wall_velocity" in row["path"].lower()),
        "step85_displaced_particle_output_count": sum(1 for row in rows if "displaced_particle" in row["path"].lower()),
        "step85_driver_run_dir_count": len(driver_dirs),
        "step85_forbidden_file_count": len(forbidden_rows),
        "step85_large_file_count": sum(1 for row in rows if int(row["size_bytes"]) > int(policy["large_file_threshold_bytes"])),
        "step85_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step85_raw_geometry_output_count": sum(1 for row in rows if row["extension"] in raw_geometry_extensions),
        "step85_real_geometry_candidate_output_count": sum(
            1 for row in rows if "real_geometry_candidate" in row["path"].lower()
        ),
        "step85_sparse_wall_velocity_output_count": sum(1 for row in rows if "sparse_wall_velocity" in row["path"].lower()),
        "step85_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step85_total_size_mb": sum(int(row["size_bytes"]) for row in rows) / (1024.0 * 1024.0),
        "step85_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        ...
        and summary["step85_dense_wall_velocity_output_count"] == 0
        and summary["step85_sparse_wall_velocity_output_count"] == 0
        and summary["step85_dense_displacement_output_count"] == 0
        and summary["step85_displaced_particle_output_count"] == 0
        and summary["step85_raw_geometry_output_count"] == 0
        and summary["step85_real_geometry_candidate_output_count"] == 0
        and summary["step85_large_file_count"] == 0
        and summary["step85_forbidden_file_count"] == 0
...
def step85_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step85_") or rel.startswith("logs/step85_"):
                files.append(path)
    return sorted(files)
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step85_output_files(root):
