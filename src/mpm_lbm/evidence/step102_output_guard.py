...
def build_step102_output_guard(
    root: Path,
    policy_path: str = "configs/step102_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    screenshot_extensions = set(policy["screenshot_extensions"])
    video_extensions = set(policy["video_extensions"])
    rows = [output_row(root, path, policy) for path in step102_output_files(root)]
    ...
    artifact_budget_pass = bool(total_size_mb < float(policy["max_step102_total_size_mb"]) and large_file_count == 0)
    summary = {
        ...
        "row_count": len(rows),
        "step102_ansys_proprietary_file_count": sum(1 for row in rows if is_ansys_proprietary_output(row, policy)),
        "step102_driver_run_dir_count": step102_driver_run_dir_count(root),
        "step102_fluent_run_output_count": sum(1 for row in rows if is_fluent_run_output(row, policy)),
        "step102_forbidden_file_count": len(forbidden_rows),
        "step102_ggui_screenshot_count": sum(1 for row in rows if row["extension"] in screenshot_extensions),
        "step102_ggui_video_count": sum(1 for row in rows if row["extension"] in video_extensions),
        "step102_large_file_count": large_file_count,
        "step102_particle_npy_count": sum(
            1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()
        ),
        "step102_total_size_bytes": total_size_bytes,
        "step102_total_size_mb": total_size_mb,
        "step102_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        ...
        and summary["step102_forbidden_file_count"] == 0
...
def step102_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step102_") or rel.startswith("logs/step102_"):
                files.append(path)
    return sorted(files)
...
def is_ansys_proprietary_output(row: dict, policy: dict) -> bool:
    lower = row["path"].lower()
    return any(token.lower() in lower for token in ("fsi_2way.zip", "flap.msh", "steady_fluid_flow.jou")) or lower.endswith(
        tuple(suffix.lower() for suffix in policy["case_data_suffixes"])
    )
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step102_output_files(root):
