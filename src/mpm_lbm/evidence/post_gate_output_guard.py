...
def build_step76_post_gate_output_guard(
    root: Path,
    policy_path: str = "configs/step76_output_guard_policy.json",
) -> tuple[list[dict], dict]:
    root = Path(root)
    policy = read_json(root / policy_path)
    rows = [output_row(root, path, policy) for path in step76_output_files(root)]
    ...
    summary = {
        ...
        "row_count": len(rows),
        "step76_forbidden_file_count": len(forbidden_rows),
        "step76_large_file_count": sum(1 for row in rows if int(row["size_bytes"]) > int(policy["large_file_threshold_bytes"])),
        "step76_optional_driver_run_dir_count": len(optional_dirs),
        "step76_particle_npy_count": sum(1 for row in rows if row["extension"] == ".npy" and "particle" in row["path"].lower()),
        "step76_required_driver_run_dir_count": len(required_dirs),
        "step76_total_size_bytes": sum(int(row["size_bytes"]) for row in rows),
        "step76_total_size_mb": sum(int(row["size_bytes"]) for row in rows) / (1024.0 * 1024.0),
        "step76_vtr_count": sum(1 for row in rows if row["extension"] == ".vtr"),
    }
    summary["output_guard_pass"] = bool(
        rows
        and summary["row_count"] == summary["pass_count"]
        and summary["step76_required_driver_run_dir_count"] == len(policy["required_driver_run_dirs"])
        and summary["step76_optional_driver_run_dir_count"] == 0
        and summary["step76_vtr_count"] == 0
        and summary["step76_particle_npy_count"] == 0
        and summary["step76_large_file_count"] == 0
        and summary["step76_forbidden_file_count"] == 0
        and summary["step76_total_size_mb"] < float(policy["max_step76_total_size_mb"])
...
def step76_output_files(root: Path):
    files = []
    for base in (root / "outputs", root / "logs"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(root).as_posix()
            if rel.startswith("outputs/step76_") or rel.startswith("logs/step76_"):
                files.append(path)
    return sorted(files)
...
def private_absolute_path_rows(root: Path) -> list[str]:
    paths = []
    for path in step76_output_files(root):
